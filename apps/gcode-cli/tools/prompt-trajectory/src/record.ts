...
interface GCodeBootstrapModule {
  createModelAdapter(input: unknown): unknown;
  createGCodeApp(input: unknown): Promise<{
    close?: () => Promise<void>;
...
interface GCodeContractsModule {
...
export async function recordPromptTrajectoryFromFixture(input: {
  ...
  try {
    const { bootstrap, contracts, provider } = await loadGCodeModules();
    ...
    const app = await bootstrap.createGCodeApp({
...
async function loadGCodeModules(): Promise<{
  bootstrap: GCodeBootstrapModule;
  contracts: GCodeContractsModule;
  provider: GCodeProviderModule;
}> {
  ...
  try {
    const [bootstrap, contracts, providerConfig, providerRegistry] = await Promise.all([
      import(bootstrapPath) as Promise<GCodeBootstrapModule>,
      import(contractsPath) as Promise<GCodeContractsModule>,
      import(providerConfigPath) as Promise<Omit<GCodeProviderModule, "ProviderRegistry">>,
      import(providerRegistryPath) as Promise<Pick<GCodeProviderModule, "ProviderRegistry">>,
    ]);
    return {
      bootstrap,
      contracts,
    ...
    throw new Error(
      "Missing built gcode-cli packages. Run `pnpm --filter @gcode/bootstrap^... build && pnpm --filter @gcode/bootstrap build` before `record`.",
      { cause: error },
    );
  }
}
