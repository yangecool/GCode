# @gcode/prompt-trajectory

OpenAI protocol trajectory recorder for inspecting gcode-cli prompt assembly.
...
```bash
pnpm --filter @gcode/bootstrap^... build
pnpm --filter @gcode/bootstrap build

pnpm --filter @gcode/prompt-trajectory record -- \
  --fixture /path/to/recording.json \
  --out /tmp/gcode-prompt-trajectory/basic-live

pnpm --filter @gcode/prompt-trajectory record:prompt -- \
  --prompt "Say hello in one short sentence."

pnpm --filter @gcode/prompt-trajectory derive -- \
  --out /tmp/gcode-prompt-trajectory/basic-live

pnpm --filter @gcode/prompt-trajectory model-io -- \
  --input ~/.gcode/cli/debug/model-io-<session>.jsonl \
  --out /tmp/gcode-prompt-trajectory/model-io-session
```
...
uses the same gcode model config resolution as the CLI. The upstream request is
...
`model-io` reads a real GCode `model-io-*.jsonl` file and turns the main
