...
if (check) {
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "gcode-bash-registry-"));
  ...
  try {
    ...
    if (!actual.equals(expected)) {
      throw new Error(
        "Generated Bash command registry is stale. Run `pnpm --dir apps/gcode-cli registry:generate`.",
      );
    }
  } finally {
