...
const cliBundle = resolve(dist, "gcode.cjs");
const seaBlobForTarget = (target) => resolve(dist, `gcode-${target}.sea.blob`);
    ...
    JSON.stringify(
      {
        assets: {
          ...
          "gcode-node-license": nodeLicensePath,
        },
        disableExperimentalSEAWarning: true,
        main: "dist/gcode.cjs",
        output: `dist/gcode-${target}.sea.blob`,
        useCodeCache: false,
        useSnapshot: false,
      },
      null,
...
const smokeTestHostTarget = async (target, binaryPath) => {
  if (isHostTarget(target)) {
    const storageRoot = await mkdtemp(join(tmpdir(), "gcode-sea-smoke-"));
    try {
      const env = {
        ...process.env,
        GCODE_STORAGE_DIR: storageRoot,
      };
      delete env.GCODE_BFS_BINARY;
      delete env.GCODE_RG_BINARY;
      delete env.GCODE_UGREP_BINARY;
...
export const main = async (argv = process.argv.slice(2)) => {
  ...
  if (!existsSync(cliBundle)) {
    throw new Error("Missing dist/gcode.cjs. Run `pnpm build` first.");
  }
