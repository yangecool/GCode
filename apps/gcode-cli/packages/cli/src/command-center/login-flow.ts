import type { TuiSelection, TuiSubmitPrompt } from "@gcode/tui";
...
export function buildLoginSelection(locale?: string): TuiSelection {
  const copy = getGCodeCopy(locale).tui.loginSetup;
  return {
...
export function loginSetupResponse(locale?: string): string {
  return getGCodeCopy(locale).tui.loginSetup.response;
}
