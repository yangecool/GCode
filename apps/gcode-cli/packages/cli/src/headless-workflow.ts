...
import { createDenyPermissionBroker } from "@gcode/core";
...
export const createHeadlessPermissionBroker = (): NonNullable<
  GCodeAppOptions["permissionBroker"]
> => {
 ...
 *
 * 刻意**不**进 `gcodeSessionEventTypeSchema`（`shared/src/gcode-protocol/index.ts`）：
 * 那是个闭集 `z.enum` 且喂给 `gcodeSessionEventSchema` 的 discriminated union，加值等于让
 ...
 * `RunDependencies.createGCodeApp` 注入点边界）。
 *
