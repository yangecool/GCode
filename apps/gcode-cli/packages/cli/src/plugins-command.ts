...
import type { SetGCodePluginEnabledResult, GCodePluginInstallData } from "@gcode/bootstrap";
import {
...
const PLUGINS_COMMAND_USAGE = `Usage: gcode plugins <command> [options]
...
Scopes: user (default), project. \`gcode plugin\` is an alias of \`gcode plugins\`.`;
...
async function runPluginsUpdateCommand(
  ...
  ctx.stdout.write(
    version && version === result.previousVersion
      ...
      : `Updated plugin ${pluginId} from ${result.previousVersion || "unknown"} to ${version || "unknown"}. Restart gcode to apply.\n`,
  );
...
async function runPluginsDisableCommand(
  ...
  const entries: Array<{ plugin: PluginListItem; result?: SetGCodePluginEnabledResult; error?: string }> = [];
