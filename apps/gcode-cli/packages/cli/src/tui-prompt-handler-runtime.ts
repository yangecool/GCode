...
// 品牌类型（SessionId）和 createGCodeApp 的联合签名收窄错。
export async function prepareTuiAppRuntime(
  ...
  const createAppFactory = deps.createGCodeApp ?? bootstrapModule?.createGCodeApp;
  if (!createAppFactory) throw new Error("GCode app factory is unavailable.");
  const prepareTelemetry =
    deps.prepareGCodeTelemetryEnv ?? bootstrapModule?.prepareGCodeTelemetryEnv;
  if (prepareTelemetry) {
    state.shutdownTelemetry =
      deps.shutdownGCodeTelemetry ?? bootstrapModule?.shutdownGCodeTelemetry;
  }
