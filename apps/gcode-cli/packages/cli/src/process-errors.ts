...
import {
  GCODE_PROCESS_DIAGNOSTIC_PREFIX,
...
function writeProcessErrorDiagnostic(
  ...
  try {
    const detail = formatProcessError(reason).slice(0, GCODE_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS);
    const diagnostic: GCodeProcessDiagnostic = {
      version: 1,
      errorId: randomUUID(),
      kind,
    ...
    stderr.write(
      `${GCODE_PROCESS_DIAGNOSTIC_PREFIX}${JSON.stringify(diagnostic)}\n[gcode] process error kind=${kind} origin=${origin}\n${detail}\n`,
    );
  } catch {
    // 诊断输出不能再次击穿进程级异常边界。
  }
