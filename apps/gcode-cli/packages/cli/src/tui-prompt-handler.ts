import { getDefaultConfigPath, updateUiLocaleInFileConfig } from "@gcode/adapters/config";
import type { SessionEvent } from "@gcode/contracts";
import type { GCodeAppOptions } from "@gcode/bootstrap";
import { DEFAULT_LOCALE, type SupportedLocale } from "@gcode/i18n";
import type { TuiRequestPermission } from "@gcode/tui";
...
export function createTuiSubmitPrompt(
  ...
  let app: Awaited<ReturnType<NonNullable<RunDependencies["createGCodeApp"]>>> | undefined;
  ...
  const permissionBroker: NonNullable<GCodeAppOptions["permissionBroker"]> = {
  ...
  const replaceApp = async (
    factory: () => Promise<Awaited<ReturnType<NonNullable<RunDependencies["createGCodeApp"]>>>>,
  ...
  const createApp = async (request: CliResumeRequest) => {
    ...
    let createdApp: Awaited<ReturnType<NonNullable<RunDependencies["createGCodeApp"]>>>;
    try {
