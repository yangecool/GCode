...
import { formatJson, type PresentationSurface } from "@gcode/core";
import type { RunContext, GlobalOptions } from "@gcode/shared-types";
...
export const runPrompt = async (
  ...
  let app:
    | Awaited<ReturnType<Awaited<ReturnType<typeof loadBootstrapModule>>["createGCodeApp"]>>
    | undefined;
  ...
  try {
    ...
    const bootstrapModule = deps.createGCodeApp ? undefined : await loadBootstrapModule();
    const createApp = deps.createGCodeApp ?? bootstrapModule?.createGCodeApp;
    if (!createApp) throw new Error("GCode app factory is unavailable.");
    ...
    const prepareTelemetry =
      deps.prepareGCodeTelemetryEnv ?? bootstrapModule?.prepareGCodeTelemetryEnv;
    if (prepareTelemetry) {
      shutdownTelemetry = deps.shutdownGCodeTelemetry ?? bootstrapModule?.shutdownGCodeTelemetry;
    }
    const appEnv = prepareTelemetry
      ? await prepareTelemetry(env, {
          cliVersion: version,
          productVersion: env.GCODE_APP_VERSION,
        })
      : env;
    ...
    // 回合。结果是 `gcode -p "/workflow ..."` 在第一个回合后就退出，把在飞的 run 孤儿化
...
async function isResolvableCustomCommand(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  //
  // 探测刻意用「保留名检查 + load」这一对，而不是直接调 resolveGCodeCustomCommandPrompt：
...
async function isReservedSlashCommandName(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  return bootstrap.isReservedGCodeSlashCommandName(name);
}
...
function writeHeadlessWorkspaceHookTrustDiagnostic(
  ...
  ctx.stderr.write(
    [
      ...
      `Review with: gcode hooks trust review --workspace ${JSON.stringify(status.workspaceIdentity)}`,
    ].join("\n") + "\n",
  );
}
