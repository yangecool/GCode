...
import {
  ...
  normalizeGCodeRuntimeEnv,
  sanitizeGCodeRuntimeEnv,
  sanitizeGCodeRuntimeEnvInPlace,
...
export function prepareCliRuntimeEnv(
  ...
  const prepared = {
    ...sanitizeGCodeRuntimeEnv(env),
    ...buildGCodeToolEnvPassthroughEnv(env),
  };
...
export function applyCliRuntimeEnvSanitization(
  ...
  const toolEnvPassthrough = buildGCodeToolEnvPassthroughEnv(env);
  sanitizeGCodeRuntimeEnvInPlace(env);
...
function applyCliRuntimeEnvDefaults(env: CliEnv, argv: readonly string[]): void {
  env[GCODE_RUNTIME_ENV_KEY] = resolveCliRuntimeEnv(env, argv);
  applyBetaStorageDefault(env, argv);
}

function resolveCliRuntimeEnv(env: CliEnv, argv: readonly string[]): GCodeRuntimeEnv {
  const explicit = normalizeGCodeRuntimeEnv(env[GCODE_RUNTIME_ENV_KEY]);
  if (explicit) {
    return explicit;
  }
...
function applyBetaStorageDefault(env: CliEnv, argv: readonly string[]): void {
  if (env.GCODE_STORAGE_DIR?.trim()) return;
  const explicitBeta = env.GCODE_BETA === "1" || env.GCODE_ENV === "beta";
  const invokedAsBeta = argv.some((arg) => /(^|[/\\])gcode-beta(?:$|\.)/u.test(arg));
  if (!explicitBeta && !invokedAsBeta) return;
  env.GCODE_STORAGE_DIR = join(homedir(), ".gcode-beta");
}
