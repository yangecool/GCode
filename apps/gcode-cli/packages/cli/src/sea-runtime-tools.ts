...
import { getRuntimeToolRuntime, type RuntimeToolId } from "@gcode/shared/runtime-tool-runtime";
...
const assetPrefix = "gcode-runtime-tools/";
const manifestAssetKey = `${assetPrefix}manifest.json`;
const markerFileName = ".gcode-runtime-tool.json";
...
export async function ensureSeaRuntimeTools(
  ...
  const configuredStorageRoot = options.storageRoot ?? env.GCODE_STORAGE_DIR?.trim();
  const storageRoot = configuredStorageRoot || join(homedir(), ".gcode");
