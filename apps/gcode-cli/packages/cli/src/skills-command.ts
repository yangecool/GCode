...
type BootstrapModule = typeof import("@gcode/bootstrap");
type CliSkillListOutcome = Awaited<ReturnType<typeof listGCodeSkills>>;
...
const SKILLS_COMMAND_USAGE = "Usage: gcode skills [list|inspect <name>]";
...
async function runSkillsListCommand(
  ...
  try {
    ...
    const listSkills = deps.listSkills ?? (await bootstrap()).listGCodeSkills;
...
async function runSkillsInspectCommand(
  ...
  try {
    ...
    const workingDirectory = (deps.cwd ?? process.cwd)();
    const bootstrap = deps.loadBootstrapModule ?? (() => import("@gcode/bootstrap"));
    const inspectSkill = deps.inspectSkill ?? (await bootstrap()).inspectGCodeSkill;
...
function formatHumanSkillInspection(
  inspection: GCodeSkillInspection,
  options: GlobalOptions,
): string {
...
const formatSkillInspectionJson = (inspection: GCodeSkillInspection, cwd: string): string =>
  formatJson({
    cwd,
