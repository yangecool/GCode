...
import { createNodeLoggerFactory } from "@gcode/adapters";
import { getRuntimeInfo, type PresentationSurface } from "@gcode/core";
import { color, formatJson, supportsColor } from "@gcode/core";
import { getGCodeCopy, isUiLocale, type UiLocale } from "@gcode/i18n";
import type { RunContext, GlobalOptions, GlobalOutputFormat } from "@gcode/shared-types";
import {
...
const normalizeLocaleOption = (value: string | undefined): UiLocale | undefined => {
  ...
  throw new Error(getGCodeCopy().cli.errors.localeUnsupported(value));
};
...
const normalizePresentationSurface = (value: string | undefined): PresentationSurface => {
  ...
  if (value.toLowerCase() === "desktop") return "gcode_desktop";
...
const runDoctor = (ctx: RunContext, options: GlobalOptions, workingDirectory: string): number => {
  ...
  ctx.stdout.write(`${color.bold("gcode doctor", colors)}\n`);
...
const runGCodeProtocolCommand = async (
  ctx: RunContext,
  ...
  try {
    ...
    // 直接退出，外层只能看到 GCode agent transport closed。
    ...
    const runProtocolAgent =
      deps.runGCodeProtocolAgent ?? (await loadBootstrapModule()).runGCodeProtocolAgent;
...
export const run = async (ctx: RunContext, deps: RunDependencies = {}): Promise<number> => {
  ...
  const commandDeps: RunDependencies = {
    ...
    logger:
      deps.logger ??
      createNodeLoggerFactory({ env }).createLogger("gcode").child({ module: "cli" }),
  ...
  switch (commandName(parsed.positionals)) {
    ...
    case "agent-server":
    case "app-server":
      return await runGCodeProtocolCommand(
        ctx,
        options,
        commandDeps,
        presentationSurface,
