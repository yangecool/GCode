...
import type { TuiClipboardImage, TuiImageMediaType, TuiReadClipboardImage } from "@gcode/tui";
...
function resolveDefaultClipboardDirectory(processEnv: NodeJS.ProcessEnv = process.env): string {
  const storageRoot = processEnv.GCODE_STORAGE_DIR?.trim() || join(homedir(), ".gcode");
...
async function readMacClipboardImage(options: {
  ...
  const directory = await mkdtemp(join(options.tempDirectory, "gcode-clipboard-"));
