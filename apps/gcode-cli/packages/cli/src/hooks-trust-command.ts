...
const USAGE = `Usage:
  gcode hooks trust status [--workspace <path-or-identity>] [--json]
  gcode hooks trust review [--workspace <path-or-identity>] [--json]
  gcode hooks trust grant --workspace <path-or-identity> --hook-digest <sha256> [--hook-digest <sha256> ...]
  gcode hooks trust grant --workspace <path-or-identity> --all-current --bundle-digest <sha256>
  gcode hooks trust revoke --workspace <path-or-identity> [--hook-digest <sha256> ... | --all]
`;
...
type HooksCommandDependencies = RunDependencies & {
  loadBootstrapModule?: () => Promise<typeof import("@gcode/bootstrap")>;
...
function formatHuman(status: WorkspaceHookTrustCliStatus, action: string): string {
  ...
  if (status.reasonCode === "workspace_hooks_pending_trust" && status.bundleDigest) {
    lines.push(
      ...
      `  gcode hooks trust grant --workspace ${quote(status.workspaceIdentity)} --hook-digest <sha256>`,
      "Or trust every currently enabled declaration in this exact bundle with:",
      `  gcode hooks trust grant --workspace ${quote(status.workspaceIdentity)} --all-current --bundle-digest ${status.bundleDigest}`,
    );
  }
