...
type CustomCommandListOutcome = Awaited<ReturnType<typeof listGCodeCustomCommands>>;
...
const COMMANDS_COMMAND_USAGE = "Usage: gcode commands [list|inspect <name>]";
...
async function resolveListCustomCommands(deps: CommandsCommandDependencies) {
  ...
  return (await bootstrap()).listGCodeCustomCommands;
}
...
async function resolveInspectCustomCommand(deps: CommandsCommandDependencies) {
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@gcode/bootstrap"));
  return (await bootstrap()).inspectGCodeCustomCommand;
}
...
function formatHumanCommandInspection(
  inspection: GCodeCustomCommandInspection,
  options: GlobalOptions,
): string {
