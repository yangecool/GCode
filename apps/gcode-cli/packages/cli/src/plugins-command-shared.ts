...
import type { listGCodePlugins } from "@gcode/bootstrap";
...
export type PluginListOutcome = ReturnType<typeof listGCodePlugins>;
 ...
 * 避免 `gcode plugins list` 这类轻命令把整个 bootstrap 图提前拉起来。
 */
export const BOOTSTRAP_EXPORTS = {
  addMarketplace: "addGCodePluginMarketplace",
  getPluginsOverview: "getGCodePluginsOverview",
  installPlugin: "installGCodeMarketplacePlugin",
  listPlugins: "listGCodePlugins",
  removeMarketplace: "removeGCodePluginMarketplace",
  setPluginEnabled: "setGCodePluginEnabled",
  uninstallPlugin: "uninstallGCodeMarketplacePlugin",
  updateMarketplace: "updateGCodePluginMarketplace",
  updatePlugin: "updateGCodeMarketplacePlugin",
  validatePluginPath: "validateGCodePluginPath",
...
/** `gcode plugins` 子命令专属旗标；由 run.ts 的全局解析器收集后原样透传。 */
...
export async function resolveDep<K extends PluginDepName>(
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@gcode/bootstrap"));
...
export function resolveScope(value: string | undefined): PluginScope | undefined {
  ...
  if (value === "local") {
    throw new PluginsUsageError("Scope 'local' is not supported by gcode. Use: user, project");
  }
