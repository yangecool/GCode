...
import type { SessionId } from "@gcode/contracts";
...
/**
 * 每个 gcode session 一个持久 NodeReplSession，跨多次 js 调用保持内存状态（globalThis）。
...
function isBrowserSurfaceSideEffect(command: import("@gcode/contracts").BrowserCommand): boolean {
...
function isAutoScreenshotTriggerCommand(
  command: import("@gcode/contracts").BrowserCommand,
): boolean {
  if (
