...
import type { ModelCatalogEntry, ModelSelection } from "@gcode/contracts";
import {
  GCODE_MODEL_REASONING_SEPARATOR,
  formatModelPickerValue,
  parseModelPickerValue,
} from "@gcode/shared/model-selection";
...
function splitReasoningLevel(text: string): { reference: string; level?: string } {
  ...
  const index = text.indexOf(GCODE_MODEL_REASONING_SEPARATOR, searchFrom);
