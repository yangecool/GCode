...
} from "@gcode/gcode-cua/frame-contract";
...
import { preserveOfficialCuaFrameResult } from "@gcode/gcode-cua/frame-contract";
...
function readBrowserScreenshotContentIndices(result: McpToolCallResult): Set<number> {
  const value = result._meta?.[GCODE_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY];
