...
import {
  GCODE_CUA_OFFICIAL_MCP_NAMESPACE_NAME as GCODE_CUA_OFFICIAL_MCP_SERVER_NAME,
  GCODE_CUA_OFFICIAL_PLUGIN_ID,
} from "@gcode/shared";
import type { McpToolDescriptor, PluginReferenceCatalog, SkillMetadata } from "@gcode/contracts";
...
const OFFICIAL_CUA_SERVER_PARTS = new Set([
  "computer-use",
  "computer_use",
  toModelVisibleMcpNamePart(GCODE_CUA_OFFICIAL_MCP_SERVER_NAME),
]);
...
export function createOfficialCuaPolicy(
  ...
  for (const descriptor of descriptors) {
    ...
    if (descriptor.serverName === GCODE_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  if (officialServerNames.has(GCODE_CUA_OFFICIAL_MCP_SERVER_NAME)) {
  ...
  for (const serverName of officialServerNames) {
    if (serverName === GCODE_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  const officialPlugin = catalog?.plugins.find(
    (plugin) => plugin.pluginId === GCODE_CUA_OFFICIAL_PLUGIN_ID,
  );
