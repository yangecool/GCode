...
import type { FileSystemPort, Logger, TraceContext } from "@gcode/contracts";
...
function resolvePersistentAgentMemoryRoot(input: {
  ...
  return input.scope === "project"
    ? join(workspace, ".gcode", "agent-memory", agentKey)
    : join(workspace, ".gcode", "agent-memory-local", agentKey);
}
