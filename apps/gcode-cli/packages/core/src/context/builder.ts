...
import type { ModelInputMessage } from "@gcode/contracts";
import type {
...
export class ContextBuilder {
  ...
  build(): ContextBuildResult {
    ...
    // 「You are GCode, an interactive coding agent」对一个
    ...
    if (hasCustomSystemPrompt) {
      ...
      // 引擎主体替换 identity 文案；安全行与 harness 块是 GCode 运行语义
    ...
    if (!hasCustomSystemPrompt) {
      if (!isWorkflowActor && this.config.presentationSurface === "gcode_desktop") {
  ...
  private assembleSystemMessages(sections: ContextSection[]): ModelInputMessage[] {
    ...
    if (dynamicSystemContent) {
      messages.push({
        role: "system",
        // GCode by design：Main Agent 的 dynamic system block 自带左边界，所有 provider 保持一致。
