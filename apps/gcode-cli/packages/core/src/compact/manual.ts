import { CompactTrigger } from "@gcode/contracts";
...
import { modelMessageContentBlockToText, modelMessageContentToText } from "@gcode/contracts";
...
function stringifyToolCallInputForTokenEstimate(input: unknown): string {
  try {
    ...
    // GCode 的模型输入仍可能包含未知内容；异常输入不能让本地预算估算中断 compact。
