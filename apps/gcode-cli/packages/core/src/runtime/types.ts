...
import type { GCodeProviderAccountAccess } from "@gcode/shared";
...
export interface AgentRuntimeConfig {
  ...
  runtimeFeatures?: {
     ...
     * 由 bootstrap 根据 GCode 官方插件启停推导，不由普通插件 manifest 自声明。
     */
    nodeRepl?: boolean;
    /**
  ...
  /**
   * 动态工作流灰度门：Host 判定后经
   * GCode Protocol 下发，runtime 只消费。**缺席即开启**——TUI、headless `-p` 与
  ...
  agentName?: string; // Default: "gcode-agent"
...
export interface ProviderRuntimeHeadersPort {
  ...
  refreshBeforeModelRequest(input: {
    accountAccess?: GCodeProviderAccountAccess;
    abortSignal?: AbortSignal;
    modelId: string;
    providerId: string;
...
export interface ExecuteToolsOptions {
  ...
  subagentModelOverride?: import("@gcode/contracts").SubagentRunOptions["modelOverride"];
  model?: Model;
