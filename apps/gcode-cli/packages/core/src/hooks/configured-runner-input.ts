...
import { resolveWorkspaceHookTimeoutMs } from "@gcode/shared/workspace-hook-discovery";
import {
  CoreErrorType,
  HookEventName,
  createCoreError,
...
export async function createCompatibleHookStdin(input: HookInput): Promise<{
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "gcode-hook-"));
  ...
  if ("toolName" in input) {
    ...
    // 这里只补无损 alias，继续保留 GCode camelCase 字段作为内部主契约。
...
export function createPluginEnvOverlay(
  ...
  const set: Record<string, string> = {
    ...
    GCODE_PROJECT_DIR: input.cwd || workingDirectory,
    GCODE_SESSION_ID: input.sessionId,
  };
  ...
  return {
    set: {
      ...
      GCODE_PLUGIN_DATA: plugin.dataPath,
      GCODE_PLUGIN_ID: plugin.id,
      GCODE_PLUGIN_NAME: plugin.name,
      GCODE_PLUGIN_ROOT: plugin.rootPath,
    },
  };
}
...
export function expandPluginVariables(
  ...
  if (plugin) {
    ...
    replacements.GCODE_PLUGIN_DATA = plugin.dataPath;
    replacements.GCODE_PLUGIN_ROOT = plugin.rootPath;
  }
  return value.replace(
    /\$\{(CLAUDE_CODE_SESSION_ID|CLAUDE_PLUGIN_DATA|CLAUDE_PLUGIN_ROOT|CLAUDE_PROJECT_DIR|CLAUDE_SESSION_ID|CLAUDE_SKILL_DIR|GCODE_PLUGIN_DATA|GCODE_PLUGIN_ROOT|GCODE_PROJECT_DIR|GCODE_SESSION_ID|GCODE_SKILL_DIR)\}/gu,
    (_match, key: string) => {
      if (key === "CLAUDE_SKILL_DIR" || key === "GCODE_SKILL_DIR") {
