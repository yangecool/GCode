...
const traceHeaderNames = ["x-gcode-trace-id", "x-trace-id", "traceparent"];
...
export class NetworkCaptureService {
  ...
  getStatus(): NetworkCaptureStatus {
    ...
    const env: Record<string, string> = url
      ? {
          GCODE_HTTP_PROXY: url,
          GCODE_AGENT_CA_CERT: caCertPath,
        }
      : {};
  ...
  private recordForContext(ctx: IContext): NetworkRequestRecord | undefined {
    const id = stringValue(ctx.tags?.gcodeCaptureId) ?? ctx.uuid;
...
export function createNetworkCaptureServiceFromEnv(
  ...
  if (isDisabled(env.GCODE_DEBUG_NETWORK_CAPTURE)) return undefined;
  return new NetworkCaptureService({
    host: env.GCODE_DEBUG_NETWORK_HOST || undefined,
    port: parsePositiveInteger(env.GCODE_DEBUG_NETWORK_PORT) ?? DEFAULT_PORT,
    caDir: env.GCODE_DEBUG_NETWORK_CA_DIR || undefined,
    maxEntries: parsePositiveInteger(env.GCODE_DEBUG_NETWORK_MAX_ENTRIES) ?? DEFAULT_MAX_ENTRIES,
  });
}
