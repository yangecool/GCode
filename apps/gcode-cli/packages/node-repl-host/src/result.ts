import {
  GCODE_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY,
  GCODE_MCP_NODE_REPL_CUA_APP_META_KEY,
} from "@gcode/contracts/mcp";
import { isOfficialCuaImageRefText } from "@gcode/gcode-cua/frame-contract";
import { CUA_APP_ASSOCIATIONS_META_KEY } from "@gcode/gcode-cua/host-display-contract";
import type { NodeReplRunResult } from "@gcode/core/repl";
 ...
 * （gcode-cua 的 frame-pixel-latest-only 测试即此契约），旧帧对模型没有使用价值。
...
export function toMcpRunResult(run: NodeReplRunResult): CallToolResult {
  ...
  delete responseMeta[GCODE_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY];
  ...
  delete responseMeta[GCODE_MCP_NODE_REPL_CUA_APP_META_KEY];
  if (run.cuaApp) responseMeta[GCODE_MCP_NODE_REPL_CUA_APP_META_KEY] = run.cuaApp;
      ...
      ? {
          _meta: {
            ...
            ...((run.images?.length ?? 0) > 0 || structuredContent.some((block) => block.type === "image")
              ? { "gcode/nodeReplEmittedImage": true }
              : {}),
