...
import type { CustomCommandRoot, CustomCommandSource } from "@gcode/contracts";
...
export async function resolveDefaultCustomCommandRoots(
  ...
  const includeGcode = options.includeGcodeCommands ?? true;
...
function commandRootsForBase(
  ...
  // 合并而不是 fallback：兼容 `.agents` 命令和原生 `.gcode` 命令需要同时可见。
  // 同一级别 `.gcode` 先扫描，命令同名时仍按“先到先赢”处理。
  return [
    root(join(baseDirectory, GCODE_DIR, COMMANDS_DIR), scope, "gcode", nextPriority()),
