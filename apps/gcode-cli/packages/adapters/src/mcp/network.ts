...
import { sanitizeGCodeRuntimeEnv } from "@gcode/shared";
...
export function buildMcpStdioEnv(options: {
  ...
  return prependRunningNodeDirectory(
    applyNetworkEgressEnv(sanitizeGCodeRuntimeEnv(filterStringEnv(sourceEnv)), {
...
function prependRunningNodeDirectory(
  ...
  // remote Agent 由 ~/.gcode/server/node 启动，但登录环境 PATH 不含该目录，
