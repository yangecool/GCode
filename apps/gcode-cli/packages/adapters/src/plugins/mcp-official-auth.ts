/*
 * Plugin `.mcp.json` 里 `auth: { type: "gcode_official" }` 的严格解析与 provenance 生成。
 *
 * 单独成文件而不是留在 mcp.ts：官方鉴权的解析规则覆盖 http 与 stdio 两种传输方式，
 * 且与模板变量解析、传输层字段解析没有耦合——放一起只会让 mcp.ts 继续膨胀（它已到 max-lines 上限）。
 */
import type { McpOfficialProvenance, GCodeOfficialMcpAuthConfig } from "@gcode/contracts";
 ...
 */
export function parseGCodeOfficialAuth(
  value: unknown,
  mcpKey: string,
): GCodeOfficialMcpAuthConfig | undefined {
  if (value === undefined) return undefined;
  if (!isRecord(value)) {
    throw new Error(`MCP server ${mcpKey}: auth must be an object`);
  }
  // 精确值匹配，区分大小写；gcode-official、gcode_official_auth 等别名一律拒绝。
  if (value.type !== "gcode_official") {
  ...
  return { type: "gcode_official", provider: "jwt_token" };
}
