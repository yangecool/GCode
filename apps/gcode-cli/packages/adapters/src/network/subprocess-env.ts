import {
  ...
  GCODE_TOOL_ENV_PASSTHROUGH_ENV_KEY,
  readGCodeToolEnvPassthroughEnv,
} from "@gcode/shared";
...
const EXPLICIT_CA_SOURCE_KEYS = [GCODE_AGENT_CA_CERT_ENV_KEY] as const;
const EXPLICIT_NO_PROXY_SOURCE_KEYS = [GCODE_NO_PROXY_ENV_KEY] as const;
...
export function applyNetworkEgressEnv(
  ...
  deleteEnvKey(env, GCODE_TOOL_ENV_PASSTHROUGH_ENV_KEY, platform);
...
function applyToolEnvPassthroughEnv(
  ...
  const passthroughEnv = readGCodeToolEnvPassthroughEnv(sourceEnv);
...
function applyProxyEnv(
  ...
  const gcodeProxy = normalizeProxyValue(
    getFirstEnvValue(sourceEnv, [GCODE_HTTP_PROXY_ENV_KEY], platform),
  );
  if (gcodeProxy) {
    for (const key of ALL_PROXY_KEYS) {
      setEnvKey(env, key, gcodeProxy, platform);
    }
    return;
  }
}
