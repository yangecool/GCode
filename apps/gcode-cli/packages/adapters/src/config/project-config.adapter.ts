...
import type { McpServerConfig, RuntimeConfigPatch } from "@gcode/contracts";
import {
    ...
    ...(hooks
      ? {
          hookCandidate: createWorkspaceHookSourceInput({
            ...
            // hooks 字段已由 loadFileConfig 经 GCodeConfigFileSchema（shared 单源
...
function getProjectConfigBaseDir(path: string): string {
  ...
  return basename(configDirectory) === ".gcode" ? dirname(configDirectory) : configDirectory;
}
