/* eslint-disable max-lines -- gcode-cli 配置 schema 需要集中维护文件解析和 provider 继承，拆散会让配置语义更难对齐。 */
import { z } from "zod";
...
export const LEGACY_CUA_PLUGIN_ID = "gcode-cua@zcode-plugins-official";
...
export const GCodeConfigFileSchema = z
  .object({
...
export type GCodeConfigFile = z.infer<typeof GCodeConfigFileSchema>;
...
function normalizeMcpServerConfigInput(value: unknown): unknown {
  ...
  // External MCP configs may carry provider-specific timeout fields. GCode does not migrate
  ...
  if (server.type === "remote") {
    // 外部 Agent 配置常把 HTTP MCP 标记为 remote；GCode 运行态协议类型是 http。
    server.type = "http";
...
export function parseConfigFileToRuntimePatchWithDiagnostics(
  ...
  const parsed = GCodeConfigFileSchema.parse(normalized);
  return {
...
function parsedConfigFileToRuntimePatch(parsed: GCodeConfigFile): RuntimeConfigPatch {
...
function normalizePluginConfig(
  plugins: GCodeConfigFile["plugins"],
