...
export interface GCodeCredentialCipher {
  decrypt(value: string): string;
  encrypt(value: string): string;
}

export interface GCodeCredentialCipherOptions {
  env?: Record<string, string | undefined>;
}

export function createGCodeCredentialCipher(
  options: GCodeCredentialCipherOptions = {},
): GCodeCredentialCipher {
  const key = deriveCipherKey(resolveCredentialSecret(options.env ?? process.env));

  return {
    decrypt(value: string): string {
      if (!isEncryptedGCodeCredentialValue(value)) {
        return value;
      }
...
export function isEncryptedGCodeCredentialValue(value: string): boolean {
