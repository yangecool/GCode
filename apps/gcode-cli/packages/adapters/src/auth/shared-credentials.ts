...
import { createGCodeCredentialCipher, type GCodeCredentialCipher } from "./credential-cipher.js";

const GCODE_DATA_BASE_DIR_ENV_KEY = "GCODE_DATA_BASE_DIR";
...
export const SHARED_GCODE_CREDENTIAL_KEYS = {
...
export interface SharedGCodeCredentialStoreOptions {
  baseDir?: string;
  cipher?: GCodeCredentialCipher;
...
export interface SharedGCodeCredentialStore {
...
export function createSharedGCodeCredentialStore(
  options: SharedGCodeCredentialStoreOptions = {},
): SharedGCodeCredentialStore {
  const env = options.env ?? process.env;
  const filePath = resolveSharedGCodeCredentialsPath(options);
  const cipher = options.cipher ?? createGCodeCredentialCipher({ env });
  ...
  return {
    ...
    async clearZaiLoginCredentials(): Promise<void> {
      await mutateRawCredentialRecord(filePath, async (rawCredentials) => {
        const activeProviderRaw = rawCredentials[SHARED_GCODE_CREDENTIAL_KEYS.activeProvider];
        const activeProvider = activeProviderRaw ? cipher.decrypt(activeProviderRaw) : null;
        delete rawCredentials[SHARED_GCODE_CREDENTIAL_KEYS.zaiAccessToken];
        delete rawCredentials[SHARED_GCODE_CREDENTIAL_KEYS.zaiRefreshToken];
        delete rawCredentials[SHARED_GCODE_CREDENTIAL_KEYS.zaiUserInfo];
        delete rawCredentials[SHARED_GCODE_CREDENTIAL_KEYS.gcodeJwtToken];
        if (activeProvider === ZAI_PROVIDER_ID) {
          delete rawCredentials[SHARED_GCODE_CREDENTIAL_KEYS.activeProvider];
        }
      });
    },
    ...
    async saveZaiLoginCredentials(payload: ZaiLoginCredentialPayload): Promise<void> {
      ...
      await mutateRawCredentialRecord(filePath, (rawCredentials) => {
        rawCredentials[SHARED_GCODE_CREDENTIAL_KEYS.activeProvider] =
          encryptedCredentials.activeProvider;
        rawCredentials[SHARED_GCODE_CREDENTIAL_KEYS.zaiAccessToken] =
          encryptedCredentials.accessToken;
        rawCredentials[SHARED_GCODE_CREDENTIAL_KEYS.gcodeJwtToken] = encryptedCredentials.jwtToken;
        rawCredentials[SHARED_GCODE_CREDENTIAL_KEYS.zaiUserInfo] = encryptedCredentials.userInfo;
      });
    },
  };
}

export function loadSharedGCodeCredentialSync(
  key: string,
  options: SharedGCodeCredentialStoreOptions = {},
): string | undefined {
  const filePath = resolveSharedGCodeCredentialsPath(options);
  ...
  try {
    ...
    const cipher = options.cipher ?? createGCodeCredentialCipher({ env: options.env });
...
export function resolveSharedGCodeCredentialsPath(
  options: SharedGCodeCredentialStoreOptions = {},
): string {
  if (options.filePath) {
  ...
  const env = options.env ?? process.env;
  const baseDir = options.baseDir ?? env[GCODE_DATA_BASE_DIR_ENV_KEY] ?? homedir();
  return join(resolveUserPath(baseDir), ".gcode", "v2", "credentials.json");
}
...
async function readRawCredentialRecord(filePath: string): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`Unable to read shared GCode credentials: ${filePath}`, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(`Shared GCode credentials are corrupt: ${filePath}.${evidence}`, {
      cause: error,
    });
  }
}
