...
export function createCwdCapturePlan(
  ...
  const cwdFilePath = join(cwdCaptureDir, `gcode-${crypto.randomUUID()}-cwd`);
...
function createPosixCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    "__gcode_status=$?",
    `if [ "$__gcode_status" -eq 0 ]; then pwd -P > ${shellQuote(cwdFilePath)}; fi`,
    'exit "$__gcode_status"',
  ].join("\n");
}
...
function createWindowsCmdCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    'set "__gcode_status=%ERRORLEVEL%"',
    `if "%__gcode_status%"=="0" cd > ${cmdQuote(cwdFilePath)}`,
    "exit /b %__gcode_status%",
  ].join("\r\n");
}
