...
import { windowsPathToGitBashPath } from "@gcode/contracts";
import type { ExecutionShellDialect } from "@gcode/contracts";
...
function pathExportLines(pathValue: string): string[] {
  return [
    ...
    "cat >> \"$SNAPSHOT_FILE\" << 'PATH_END_GCODE_SHELL_INIT_SNAPSHOT'",
    `export PATH=${shellQuoteAlways(pathValue)}`,
    "PATH_END_GCODE_SHELL_INIT_SNAPSHOT",
  ];
}
