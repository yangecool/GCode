 ...
 * 输入改为 GCode `ModelInputMessage` 的结构子集（工具结果是 role:'tool' 消息；
 * 图片块自带 dataUrl，无需附件服务）。
 *
 * replay 载体（H5 双表示的 GCode 侧约定）：assistant 块的 `providerOptions`
...
/** 结构子集对齐 GCode `ModelInputMessage`（contracts/src/model/index.ts:402）。 */
...
/** 结构子集对齐 GCode 内容块（contracts/src/model/index.ts:336-384）。 */
...
/** 结构子集对齐 GCode `ModelInputMessage`。 */
...
/**
 * Serialize GCode-shaped history into native Responses input items.
...
/** 模型可见函数工具的最小结构（GCode `ModelToolContract` 子集）。 */
