...
import type { LogContext, LogEntry, Logger, LoggerFactory, LogRedactor } from "@gcode/contracts";
import { LogLevel, LogLevelName } from "@gcode/contracts";
...
export function createNodeLoggerFactory(options: NodeLoggerFactoryOptions = {}): NodeLoggerFactory {
  ...
  const logDir = options.logDir ?? options.env?.GCODE_LOG_DIR ?? getDefaultLogDir();
  const consoleStream =
    typeof options.console === "object"
      ? options.console.stream
      : options.console === true || options.env?.GCODE_LOG_CONSOLE === "1"
        ? process.stderr
        : undefined;
...
export function getDefaultLogDir(): string {
  return join(homedir(), ".gcode", "cli", "log");
}
...
function isDevelopmentMode(env: NodeJS.ProcessEnv): boolean {
  const runtimeEnv = normalizeGCodeRuntimeEnv(env[GCODE_RUNTIME_ENV_KEY]);
...
function getLogFileName(): string {
  return `gcode-${formatLocalLogDate(new Date())}.jsonl`;
}
