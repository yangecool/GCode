...
export class OtelAgentTelemetryMetrics implements AgentTelemetryMetricRecorder {
  ...
  constructor(meter: Meter) {
    this.durations = {
      agent_turn: duration(meter, "gcode.agent.turn.duration"),
      agent_step: duration(meter, "gcode.agent.step.duration"),
      tool_execution: duration(meter, "gcode.tool.execution.duration"),
      command_execution: duration(meter, "gcode.command.execution.duration"),
      context_compaction: duration(meter, "gcode.context.compaction.duration"),
      detached_operation: duration(meter, "gcode.detached.operation.duration"),
      model_call: duration(meter, "gcode.model.call.duration"),
      model_attempt: duration(meter, "gcode.model.attempt.duration"),
    };
    this.modelCallAttempts = meter.createHistogram("gcode.model.call.attempts", {
      description: "Physical provider attempts per logical model call",
      unit: "{attempt}",
    });
    this.modelTokens = meter.createCounter("gcode.model.attempt.tokens", {
      description: "Observed provider token delta by token type",
      unit: "{token}",
    });
    this.modelAttemptFirstProviderEvent = duration(
      meter,
      "gcode.model.attempt.time_to_first_provider_event",
    );
    this.modelAttemptFirstContent = duration(meter, "gcode.model.attempt.time_to_first_content");
    this.modelAttemptFirstText = duration(meter, "gcode.model.attempt.time_to_first_text");
    this.modelAttemptStreamStalls = meter.createCounter("gcode.model.attempt.stream_stall.count", {
      description: "Observed stream stalls",
      unit: "{stall}",
    });
    this.modelAttemptStreamMaxIdle = duration(meter, "gcode.model.attempt.stream_max_idle");
    this.commandFirstOutput = duration(meter, "gcode.command.execution.time_to_first_output");
    this.creationDrop = meter.createCounter("gcode.telemetry.creation_drop.count", {
      description: "Spans rejected before creation by runtime capacity protection",
      unit: "{span}",
    });
    this.abandoned = meter.createCounter("gcode.telemetry.abandoned.count", {
