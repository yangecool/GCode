...
export function createOwnedAgentTelemetryRuntime(
  ...
  const tracer = provider.getTracer("@gcode/cli-agent-telemetry", String(TELEMETRY_SCHEMA_VERSION));
  const meter = meterProvider.getMeter(
    "@gcode/cli-agent-telemetry",
...
function metricViews(): ViewOptions[] {
  return [
    histogramView("gcode.agent.turn.duration", [1, 5, 10, 30, 60, 300, 600]),
    histogramView("gcode.model.*.duration", [0.25, 1, 2, 5, 10, 30, 60, 120]),
    histogramView(
      "gcode.context.compaction.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView(
      "gcode.detached.operation.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("gcode.agent.step.duration", [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600]),
    histogramView("gcode.tool.execution.duration", [0.01, 0.1, 0.5, 1, 5, 30, 120, 300]),
    histogramView(
      "gcode.command.execution.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("gcode.*.time_to_first_*", [0.25, 0.5, 1, 2, 5, 10, 30, 60]),
    histogramView("gcode.model.call.attempts", [1, 2, 3, 5, 8]),
    cardinalityView("gcode.model.attempt.tokens"),
    cardinalityView("gcode.model.attempt.stream_stall.count"),
    cardinalityView("gcode.telemetry.creation_drop.count"),
    cardinalityView("gcode.telemetry.abandoned.count"),
  ];
}
...
function telemetryResourceAttributes(
  ...
  return compactResourceAttributes({
    ...
    "gcode.build.commit_id": resource.buildCommitId,
    "gcode.device.installation_id": options.includeInstallationId
      ? resource.installationId
      : undefined,
    "gcode.product.version": resource.productVersion,
    "gcode.runtime.distribution": resource.runtimeDistribution,
    "gcode.runtime.surface": resource.runtimeSurface,
    "gcode.telemetry.schema_owner": "cli",
    "gcode.telemetry.schema_version": TELEMETRY_SCHEMA_VERSION,
  });
}
