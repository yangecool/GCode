...
import type {
  ...
  ToolTraceStart,
} from "@gcode/contracts/telemetry";
import {
  activeWriterContext,
  BaseSpanWriter,
  compactAttributes,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "gcode.agent_turn.turn_number": integer(input.turnNumber),
            "gcode.agent_turn.input_source": safeEnum(input.inputSource),
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "gcode.agent_step.step_id": safeId(input.stepId),
            "gcode.agent_step.step_index": integer(input.stepIndex),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "gcode.execution.tool_call_id": safeId(input.toolCallId),
            "gcode.tool_execution.tool_name": toolName,
            ...toolCompatibilityAttributes({
    ...
    return this.safeCreate(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent?.correlation),
            "gcode.context_compaction.trigger": safeEnum(input.trigger),
            "gcode.context_compaction.phase": safeEnum(input.phase),
            "gcode.context_compaction.model_mode": safeEnum(input.modelMode),
            "gcode.context_compaction.outer_attempt": integer(input.outerAttempt),
            "gcode.context_compaction.max_attempts": integer(input.maxAttempts),
            "gcode.context_compaction.triggering_step_index": integer(input.triggeringStepIndex),
            "gcode.context_compaction.policy_context_window_tokens": finiteNonNegative(
              input.policyContextWindowTokens,
            ),
            "gcode.context_compaction.threshold_tokens": finiteNonNegative(input.thresholdTokens),
            "gcode.context_compaction.token_source": safeEnum(input.tokenSource),
            "gcode.context_compaction.recovered_from_logical_call_id": safeId(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "gcode.detached_operation.operation": safeEnum(input.operation),
            "gcode.detached_operation.execution_kind": safeEnum(input.executionKind),
            "gcode.detached_operation.trigger": safeEnum(input.trigger),
            "gcode.detached_operation.target_kind": safeEnum(input.targetKind),
            "gcode.detached_operation.goal_iteration": integer(input.goalIteration),
            "gcode.detached_operation.chunk_index": integer(input.chunkIndex),
            "gcode.detached_operation.chunk_count": integer(input.chunkCount),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "gcode.execution.logical_call_id": safeId(input.logicalCallId),
            "gcode.model_call.operation": safeEnum(input.operation),
            "gcode.model_call.streaming": input.streaming,
            "gcode.model_call.model_role": safeEnum(input.modelRole),
            "gcode.model_call.requested_provider_id": safeString(input.requested.providerId, 128),
            "gcode.model_call.requested_model": safeString(input.requested.requestedModel, 128),
            "gcode.model_call.reasoning_capability": safeEnum(input.requested.reasoning.capability),
            "gcode.model_call.reasoning_requested_state": safeEnum(
              input.requested.reasoning.requestedState,
            ),
            "gcode.model_call.reasoning_requested_control": safeEnum(
              input.requested.reasoning.requestedControl,
            ),
            "gcode.model_call.reasoning_requested_level": safeString(
              input.requested.reasoning.requestedLevel,
              128,
            ),
            "gcode.model_call.reasoning_requested_budget_tokens": integer(
              input.requested.reasoning.requestedBudgetTokens,
            ),
            "gcode.model_call.call_cause": safeEnum(input.callCause),
            "gcode.model_call.previous_logical_call_id": safeId(input.previousLogicalCallId),
          }),
          context: parentContext,
          correlation: parent?.correlation,
          parent,
    ...
    return this.safeCreate(
      ...
      () => {
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent.correlation),
            "gcode.execution.tool_call_id": safeId(parent.toolCallId),
            "gcode.command_execution.safe_name": safeString(input.safeName, 128),
            "gcode.command_execution.category": safeEnum(input.category),
            "gcode.command_execution.command_count": integer(input.commandCount),
            "gcode.command_execution.shell_kind": safeEnum(input.shellKind),
            "gcode.command_execution.sandboxed": input.sandboxed,
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "gcode.execution.tool_call_id": safeId(parent.toolCallId),
            "gcode.execution.logical_call_id": parentWriter.logicalCallId,
            "gcode.execution.model_operation": parentWriter.operation,
            "gcode.execution.model_role": parentWriter.modelRole,
            "gcode.model_attempt.request_id": safeId(input.requestId),
            "gcode.model_attempt.attempt_number": integer(input.attemptNumber),
            "gcode.model_attempt.max_attempts": integer(input.maxAttempts),
            "gcode.model_attempt.attempt_cause": safeEnum(input.attemptCause),
            "gcode.model_attempt.previous_request_id": safeId(input.previousRequestId),
            "gcode.model_attempt.retry_delay_ms": finiteNonNegative(input.retryDelayMs),
            "gcode.model_attempt.provider_id": safeString(target.providerId, 128),
            "gcode.model_attempt.provider_kind": safeEnum(target.providerKind),
            "gcode.model_attempt.provider_origin": safeString(target.providerOrigin),
            "gcode.model_attempt.provider_route": safeString(target.providerRoute),
            "gcode.model_attempt.requested_model": safeString(target.requestedModel, 128),
            "gcode.model_attempt.transport": safeEnum(input.transport),
            "gcode.model_attempt.api_operation": safeEnum(input.apiOperation),
            "gcode.model_attempt.reasoning_capability": safeEnum(target.reasoning.capability),
            "gcode.model_attempt.reasoning_requested_state": safeEnum(
              target.reasoning.requestedState,
            ),
            "gcode.model_attempt.reasoning_requested_control": safeEnum(
              target.reasoning.requestedControl,
            ),
            "gcode.model_attempt.reasoning_requested_level": safeString(
              target.reasoning.requestedLevel,
              128,
            ),
            "gcode.model_attempt.reasoning_requested_budget_tokens": integer(
...
class TurnWriter extends TrackedBaseWriter implements AgentTurnSpanWriter {
  ...
  finishCompleted(
    ...
    this.finishCompletedIfOpen(() => this.setAttribute("gcode.agent_turn.result_type", resultType));
  }
...
class StepWriter extends TrackedBaseWriter implements AgentStepSpanWriter {
  ...
  finishCompleted(
    ...
    this.finishCompletedIfOpen(() =>
      this.setAttribute("gcode.agent_step.terminal_reason", terminalReason),
    );
  }
...
class ToolWriter extends TrackedBaseWriter implements ToolExecutionSpanWriter {
  ...
  setPermissionDecision(decision: "granted" | "denied" | "not_required"): void {
    this.setAttribute("gcode.tool_execution.permission_decision", decision);
  ...
  setOutputBytes(bytes: number): void {
    this.setAttribute("gcode.tool_execution.output_bytes", finiteNonNegative(bytes));
  }

  setOutputTruncated(truncated: boolean): void {
    this.setAttribute("gcode.tool_execution.output_truncated", truncated);
  }
  ...
  finishDenied(reason: "user_denied" | "policy_denied" | "unavailable" | "unknown"): void {
    this.finishDomainOutcomeIfOpen("denied", () =>
      this.setAttribute("gcode.tool_execution.permission_denial_reason", reason),
    );
  }
...
class CommandWriter extends TrackedBaseWriter implements CommandExecutionSpanWriter {
  ...
  markFirstOutput(): void {
    ...
    this.setAttribute("gcode.command_execution.first_output_ms", elapsed);
  ...
  setExitCode(exitCode: number): void {
    ...
    this.setAttribute("gcode.command_execution.exit_code", normalized);
  ...
  setSignal(signal: string): void {
    ...
    this.setAttribute("gcode.command_execution.signal", normalized);
    this.setAttributes(commandCompatibilityAttributes({ signal: normalized }));
  }

  setOutputBytes(bytes: number): void {
    this.setAttribute("gcode.command_execution.output_bytes", finiteNonNegative(bytes));
  }

  setTimedOut(timedOut: boolean): void {
    this.setAttribute("gcode.command_execution.timed_out", timedOut);
  }
...
class CompactionWriter extends TrackedBaseWriter implements ContextCompactionSpanWriter {
  ...
  setInputTokens(tokens: number): void {
    this.setAttribute("gcode.context_compaction.input_tokens", finiteNonNegative(tokens));
  }

  setOutputTokens(tokens: number): void {
    this.setAttribute("gcode.context_compaction.output_tokens", finiteNonNegative(tokens));
  }
...
class DetachedWriter extends TrackedBaseWriter implements DetachedOperationSpanWriter {
  ...
  setResultType(resultType: "text" | "boolean" | "metadata" | "other"): void {
    this.setAttribute("gcode.detached_operation.result_type", resultType);
  }
...
class ModelAttemptWriter extends TrackedBaseWriter implements ModelAttemptSpanWriter {
  ...
  setProviderRequestId(requestId: string): void {
    this.setAttribute("gcode.model_attempt.provider_request_id", safeString(requestId, 256));
  }

  setResponseModel(model: ResponseModelTelemetryDescriptor): void {
    const value = safeString(model.model, 128);
    this.setAttribute("gcode.model_attempt.response_model", value);
  ...
  setEffectiveReasoningState(state: ModelReasoningState): void {
    this.setAttribute("gcode.model_attempt.reasoning_effective_state", safeEnum(state));
  }

  setEffectiveReasoningControl(control: ModelReasoningControlType): void {
    this.setAttribute("gcode.model_attempt.reasoning_effective_control", safeEnum(control));
  }

  setEffectiveReasoningLevel(level: string): void {
    this.setAttribute("gcode.model_attempt.reasoning_effective_level", safeString(level, 128));
  }

  setEffectiveReasoningBudgetTokens(tokens: number): void {
    this.setAttribute(
      "gcode.model_attempt.reasoning_effective_budget_tokens",
  ...
  setFinishReason(reason: ModelFinishReason): void {
    ...
    this.setAttribute("gcode.model_attempt.finish_reason", value);
    this.setAttributes(modelResponseCompatibilityAttributes({ finishReason: value }));
  }

  setInputTokens(tokens: number): void {
    this.setUsage("inputTokens", "input", tokens, "gcode.model_attempt.input_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ inputTokens: tokens }));
  }

  setOutputTokens(tokens: number): void {
    this.setUsage("outputTokens", "output", tokens, "gcode.model_attempt.output_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ outputTokens: tokens }));
  }

  setReasoningTokens(tokens: number): void {
    this.setUsage("reasoningTokens", "reasoning", tokens, "gcode.model_attempt.reasoning_tokens");
  }

  setCacheReadTokens(tokens: number): void {
    this.setUsage("cacheReadTokens", "cache_read", tokens, "gcode.model_attempt.cache_read_tokens");
  }
  ...
  setCacheWriteTokens(tokens: number): void {
    this.setUsage(
      ...
      "cache_write",
      tokens,
      "gcode.model_attempt.cache_write_tokens",
    );
  }

  setStreamOutputCommitted(committed: boolean): void {
    this.setAttribute("gcode.model_attempt.stream_output_committed", committed);
  }

  setHttpStatusCode(statusCode: number): void {
    const normalized = integer(statusCode);
    this.setAttribute("gcode.model_attempt.http_status_code", normalized);
    this.setAttributes(httpResponseCompatibilityAttributes(statusCode));
  }

  setProviderErrorCode(code: string): void {
    this.setAttribute("gcode.model_attempt.provider_error_code", safeString(code, 128));
  }

  setProviderErrorMessage(message: string): void {
    this.setAttribute("gcode.model_attempt.provider_error_message", sanitizeErrorMessage(message));
  }

  setRetryAfterMs(delayMs: number): void {
    this.setAttribute("gcode.model_attempt.retry_after_ms", finiteNonNegative(delayMs));
  }
  ...
  markFirstProviderEvent(): void {
    ...
    this.setAttribute("gcode.model_attempt.time_to_first_provider_event_ms", elapsed);
  ...
  markFirstContent(): void {
    ...
    this.setAttribute("gcode.model_attempt.time_to_first_content_ms", elapsed);
  ...
  markFirstText(): void {
    ...
    this.setAttribute("gcode.model_attempt.time_to_first_text_ms", elapsed);
