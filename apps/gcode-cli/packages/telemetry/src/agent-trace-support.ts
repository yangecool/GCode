...
const ACTIVE_WRITER_CONTEXT_KEY = createContextKey("@gcode/telemetry/active-writer-v4");
...
export function contextFromCausation(causation: AgentTelemetryCausation): Context {
  ...
  // 为底，否则会把另一个异步任务的 Baggage 或 GCode Writer 私有状态夹带进来。
...
export function executionProjection(
  ...
  return {
    "gcode.execution.session_id": options.includeSession
      ? safeId(correlation.sessionId)
      : undefined,
    "gcode.execution.parent_session_id": options.includeSession
      ? safeId(correlation.parentSessionId)
      : undefined,
    "gcode.execution.turn_id": safeId(correlation.turnId),
    "gcode.execution.parent_turn_id": options.includeSession
      ? safeId(correlation.parentTurnId)
      : undefined,
    "gcode.execution.query_id": options.includeQuery ? safeId(correlation.queryId) : undefined,
    "gcode.execution.actor_kind": options.includeActor
      ? safeEnum(correlation.actorKind)
      : undefined,
    "gcode.execution.agent_name": options.includeAgent
      ? safeIdentifier(correlation.agentName)
      : undefined,
    "gcode.execution.identity_state": options.includeIdentity
      ? safeEnum(correlation.identityState)
      : undefined,
    "gcode.execution.user_subject_id": options.includeIdentity
      ? safeId(correlation.userSubjectId)
      : undefined,
    "gcode.execution.launch_surface": safeEnum(correlation.launchSurface),
  };
}
