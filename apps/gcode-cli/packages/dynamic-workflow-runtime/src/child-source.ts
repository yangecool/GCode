 ...
 *     自包含 ESM，harness 写到 `<cwd>/.gcode/workflow-runs/<runId>.mjs`（child-entry-file.ts）；
 *   - 普通 Node：`node --max-old-space-size=N <entry>`，入口文件发现自己就是进程入口时自启；
 *   - SEA 单文件二进制：CLI 的隐藏子命令 `__gcode-dwf-child <entry>` `import()` 这份文件并调
...
export function renderChildEntry(payload: ChildPayload, meta: { runId: string }): string {
  ...
  return `// gcode dynamic workflow run ${runId}
// Generated by @gcode/dynamic-workflow-runtime before every launch; safe to delete once the run has settled.
