import type { CollaborationMode, ModelUsageSummary, TodoItem, TurnId } from "@gcode/contracts";
...
export function useTuiApplyResult(input: TuiApplyResultInput) {
  return React.useCallback(
    (result: TuiSubmitPromptResult, preserveTurnState = false) => {
      const resultCopy = getGCodeCopy(result.locale ?? input.fallback.locale).tui;
