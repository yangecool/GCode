import type { Logger, McpPort } from "@gcode/contracts";
import type { McpConnectionPool, McpTelemetryTracker } from "@gcode/adapters/mcp";
import type { SqliteSessionStore } from "@gcode/adapters/storage";
import { shutdownPreparedModelTelemetry } from "@gcode/telemetry";
...
import type { GCodeProtocolAgentServer } from "./server.js";
...
export async function cleanupProtocolRuntime(options: {
  logger: Logger;
  deadlineAt?: number;
  server?: Pick<GCodeProtocolAgentServer, "shutdown" | "disposeProjections">;
  processResourceSampler?: Pick<GCodeProcessResourceSampler, "stop">;
  ...
  const step = async (resource: string, cleanup: () => unknown | Promise<unknown>) => {
    ...
    try {
      ...
      options.logger.warn(`GCode Protocol ${resource} shutdown failed`, {
        errorType: error instanceof Error ? error.name : typeof error,
        event: `gcode_protocol.${resource}.shutdown.failed`,
