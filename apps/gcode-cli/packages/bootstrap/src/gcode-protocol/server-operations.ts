/* eslint-disable max-lines -- GCode Protocol 的 session/workspace 方法共享同一个 server context 与 snapshot helpers，迁移期先集中维护。 */
import { observeSessionDebug } from "./session-debug.js";
import {
  TASK_LIST_SESSION_TYPES,
  isTaskListSessionType,
} from "../gcode-protocol-v4/task-list-session-membership.js";
import { resolveEffectiveBashShellSelection } from "@gcode/adapters/exec";
import { inputIntentMetadata } from "../gcode-protocol-v4/commands/input-intent.js";
import { createModelExecutionContext } from "./model-execution.js";
import type { SendInputOptions } from "../app/types.js";
import { repairPersistedRemoteSessionPaths, type TurnAttachment } from "@gcode/core";
import {
  CoreErrorType,
...
import {
  ...
  gcodeProtocolErrorCodes,
  gcodeProtocolMethods,
  gcodeSessionCancelBackgroundTaskParamsSchema,
  gcodeSessionCompactParamsSchema,
  gcodeSessionCloseParamsSchema,
  gcodeSessionCreateParamsSchema,
  gcodeSessionEventsParamsSchema,
  gcodeSessionForkParamsSchema,
  gcodeSessionGoalParamsSchema,
  gcodeSessionListParamsSchema,
  gcodeSessionMessagesParamsSchema,
  gcodeSessionReadParamsSchema,
  gcodeSessionRuntimePreferencesResultSchema,
  gcodeSessionResumeParamsSchema,
  gcodeSessionSendParamsSchema,
  gcodeSessionSetModeParamsSchema,
  gcodeSessionSetModelParamsSchema,
  gcodeSessionSetThoughtLevelParamsSchema,
  gcodeSessionStopParamsSchema,
  gcodeSessionSubscribeParamsSchema,
  gcodeSessionSubagentsParamsSchema,
  gcodeTaskTokenUsageParamsSchema,
  gcodeUsageStatsParamsSchema,
  gcodeWorkspaceGenerateTextParamsSchema,
  getConversationMessageProjectionPolicy,
  parseRemoteWorkspaceIdentity,
  type GCodeSessionCreateParams,
  type GCodeDeliveryKind,
  type IntegratedTerminalShellSelection,
  type GCodeSessionRuntimePreferencesScope,
  type GCodeSessionRuntimePreferencesResult,
  type GCodeModelContextBudgetStrategy,
  type GCodeProtocolTrace,
  type GCodeSessionEvent,
  type GCodeSessionHistoryTarget,
  type GCodeSessionResumeParams,
  type GCodeSessionPersistence,
...
import {
  ...
  type GCodeProtocolSessionRecord,
  type GCodeProtocolToolInputTransmissionState,
} from "./server-types.js";
import { createWorkspaceGCodeApp, ensureSessionModelAvailable } from "./workspace-model-runtime.js";
...
import { runSessionModelConfigMutation } from "../gcode-protocol-v4/model-config-mutation.js";
...
type ProtocolGoalTarget = NonNullable<
  Awaited<ReturnType<NonNullable<GCodeProtocolSessionRecord["app"]["readTarget"]>>>
>;

type GCodeSessionRecordParams = (
  | GCodeSessionCreateParams
  | (GCodeSessionResumeParams & {
      mode?: GCodeSessionCreateParams["mode"];
      model?: GCodeSessionCreateParams["model"];
      parentSessionId?: GCodeSessionCreateParams["parentSessionId"];
      thoughtLevel?: GCodeSessionCreateParams["thoughtLevel"];
      // 自动化会话会在创建期关闭标题二次生成，而 create/resume
      // 共用 record 初始化函数；resume 兼容分支也必须声明该策略字段。
      titleGenerationEnabled?: GCodeSessionCreateParams["titleGenerationEnabled"];
    })
) & { taskType?: SessionTaskType };

interface SessionStartupPreferences {
  memoryEnabled: boolean;
  modelContextBudgetStrategy: GCodeModelContextBudgetStrategy;
...
type SessionStartupPreferencesSource =
  | { kind: "host" }
  | { kind: "inherit"; parent: GCodeProtocolSessionRecord };

function resolveSupportedAppThoughtLevel(
  app: Pick<GCodeProtocolSessionRecord["app"], "listThoughtLevels">,
...
function getProtocolToolInputTransmissionState(
  record: GCodeProtocolSessionRecord,
  deliveryKind?: GCodeDeliveryKind,
): GCodeProtocolToolInputTransmissionState {
  const key = protocolEventSequenceKey(deliveryKind);
  const existing = record.protocolToolInputTransmissions.get(key);
  if (existing) {
    return existing;
  }
  const created: GCodeProtocolToolInputTransmissionState = {
...
function readProtocolStreamingParentToolUseId(
  ...
  const gcode = asRecord(meta.gcode);
  return (
    stringValue(meta.parentToolUseId) ??
    stringValue(meta.parentToolCallId) ??
    stringValue(gcode.parentToolUseId) ??
    stringValue(gcode.parentToolCallId)
  );
}
...
function markStreamedToolInputIfPresent(
  mappedEvent: GCodeSessionEvent,
  toolInputTransmissions: GCodeProtocolToolInputTransmissionState,
): void {
...
function mapProtocolSessionEvent(
...
): GCodeSessionEvent | null {
  if (shouldHideProtocolSessionEvent(record, event)) {
    return null;
  }
  if (!shouldExposeSessionEventToProtocol(event)) {
    const payload = asRecord(event.payload);
    context.logger?.debug("GCode Protocol session event filtered", {
      event: "gcode_protocol.session_event.filtered",
      eventId: String(event.id),
      module: "bootstrap.gcode_protocol",
  ...
  // tool_input_delta 等高频模型中间态。GCode Protocol 的 seq 是 UI/replay 的恢复键，
...
): Promise<GCodeSessionEvent[]> {
  const events = await record.eventStore.getEvents(record.app.sessionId as SessionId);
  const visibleEvents: GCodeSessionEvent[] = [];
  const toolInputTransmissions: GCodeProtocolToolInputTransmissionState = {
    streamedToolCallIdsWithInput: new Set(),
  };
...
function logProtocolSessionEventSent(
  ...
  context.logger?.debug("GCode Protocol session event sent", {
    deliveryKind: mappedEvent.deliveryKind,
    event: "gcode_protocol.session_event.sent",
...
function isImportedHistoryMessage(
  ...
  source: NonNullable<GCodeSessionCreateParams["importedHistory"]>["source"],
): boolean {
...
async function removePreviousImportedSessionHistory(params: {
  sessionStore: NonNullable<GCodeProtocolAgentServerContext["deps"]["sessionStore"]>;
  sessionId: SessionId;
  source: NonNullable<GCodeSessionCreateParams["importedHistory"]>["source"];
}): Promise<number> {
...
async function persistImportedSessionHistory(params: {
  ...
  record: GCodeProtocolSessionRecord;
  sessionId: SessionId;
  createParams: GCodeSessionCreateParams;
}): Promise<void> {
  ...
  params.context.logger?.info("GCode Protocol imported history persisted", {
    event: "gcode_protocol.session_import.persisted",
...
async function createSessionWithProjection<T>(
  context: GCodeProtocolAgentServerContext,
  rawParams: unknown,
  trace: GCodeProtocolTrace | undefined,
  project: (
    record: GCodeProtocolSessionRecord,
  ) => Promise<{ value: T; phaseDurationsMs?: SnapshotPhaseDurationsMs; messageCount?: number }>,
): Promise<T> {
  const params = parseParams(gcodeSessionCreateParamsSchema, rawParams);
  ...
  context.logger?.info("GCode Protocol session/create started", {
    event: "gcode_protocol.session_create.started",
  ...
  try {
    await runSessionModelConfigMutation(record.app, async () => {
      ...
      if (supportedInitialThoughtLevel) {
        ...
        context.logger?.warn("GCode Protocol session/create skipped unsupported thought level", {
          event: "gcode_protocol.session_create.thought_level_skipped",
          module: "bootstrap.gcode_protocol",
    ...
    if (params.importedHistory) {
      ...
      // 让导入结果从第一刻起就是可续聊、可切模型的 GCode session。
    ...
    context.logger?.info("GCode Protocol session/create completed", {
      durationMs: Date.now() - startedAt,
      event: "gcode_protocol.session_create.completed",
    ...
    context.logger?.warn("GCode Protocol session/create failed", {
      durationMs: Date.now() - startedAt,
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "gcode_protocol.session_create.failed",
      module: "bootstrap.gcode_protocol",
      persistence: params.persistence,
      recordCreateDurationMs,
      rootTraceId: record.traceContext.traceId,
...
export async function activateSessionForResume(
  context: GCodeProtocolAgentServerContext,
  params: GCodeSessionResumeParams,
  ...
  context.logger?.debug("GCode Protocol session resume started", {
    activeBeforeWait,
    event: "gcode_protocol.session.resume_started",
  ...
  if (!session) {
    ...
    context.logger?.warn("GCode Protocol session resume found no persisted record", {
      activeBeforeWait,
      activeSessionCount: context.sessions.size,
      durationMs: Math.max(0, Date.now() - resumeStartedAt),
      event: "gcode_protocol.session.resume_persisted_missing",
      hasSessionStore: Boolean(context.deps.sessionStore),
      module: "bootstrap.gcode_protocol",
      sessionId: params.sessionId,
    });
    throw new ProtocolRequestError(
      gcodeProtocolErrorCodes.sessionUnavailable,
  ...
  context.logger?.info("GCode Protocol session resume completed", {
    activeSessionCount: context.sessions.size,
    durationMs: Math.max(0, Date.now() - resumeStartedAt),
    event: "gcode_protocol.session.resume_completed",
    module: "bootstrap.gcode_protocol",
...
export async function resumeSession(context: GCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(gcodeSessionResumeParamsSchema, rawParams);
...
async function repairLegacyRemoteSessionWorkspaceForResume(
  ...
  if (!repairPersisted) {
    context.logger?.warn("legacy remote session workspace repair was not persisted", {
      event: "gcode_protocol.session_resume.legacy_remote_workspace_repair_rejected",
      legacyWorkspaceDirectory,
  ...
  if (repaired) {
    context.logger?.info("legacy remote session workspace repaired", {
      event: "gcode_protocol.session_resume.legacy_remote_workspace_repaired",
      legacyWorkspaceDirectory,
      legacyWorkspaceIdentity: legacyRemote.workspaceIdentity,
      module: "bootstrap.gcode_protocol",
      sessionId: session.id,
...
export async function listSessions(context: GCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(gcodeSessionListParamsSchema, rawParams ?? {});
...
export async function listSessionSubagents(
  ...
  const params = parseParams(gcodeSessionSubagentsParamsSchema, rawParams ?? {});
  const store = context.deps.sessionStore;
  ...
  if (!parentSession) {
    ...
    context.logger?.warn("GCode Protocol session subagents has no persisted parent", {
      activeSessionCount: context.sessions.size,
      activeSession: Boolean(liveParent),
      event: "gcode_protocol.session.persisted_missing",
      module: "bootstrap.gcode_protocol",
...
export async function getUsageStats(context: GCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(gcodeUsageStatsParamsSchema, rawParams ?? {});
...
export async function readSession(context: GCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(gcodeSessionReadParamsSchema, rawParams);
...
export async function readMessages(context: GCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(gcodeSessionMessagesParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function readEvents(context: GCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(gcodeSessionEventsParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  return {
...
export async function subscribeSession(
  ...
  const params = parseParams(gcodeSessionSubscribeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function sendPrompt(context: GCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(gcodeSessionSendParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  ...
  if (record.activeAbortController) {
    context.logger?.warn("GCode Protocol session/send rejected: active prompt exists", {
  ...
  context.logger?.info("GCode Protocol session/send accepted", {
...
export async function compactSession(context: GCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(gcodeSessionCompactParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
async function runCompactTurnInBackground(
  ...
  context.logger?.info("GCode Protocol background compact started", {
    inputId: params.inputId,
    sessionId: record.app.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  try {
    ...
    context.logger?.info("GCode Protocol background compact completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      sessionId: record.app.sessionId,
      workspacePath: record.workspace.workspacePath,
    ...
    context.logger?.warn("GCode Protocol background compact failed", {
      durationMs: Date.now() - startedAt,
...
export async function goalSession(context: GCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(gcodeSessionGoalParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  ...
  if (params.action === "pause") {
    ...
    if (target && activeAbortController) {
      context.logger?.info("GCode Protocol goal pause aborting active turn", {
        inputId: params.inputId,
        sessionId: params.sessionId,
        targetId: target.targetID,
        workspacePath: record.workspace.workspacePath,
      });
      activeAbortController.abort(new Error("GCode Protocol goal paused"));
    }
...
export async function forkSession(context: GCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(gcodeSessionForkParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function registerForkedSession(
  context: GCodeProtocolAgentServerContext,
  record: GCodeProtocolSessionRecord,
  fork: Awaited<ReturnType<GCodeProtocolSessionRecord["app"]["forkFromCheckpoint"]>>,
  options: {
    runtimeConfig: {
...
async function inheritForkedSessionTarget(
  context: GCodeProtocolAgentServerContext,
  parentRecord: GCodeProtocolSessionRecord,
  forkRecord: GCodeProtocolSessionRecord,
...
async function runPromptTurnInBackground(
  ...
  const startedAt = Date.now();
  context.logger?.info("GCode Protocol background turn started", {
    inputId: params.inputId,
    queryId: params.queryId,
    sessionId: record.app.sessionId,
    textLength: params.content.length,
  ...
  try {
    ...
    context.logger?.info("GCode Protocol background turn completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      queryId: params.queryId,
      sessionId: record.app.sessionId,
    ...
    context.logger?.warn("GCode Protocol background turn failed", {
      durationMs: Date.now() - startedAt,
      error: error instanceof Error ? error.message : String(error),
      inputId: params.inputId,
      sessionId: record.app.sessionId,
    ...
    if (record.activeAbortController === params.abortController) {
      ...
      record.activeAbortController = undefined;
      context.logger?.info("GCode Protocol background turn cleared active controller", {
        durationMs: Date.now() - startedAt,
        inputId: params.inputId,
        sessionId: record.app.sessionId,
        workspacePath: record.workspace.workspacePath,
...
export async function stopSession(context: GCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(gcodeSessionStopParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  const hadActivePrompt = Boolean(record.activeAbortController);
  let pausedTarget: ProtocolGoalTarget | null = null;
  context.logger?.info("GCode Protocol session/stop received", {
    hadActivePrompt,
    sessionId: params.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  ...
  record.activeAbortController?.abort(new Error("GCode Protocol session stopped"));
  if (pausedTarget) {
...
async function pauseActiveGoalForSessionStop(
  ...
  try {
    ...
    if (pausedTarget) {
      context.logger?.info("GCode Protocol session/stop paused active goal", {
        sessionId: params.sessionId,
    ...
    context.logger?.warn("GCode Protocol session/stop failed to pause active goal", {
      error: error instanceof Error ? error.message : String(error),
      sessionId: params.sessionId,
      targetId: target.targetID,
      workspacePath: record.workspace.workspacePath,
...
export async function cancelBackgroundTask(
  ...
  const params = parseParams(gcodeSessionCancelBackgroundTaskParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  context.logger?.info("GCode Protocol session/cancelBackgroundTask received", {
    sessionId: params.sessionId,
...
export async function setModel(context: GCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(gcodeSessionSetModelParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  await runSessionModelConfigMutation(record.app, async () => {
...
export async function setThoughtLevel(
  ...
  const params = parseParams(gcodeSessionSetThoughtLevelParamsSchema, rawParams);
...
export async function setMode(context: GCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(gcodeSessionSetModeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function closeSession(context: GCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(gcodeSessionCloseParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
function shouldCloseSessionForExpectedPersistence(
  currentPersistence: GCodeSessionPersistence,
  expectedPersistence?: GCodeSessionPersistence,
): boolean {
...
export async function generateWorkspaceText(
  ...
  const params = parseParams(gcodeWorkspaceGenerateTextParamsSchema, rawParams);
  ...
  const app =
    active?.app ??
    (await createWorkspaceGCodeApp(context, params.workspace, {
...
function ensureNoActiveTurn(record: GCodeProtocolSessionRecord, message: string): void {
  if (!record.activeAbortController) {
    return;
  }
...
function assertExpectedProviderRevision(
  _context: GCodeProtocolAgentServerContext,
  _record: GCodeProtocolSessionRecord,
...
async function resolveForkTarget(
  context: GCodeProtocolAgentServerContext,
  record: GCodeProtocolSessionRecord,
  target: GCodeSessionHistoryTarget,
...
export async function getTaskTokenUsage(
  ...
  const params = parseParams(gcodeTaskTokenUsageParamsSchema, rawParams ?? {});
  const usageStore = context.deps.sessionStore as Partial<UsageStorePort> | undefined;
 ...
 */
function reconcileRecordPersistence(record: GCodeProtocolSessionRecord): void {
...
export function onSessionEvent(
  ...
  if (computerUseOperationEvent) {
    try {
      ...
      context.notify({
        method: gcodeProtocolMethods.computerUseOperationEvent,
      ...
      context.logger?.debug("Computer Use operation lifecycle notification sent", {
        event: "gcode_protocol.computer-use.operation-event.sent",
      ...
      context.logger?.warn("Computer Use operation lifecycle notification failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "gcode_protocol.computer-use.operation-event.failed",
        eventId: computerUseOperationEvent.eventId,
        sessionId: computerUseOperationEvent.sessionId,
        turnId:
          "turnId" in computerUseOperationEvent ? computerUseOperationEvent.turnId : undefined,
...
async function requestSessionRuntimePreferences(
  ...
  const startedAt = Date.now();
  // GCodeProtocolTrace.traceId 是协议层的 string；LogContext 需要 branded TraceId，
  // 按本文件既有惯例断言收口，避免多处日志构造重复转换。
  const traceId = trace?.traceId as TraceId | undefined;
  context.logger?.debug("GCode Protocol runtime preferences request started", {
    event: "gcode_protocol.runtime_preferences.request_started",
    module: "bootstrap.gcode_protocol",
    scope,
    sessionId,
    traceId,
  ...
  try {
    const result = await context.requestClient(
      gcodeProtocolMethods.sessionRequestRuntimePreferences,
      { sessionId, scope },
      gcodeSessionRuntimePreferencesResultSchema,
      {
    ...
    context.logger?.debug("GCode Protocol runtime preferences response received", {
      durationMs: Math.max(0, Date.now() - startedAt),
      event: "gcode_protocol.runtime_preferences.response_received",
      module: "bootstrap.gcode_protocol",
      scope,
      sessionId,
      traceId,
    ...
    const diagnostic = {
      ...
      errorCode,
      errorMessage,
      event: "gcode_protocol.runtime_preferences.request_failed",
      module: "bootstrap.gcode_protocol",
      scope,
      sessionId,
      traceId,
    ...
    if (errorCode === -32022) {
      ...
      context.logger?.warn("GCode Protocol runtime preferences request timed out", diagnostic);
    } else if (errorCode !== -32601 && errorCode !== -32020) {
      context.logger?.warn("GCode Protocol runtime preferences request failed", diagnostic);
    } else {
      context.logger?.debug(
        "GCode Protocol runtime preferences compatibility fallback",
        diagnostic,
      );
    }
    if (error instanceof ProtocolRequestError && (error.code === -32601 || error.code === -32020)) {
      ...
      return {
        ...
        memoryEnabled: false,
        modelContextBudgetStrategy: DEFAULT_GCODE_MODEL_CONTEXT_BUDGET_STRATEGY,
...
async function materializeSessionRecord(
  context: GCodeProtocolAgentServerContext,
  params: GCodeSessionRecordParams,
  sessionId: SessionId,
  resume: boolean,
  source: SessionStartupPreferencesSource,
  trace?: GCodeProtocolTrace,
): Promise<GCodeProtocolSessionRecord> {
...
async function createRecord(
  ...
  context.logger?.info("GCode Protocol createRecord MCP config", {
    event: "gcode_protocol.create_record.mcp_config",
  ...
  let ownSessionRecord: GCodeProtocolSessionRecord | undefined;
  const app = await createWorkspaceGCodeApp(context, workspace, {
    env: context.deps.env,
    eventStore,
    resume,
    runtimeConfig: {
      ...
      // desktop-continuous session/create 由 UI 先解析 ~/.gcode/.agents 的 enabled MCP，
      ...
      // GCode Protocol app-server 创建的 session（desktop/web/mobile）没有传，导致
    ...
    // GCode Protocol app-server 以前没有注入可等待的交互 broker，
  ...
  const now = Date.now();
  const record: GCodeProtocolSessionRecord = {
    app,
    createdAt: now,
    eventStore,
...
): GCodeSessionCreateParams["mode"] | undefined {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const info = messages[index]?.info;
    if (info?.role === "assistant" && isGCodeSessionMode(info.mode)) return info.mode;
  }
  return undefined;
}

function isGCodeSessionMode(
  value: unknown,
): value is NonNullable<GCodeSessionCreateParams["mode"]> {
  return (
    value === "plan" ||
    value === "build" ||
    value === "edit" ||
...
async function snapshotWithDiagnostics(
  ...
  if (totalDurationMs >= SLOW_SNAPSHOT_LOG_THRESHOLD_MS) {
    ...
    context.logger?.warn("GCode Protocol session snapshot slow", {
      durationMs: totalDurationMs,
      event: "gcode_protocol.session_snapshot.slow",
...
async function readSnapshotTarget(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session goal for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "gcode_protocol.snapshot.target_read_failed",
      sessionId: record.app.sessionId,
    });
    return undefined;
  }
...
async function readSnapshotTodos(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session todos for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "gcode_protocol.snapshot.todos_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
...
async function readSessionTargetCompletionVerificationEvents(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted goal verification entries", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "gcode_protocol.snapshot.goal_verification_entries_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
  ...
  return await repairPersistedRemoteSessionPaths(sessionStore, session, {
    onPersistenceFailure: (error) => {
      context.logger?.warn("Session path repair persistence failed; using in-memory repair", {
        errorMessage: error instanceof Error ? error.message : String(error),
        event: "gcode_protocol.session.path_repair.persist_failed",
        sessionId,
      });
    },
  });
...
function emitStateUpdated(
  ...
  const notification: GCodeStateUpdatedNotification = {
    patch,
    reason,
