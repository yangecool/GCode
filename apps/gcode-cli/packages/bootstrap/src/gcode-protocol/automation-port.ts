...
import {
  ...
  gcodeAutomationCheckTaskBindingResultSchema,
  gcodeAutomationCreateResultSchema,
  gcodeAutomationDeleteResultSchema,
  gcodeAutomationListResultSchema,
  gcodeAutomationUpdateResultSchema,
  gcodeProtocolMethods,
  type GCodeAutomationProtocol,
} from "@gcode/shared";
import {
...
export function createProtocolAutomationPort(
  ...
  resolveOwnSession?: () => GCodeProtocolSessionRecord | undefined,
): AutomationPort {
  return {
    async create(input, createContext) {
        ...
        try {
          try {
            const result = await context.requestClient(
              gcodeProtocolMethods.automationCheckTaskBinding,
            ...
            const legacyResult = await context.requestClient(
              gcodeProtocolMethods.automationList,
              {},
              gcodeAutomationListResultSchema,
            );
      ...
      try {
        result = await context.requestClient(
          gcodeProtocolMethods.automationCreate,
          {
    ...
    async update(input) {
      const hasIntervalCarrier = input.intervalUnit !== undefined && input.interval !== undefined;
      const result = await context.requestClient(
        gcodeProtocolMethods.automationUpdate,
        {
    ...
    async delete(input) {
      const result = await context.requestClient(
        gcodeProtocolMethods.automationDelete,
        { automationId: input.id },
        gcodeAutomationDeleteResultSchema,
      );
...
function normalizeCronAutomationMode(
  mode: GCodeAutomationProtocol["mode"],
): CronAutomation["mode"] {
  switch (mode) {
    case undefined:
      return undefined;
...
function toCronAutomation(input: GCodeAutomationProtocol): CronAutomation {
  return {
