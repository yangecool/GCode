...
import { SqliteSessionStore, type SqliteMigrationProgress } from "@gcode/adapters/storage";
import {
  classifyDatabaseStartupError,
  databaseStartupErrorDetails,
  gcodeProtocolNotifications,
  gcodeStorageStartupStateSchema,
  type GCodeStorageStartupState,
} from "@gcode/shared";

export async function openProtocolStartupStorage(options: {
  dbPath: string;
  output: NodeJS.WritableStream;
  onProgress?: (progress: GCodeStorageStartupState) => void;
  ...
  const report = async (progress: SqliteMigrationProgress) => {
    const params = gcodeStorageStartupStateSchema.parse({
      schemaVersion: 1,
      attemptId,
      databaseId,
    ...
    await new Promise<void>((resolve, reject) => {
      options.output.write(
        `${JSON.stringify({ method: gcodeProtocolNotifications.storageStartup, params })}\n`,
...
export async function prepareProtocolStartupStorage(options: {
  ...
  const { gcodeStoragePathReadySchema, classifyDatabaseStartupError } =
    await import("@gcode/shared");
  ...
  const acknowledgement = new Promise<boolean>((resolve, reject) => {
    ...
    lines.once("line", (line) => {
      try {
        ...
        const ack = gcodeStoragePathReadySchema.parse(JSON.parse(line));
