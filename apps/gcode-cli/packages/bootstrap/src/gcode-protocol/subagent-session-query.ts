...
import type { GCodeSessionEndedSubagent, GCodeSessionRunningSubagent } from "@gcode/shared";
...
interface SessionSubagentProjection {
  revision: number;
  running: GCodeSessionRunningSubagent[];
  ended: GCodeSessionEndedSubagent[];
}
...
function runningStatus(input: {
  ...
  parentProjection?: SessionProjection;
}): GCodeSessionRunningSubagent["status"] | undefined {
...
): GCodeSessionEndedSubagent["status"] | undefined {
  switch (task?.status) {
    case "completed":
      return "success";
    case "cancelled":
...
function endedStatus(input: {
  ...
  childOutcome: ReturnType<typeof lastChildOutcome>;
  childProjection?: SessionProjection;
}): GCodeSessionEndedSubagent["status"] {
...
export function projectSessionSubagents(
  ...
  const running: GCodeSessionRunningSubagent[] = [];
  const ended: GCodeSessionEndedSubagent[] = [];
...
function encodeCursor(item: GCodeSessionEndedSubagent): string {
  return Buffer.from(
...
export function paginateEndedSubagents(
  ended: readonly GCodeSessionEndedSubagent[],
  options: { cursor?: string; limit: number },
): { items: GCodeSessionEndedSubagent[]; nextCursor?: string } {
