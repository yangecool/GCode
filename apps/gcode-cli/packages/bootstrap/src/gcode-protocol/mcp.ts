...
import type { Logger, McpConnectionSnapshot, McpPort, McpServerStatus } from "@gcode/contracts";
import {
  gcodeMcpListParamsSchema,
  gcodeMcpListResultSchema,
  type GCodeMcpListResult,
} from "@gcode/shared";
import {
...
export async function listMcpServers(
  ...
  const params = parseParams(gcodeMcpListParamsSchema, rawParams);
  ...
  const startupTimer = new StartupTimer(
    ...
    {
      module: "bootstrap.gcode_protocol.mcp",
  ...
  const configuredMcpServers = {
    ...
    // 设置页的本地 MCP 列表由 desktop main 解析 `.gcode` / `.agents` fallback，
  ...
  if (mcpPort) {
    if (params.mode === "status") {
      ...
      return gcodeMcpListResultSchema.parse({ statuses });
    }
    ...
    if (pendingAuthorizationSnapshot) {
      ...
      return gcodeMcpListResultSchema.parse({
