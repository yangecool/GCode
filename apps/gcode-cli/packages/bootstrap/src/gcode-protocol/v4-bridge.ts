...
// - 命令执行 = V4CommandExecutor（gcode-protocol-v4/commands/，原生直驱 core）；
...
import {
  ...
  type GCodeSessionContextUsage,
  type GCodeWorkspaceRef,
} from "@gcode/shared";
import { createExternalTurnFaultError } from "@gcode/core";
import {
  V4_NOTIFICATIONS,
...
import { V4CommandExecutor } from "../gcode-protocol-v4/commands/executor.js";
import { V4QueuePromotionLeaseUnavailableError } from "../gcode-protocol-v4/commands/handlers/queue.js";
import { V4CapabilityUnsupportedError } from "../gcode-protocol-v4/commands/handlers/interaction-background.js";
import {
  buildColdFileChangeSummaries,
  readConversationFileChangesFromEvents,
} from "../gcode-protocol-v4/cold-file-change-summaries.js";
import {
  loadPersistedConversationMaterialization,
  mergeColdConversationEvents,
} from "../gcode-protocol-v4/cold-event-merge.js";
import { lookupGlobalCreateSessionCommand } from "../gcode-protocol-v4/create-session-command-fact.js";
import type { V4CommandCoreHost } from "../gcode-protocol-v4/commands/types.js";
import type {
  ConversationRowTargetResolution,
  SessionUsageSeed,
} from "../gcode-protocol-v4/product-projection.js";
import { PersistentCommandIndex } from "../gcode-protocol-v4/persistent-command-index.js";
import { queueItemIdForCommand } from "../gcode-protocol-v4/command-inbox.js";
import { resolveStableForkTargetFromTranscript } from "../gcode-protocol-v4/stable-fork-target.js";
import { shouldAutoDrainV4QueueHead } from "../gcode-protocol-v4/queue-auto-drain.js";
import { persistAssistantFeedback } from "../gcode-protocol-v4/assistant-feedback-persistence.js";
import {
...
} from "../gcode-protocol-v4/persistent-command-facts.js";
import {
  ConversationV4Gateway,
  V4CommandNotImplementedError,
} from "../gcode-protocol-v4/v4-gateway.js";
import {
...
import { HYDRATION_TRACE_ID } from "../gcode-protocol-v4/projection-state.js";
...
import type {
  GCodeProtocolAgentServerContext,
...
function sessionUsageSeedFromRuntimeContextUsage(
  contextUsage: GCodeSessionContextUsage | undefined,
...
function cloneModelSelection(
  selection: ReturnType<GCodeProtocolSessionRecord["app"]["runtime"]["getSessionModelSelection"]>,
): ModelSelection | undefined {
...
async function replayDynamicWorkflowRunEvents(
  ...
  try {
    ...
    context.logger?.warn("v4 hydrate dynamic workflow replay failed", {
      ...
      event: "gcode_protocol.v4.hydrate_workflow_replay_failed",
...
async function resolveConversationBackingRecord(
  context: GCodeProtocolAgentServerContext,
  sessionId: string,
): Promise<GCodeProtocolSessionRecord | undefined> {
...
export function createConversationV4Gateway(
  ...
  const log = createProtocolLogger(context.deps)?.child({
    module: "bootstrap.gcode_protocol_v4_gateway",
  });
  ...
  const autoDrainV4QueueIfReady = async (record: GCodeProtocolSessionRecord): Promise<void> => {
  ...
  const scheduleAutoDrainRetry = (record: GCodeProtocolSessionRecord): void => {
  ...
  const coreHost: V4CommandCoreHost = {
    ...
    ensureModelReady: (record) =>
      ensureSessionModelAvailableForNextTurn(context, record as GCodeProtocolSessionRecord),
    ...
    afterLegacyStateMutation: async (record, reason) => {
      await afterStateMutation(context, record as GCodeProtocolSessionRecord, reason);
      await autoDrainV4QueueIfReady(record as GCodeProtocolSessionRecord);
    },
  ...
  const loadStoredSessionSummaries = async (
    ...
    try {
        ...
        try {
          ...
          if (claimedCount > 0) {
            context.logger?.info("legacy remote sessions claimed by task-index allowlist", {
              claimedCount,
              event: "gcode_protocol.v4.sessions_index_legacy_remote_claimed",
              module: "bootstrap.gcode_protocol",
              workspaceId,
            });
          }
          ...
          context.logger?.warn("legacy remote sessions claim failed; continuing strict load", {
            error: error instanceof Error ? error.message : String(error),
            event: "gcode_protocol.v4.sessions_index_legacy_remote_claim_failed",
            module: "bootstrap.gcode_protocol",
            workspaceId,
          });
        }
      ...
      context.logger?.warn("sessions-index stored summaries failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "gcode_protocol.v4.sessions_index_stored_failed",
        module: "bootstrap.gcode_protocol",
      });
      return [];
    }
  ...
  return new ConversationV4Gateway({
    ...
    resumePersistedSession: async (
      ...
      if (!persisted) {
        context.logger?.warn("GCode Protocol v4 cold resume has no persisted session", {
          activeSessionCount: context.sessions.size,
          event: "gcode_protocol.v4.resume_persisted_missing",
          module: "bootstrap.gcode_protocol",
          sessionId,
        });
    ...
    loadPersistedEvents: async (sessionId, persistedMessages) => {
      ...
      if (!record) {
        ...
        context.logger?.warn("GCode Protocol v4 hydrate has no active runtime", {
          activeSessionCount: context.sessions.size,
          event: "gcode_protocol.v4.hydrate_runtime_missing",
          module: "bootstrap.gcode_protocol",
                      ...
                      sessionEntries: (input) =>
                        store.sessionEntries!(input).catch((error) => {
                          context.logger?.warn("v4 hydrate session entries read failed", {
                            error: error instanceof Error ? error.message : String(error),
                            event: "gcode_protocol.v4.hydrate_session_entries_failed",
                            module: "bootstrap.gcode_protocol",
                          });
                          return [];
                        }),
      ...
      const fileChangeSummariesByMessageId = await buildColdFileChangeSummaries({
        ...
        onArtifactError: (messageId, error) =>
          context.logger?.warn("v4 cold file change artifact read failed", {
            error: error instanceof Error ? error.message : String(error),
            event: "gcode_protocol.v4.hydrate_file_changes_failed",
            messageId,
            module: "bootstrap.gcode_protocol",
            sessionId,
          }),
      ...
      for (const diagnostic of merged.diagnostics) {
        const fields = {
          ...diagnostic,
          event: "gcode_protocol.v4.hydrate_three_source_merge",
          module: "bootstrap.gcode_protocol",
          sessionId,
        };
        if (
    ...
    onError: (scope, error, errorContext) =>
      context.logger?.warn("GCode Protocol v4 gateway error", {
        ...errorContext,
        error: error instanceof Error ? error.message : String(error),
        event: "gcode_protocol.v4.gateway_error",
        module: "bootstrap.gcode_protocol",
        scope,
      }),
  });
