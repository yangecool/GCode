...
// 解析器与序列化器只从 @gcode/core 取：这里不解析 frontmatter，也不拼 YAML。
//
// 全局作用域：五个方法的 params 收可选 `scope`（缺省
// `project`）。`global` 时改按本机全局根（`~/.gcode/workflows/`）操作，`workspace` 只是**载体**——
// 处理器对全局档不读它的路径。`workflows/move` 把全局档搬回 `workspace` 项目（只此一向）。
import { unlink, writeFile } from "node:fs/promises";
import { SavedWorkflowMetaSchema, isValidSavedWorkflowName } from "@gcode/contracts";
import {
  listSavedWorkflows,
  moveSavedWorkflow,
...
import {
  GCODE_WORKFLOWS_RUNS_MAX_LIMIT,
  gcodeWorkflowsDeleteParamsSchema,
  gcodeWorkflowsGetParamsSchema,
  gcodeWorkflowsListParamsSchema,
  gcodeWorkflowsMoveParamsSchema,
  gcodeWorkflowsRunsParamsSchema,
  gcodeWorkflowsUpdateMetaParamsSchema,
  type GCodeSavedWorkflowRun,
  type GCodeSavedWorkflowScope,
  type GCodeWorkflowsDeleteResult,
  type GCodeWorkflowsGetResult,
  type GCodeWorkflowsListResult,
  type GCodeWorkflowsMoveResult,
  type GCodeWorkflowsRunsResult,
  type GCodeWorkflowsUpdateMetaResult,
} from "@gcode/shared";
import type { JournalStorePort } from "@gcode/dynamic-workflow";
...
function scopeOf(params: { scope?: GCodeSavedWorkflowScope }): GCodeSavedWorkflowScope {
...
export async function listSavedWorkflowsOp(
  ...
  rawParams: unknown,
): Promise<GCodeWorkflowsListResult> {
  const params = parseParams(gcodeWorkflowsListParamsSchema, rawParams);
...
export async function getSavedWorkflowOp(
  _context: GCodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<GCodeWorkflowsGetResult> {
  const params = parseParams(gcodeWorkflowsGetParamsSchema, rawParams);
...
export async function updateSavedWorkflowMetaOp(
  ...
  const params = parseParams(gcodeWorkflowsUpdateMetaParamsSchema, rawParams);
...
export async function deleteSavedWorkflowOp(
  ...
  const params = parseParams(gcodeWorkflowsDeleteParamsSchema, rawParams);
...
export async function listSavedWorkflowRunsOp(
  ...
  const params = parseParams(gcodeWorkflowsRunsParamsSchema, rawParams);
  ...
  const limit = Math.min(GCODE_WORKFLOWS_RUNS_MAX_LIMIT, params.limit);
  ...
  const runs: GCodeSavedWorkflowRun[] = page.map((row) => ({
    runId: row.runId,
...
): Map<string, GCodeSavedWorkflowRun["artifacts"]> {
  const byRun = new Map<string, GCodeSavedWorkflowRun["artifacts"]>();
...
export async function moveSavedWorkflowOp(
  _context: GCodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<GCodeWorkflowsMoveResult> {
  const params = parseParams(gcodeWorkflowsMoveParamsSchema, rawParams);
