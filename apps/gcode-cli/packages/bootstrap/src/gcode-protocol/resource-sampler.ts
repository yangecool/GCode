...
import {
  createGCodeProcessResourceSampler,
  type GCodeProcessResourceSampler,
...
export function startProtocolResourceSampler(
  server: GCodeProtocolAgentServer,
  send: (message: GCodeProtocolNotification) => void,
  logger: Logger,
): GCodeProcessResourceSampler | undefined {
  try {
    // 本地内存诊断日志：复用同一 60s 节拍，
    // 变化/心跳门控后才写一行，避免与消息流同频刷盘。
    const memoryDiagnosticsGate = createMemorySampleWriteGate();
    const sampler = createGCodeProcessResourceSampler({
      onSample: (sample, memoryUsage) => {
        send({
          method: gcodeProtocolNotifications.processResourceSample,
          params: sample,
        });
        ...
        try {
          ...
          if (reason) {
            ...
            logger.info("Process memory sample", {
              event: "gcode_protocol.process.memory_sample",
              reason,
              ...memoryFields,
              counters,
            });
