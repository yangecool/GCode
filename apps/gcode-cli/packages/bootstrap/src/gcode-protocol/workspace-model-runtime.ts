...
import { createInMemorySessionEventStore } from "@gcode/adapters/storage";
import type { ModelSelection } from "@gcode/contracts";
import {
  gcodeProviderTestModelConnectivityParamsSchema,
  gcodeWorkspaceReadPresentationParamsSchema,
...
export async function readWorkspacePresentation(
  ...
  const params = parseParams(gcodeWorkspaceReadPresentationParamsSchema, rawParams);
  return {
...
export async function testProviderModelConnectivity(
  ...
  const params = parseParams(gcodeProviderTestModelConnectivityParamsSchema, rawParams);
...
export async function createWorkspaceGCodeApp(
  context: GCodeProtocolAgentServerContext,
  ...
  return context.deps.createGCodeApp({
    ...options,
    platform: context.deps.platform,
    providerRuntimeHeadersPort,
    runtimeConfig: {
      ...options.runtimeConfig,
      // createGCodeApp 会把 workingDirectory 规范化为执行 cwd。把协议入口的
...
export function hasSessionModelProvider(
  ...
  record: Pick<GCodeProtocolSessionRecord, "app" | "workspace">,
  providerId: string,
): boolean {
...
export function resolveSessionModelContextWindow(
  _context: GCodeProtocolAgentServerContext,
  record: Pick<GCodeProtocolSessionRecord, "app" | "workspace" | "restoredModelSelection">,
): number | undefined {
