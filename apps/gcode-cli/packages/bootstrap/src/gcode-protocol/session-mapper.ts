import {
  ...
  type GCodeActiveToolCall,
  type GCodeContextUsageBreakdownItem,
  type GCodeDeliveryKind,
  type GCodePendingPermission,
  type GCodeSessionContextUsage,
  type GCodeSessionEvent,
  type GCodeSessionGoal,
  type GCodeSessionGoalVerification,
  type GCodeSessionGoalVerificationTimeline,
  type GCodeSessionGoalStats,
  type GCodeSessionInfo,
  type GCodeSessionKind,
  type GCodeSessionProjection,
  type GCodeSessionRuntimeState,
  type GCodeSessionSettingsState,
  type GCodeSessionStateSnapshot,
  type GCodeSessionTodoGroup,
...
import type { GCodeApp } from "../app/types.js";
...
export async function buildSessionSnapshot(input: {
  ...
  return {
    ...
    protocol: {
      name: GCODE_PROTOCOL_NAME,
      version: GCODE_PROTOCOL_VERSION,
    },
...
async function mapSnapshotMessages(
  app: Pick<GCodeApp, "readToolResultArtifact">,
...
async function hydrateSnapshotFilePartUrl(
  ...
  // 历史图片附件持久化后只剩 gcode-artifact:// 引用，UI/手机端不能直接渲染。
...
function snapshotFilePartArtifactUri(
  ...
  return artifactUri.startsWith("gcode-artifact://") ? artifactUri : undefined;
}
...
): Promise<GCodeSessionSettingsState> {
...
export function mapSessionInfo(input: {
  app?: Pick<GCodeApp, "getMode" | "getModel" | "sessionId" | "traceId">;
  fallbackCreatedAt?: number;
  fallbackUpdatedAt?: number;
  ...
  return {
    ...
    sessionId,
    sessionKind: (input.session?.taskType ?? input.taskType ?? "interactive") as GCodeSessionKind,
...
export function mapSessionEvents(
  ...
  return events
    ...
    .filter((event): event is GCodeSessionEvent => event !== null);
}
...
function mapModelNetworkStatusPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = gcodeApiRetryFromModelNetworkStatusPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
  const gcodeMeta = asRecord(meta.gcode);
  return {
    ...record,
    _meta: {
      ...meta,
...
function mapStreamRecoveryPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = gcodeApiRetryFromStreamRecoveryPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
...
function mapSessionProjection(projection: SessionProjection): GCodeSessionProjection {
  return {
...
function mapRuntimeState(input: {
  activeTurn?: ReturnType<GCodeApp["runtime"]["getActiveTurnInfo"]>;
  deliveryKind?: GCodeDeliveryKind;
  eventSeq: number;
  messages: MessageWithParts[];
...
export function resolveSessionContextUsage(input: {
  ...
  persistedContextUsageBreakdownEvents?: readonly SessionEvent[];
  projection: SessionProjection;
}): GCodeSessionContextUsage | undefined {
...
): GCodeSessionGoalVerificationTimeline[] {
...
function contextUsageFromProjection(
  projection: SessionProjection,
  cache: GCodeSessionContextUsage["cache"] | undefined,
): GCodeSessionContextUsage | undefined {
...
function contextCacheUsageFromMessages(
  messages: readonly MessageWithParts[],
): GCodeSessionContextUsage["cache"] | undefined {
  let inputTokens = 0;
...
function mapPendingPermission(permission: PendingPermission): GCodePendingPermission {
  // display / optionsPolicy 刻意不进 legacy v3 输出。
  // 根因不是"扩 schema 只能单向兼容"，而是 strict schema 随 packages/shared 打进每个桌面端
  // 的产物：今天把 gcodePendingPermissionSchema（shared/src/gcode-protocol/index.ts:1139）和
  // gcodePermissionRequestedEventPayloadSchema（同文件:1536）改成可选，也保护不了已经装出去
...
function mapPermissionRequestedPayload(payload: unknown): Record<string, unknown> {
  ...
  // 剔除，否则会直接漏进 strict 的 gcodePermissionRequestedEventPayloadSchema。
...
function mapActiveToolCall(toolCall: ActiveToolCall): GCodeActiveToolCall {
  return {
...
function mapSessionGoal(goal: SessionGoal | null | undefined): GCodeSessionGoal | null | undefined {
...
function buildGoalStats(
  projection: SessionProjection,
  messages: readonly MessageWithParts[],
): GCodeSessionGoalStats | undefined {
  const target = projection.target;
  if (!target) {
    return undefined;
  }
...
function buildTodoGroups(
  ...
  projection: SessionProjection,
): GCodeSessionTodoGroup[] {
  const target = projection.target;
  const timeline = getTargetGoalVerificationTimeline(projection, target);
  const groups = new Map<string, GCodeSessionTodoGroup>();
...
function ensureTodoGroup(
  groups: Map<string, GCodeSessionTodoGroup>,
  input: {
  ...
  const group: GCodeSessionTodoGroup = {
    id: input.groupId,
...
function mapSessionEventType(type: SessionEvent["type"]): GCodeSessionEvent["type"] {
  switch (type) {
