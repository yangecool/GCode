import type { OffPeakPort, OffPeakTaskSummary } from "@gcode/contracts";
import {
  gcodeOffPeakCreateResultSchema,
  gcodeOffPeakListResultSchema,
  gcodeProtocolMethods,
  type GCodeOffPeakTaskProtocolSnapshot,
} from "@gcode/shared";
import type {
  ...
  return {
    async create(input, createContext) {
      ...
      if (boundSessionId) {
        ...
        try {
          const listed = await context.requestClient(
            gcodeProtocolMethods.offPeakList,
            {},
            gcodeOffPeakListResultSchema,
          );
      ...
      const result = await context.requestClient(
        gcodeProtocolMethods.offPeakCreate,
        {
          title: input.title,
          prompt: input.prompt,
...
function toOffPeakTaskSummary(input: GCodeOffPeakTaskProtocolSnapshot): OffPeakTaskSummary {
  return {
