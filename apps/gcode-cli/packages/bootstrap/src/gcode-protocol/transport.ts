...
import { gcodeProtocolMessageSchema, gcodeProtocolMethods } from "@gcode/shared";
import type { Logger } from "@gcode/contracts";

type GCodeProtocolOutgoingMessage =
  | GCodeProtocolError
  | GCodeProtocolNotification
  | GCodeProtocolRequest
  | GCodeProtocolResponse;

type GCodeProtocolMessageHandler = (
  message: GCodeProtocolMessage,
) => Promise<GCodeProtocolOutgoingMessage | undefined>;

const PROTOCOL_EOF_DRAIN_MS = 100;

interface GCodeProtocolNdjsonConnectionOptions {
  signal?: AbortSignal;
  clearPostResponseMessages?: () => void;
  handleMessage: GCodeProtocolMessageHandler;
  ...
  ) => readonly GCodeProtocolOutgoingMessage[];
  takePostResponseBatch?: (requestId: GCodeProtocolRequestId) => {
    readonly messages: readonly GCodeProtocolOutgoingMessage[];
    commit(): boolean;
  } | null;
}

export class GCodeProtocolNdjsonConnection {
  private buffer = "";
  ...
  constructor(private readonly options: GCodeProtocolNdjsonConnectionOptions) {
  ...
  send(message: GCodeProtocolOutgoingMessage): void {
  ...
  private readonly onClose = (): void => {
    ...
    this.draining = true;
    this.notifyTransportClosed(new Error("GCode Protocol client connection closed"));
  ...
  private readonly onAbort = (): void => {
    this.notifyTransportClosed(new Error("GCode Protocol runtime stopping"));
    this.finish();
  };
  ...
  private decodeLine(line: string): GCodeProtocolMessage | null {
    let decoded: unknown;
    try {
      decoded = JSON.parse(line);
    } catch (error) {
      this.options.logger?.warn("GCode Protocol JSON parse failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "gcode_protocol.parse.failed",
    ...
    const parsed = gcodeProtocolMessageSchema.safeParse(decoded);
    if (!parsed.success) {
      this.sendError("invalid-message", -32600, "Invalid GCode Protocol message", {
  ...
  private shouldBypassProcessingQueue(message: GCodeProtocolMessage): boolean {
    // 模型任务占住串行队列时，停止/取消请求必须仍能进入 server，
    // 才能把底层 AbortSignal 传给真实模型请求。控制面只旁路当前执行，普通请求仍保持串行。
    return (
      "id" in message &&
      "method" in message &&
      (message.method === gcodeProtocolMethods.sessionStop ||
        message.method === gcodeProtocolMethods.workspaceCancelGenerateText)
    );
  }

  private async handleMessage(message: GCodeProtocolMessage): Promise<void> {
    if (this.terminal) return;
