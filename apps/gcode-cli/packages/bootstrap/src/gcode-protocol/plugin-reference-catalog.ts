...
import {
  ...
  gcodePluginsReferenceCatalogParamsSchema,
  gcodePluginsResolveSuggestedReferenceParamsSchema,
  type GCodePluginReferenceCatalogEntry,
  type GCodePluginDiagnostic as SharedPluginDiagnostic,
  type GCodePluginsReferenceCatalogResult,
  type GCodePluginsResolveSuggestedReferenceResult,
} from "@gcode/shared";
import type { PluginReferenceCatalogEntry } from "@gcode/contracts";
import { buildPluginReferenceCatalog } from "@gcode/core";
import {
...
export async function getPluginReferenceCatalog(
  ...
  const params = parseParams(gcodePluginsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
...
const SUGGESTED_PLUGIN_MARKETPLACE = GCODE_OFFICIAL_PLUGIN_MARKETPLACE_ID;
...
export async function resolveSuggestedPluginReference(
  ...
  rawParams: unknown,
  signal?: AbortSignal,
): Promise<GCodePluginsResolveSuggestedReferenceResult> {
  const params = parseParams(gcodePluginsResolveSuggestedReferenceParamsSchema, rawParams);
  ...
  const readState = () => {
    const outcome = resolveGCodePlugins({ logger: context.logger, workingDirectory });
  ...
  context.notify({
    method: gcodeProtocolNotifications.pluginOperationProgress,
  ...
  try {
    const refreshRequest = updateGCodePluginMarketplace({
  ...
  const overview = getGCodePluginsOverview({ logger: context.logger, workingDirectory });
...
function resolveReferenceListingDisplayByPluginId(
  ...
  const overview = getGCodePluginsOverview({ workingDirectory: workspacePath });
...
function toReferenceCatalogEntry(
  ...
  includeCategory = false,
): GCodePluginReferenceCatalogEntry {
