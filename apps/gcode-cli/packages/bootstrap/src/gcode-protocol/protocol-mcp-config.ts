import type { McpServerConfig } from "@gcode/contracts";
import type { GCodeProtocolMcpServer } from "@gcode/shared";

export function protocolMcpServersToRuntimeMcpConfig(
  servers: GCodeProtocolMcpServer[] | undefined,
  ...
  for (const server of servers) {
    if ("command" in server) {
      runtimeServers[server.name] = {
        ...
        // GCode Protocol 的 mcpServers 是 session/runtime 和状态查询的完整覆盖配置。
