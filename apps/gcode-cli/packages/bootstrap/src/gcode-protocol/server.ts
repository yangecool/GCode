...
import {
  gcodePluginsCancelOperationParamsSchema,
  gcodeProtocolMethods,
  gcodeWorkspaceCancelGenerateTextParamsSchema,
  gcodeWorkspaceHookTrustGrantParamsSchema,
} from "@gcode/shared";
import type { BrowserControlPort } from "@gcode/contracts";
import { InMemoryWorkspaceHookPolicyProvider } from "@gcode/core";
import {
  V4_METHODS,
  V4_NOTIFICATIONS,
...
import type {
  GCodeProtocolError,
  GCodeProtocolMessage,
  GCodeProtocolMethod,
  GCodeProtocolNotification,
  GCodeProtocolRequest,
  GCodeProtocolRequestId,
  GCodeProtocolResponse,
} from "@gcode/shared";
import {
...
} from "../gcode-protocol-v4/interaction-registry.js";
...
import {
  ...
  type GCodeProtocolAgentDependencies,
  type GCodeProtocolAgentServerContext,
  type GCodeProtocolSessionRecord,
} from "./server-types.js";
import { createInMemorySessionEventStore } from "@gcode/contracts";

export type { GCodeProtocolAgentDependencies, GCodeProtocolSessionRecord };

const MAX_CLIENT_REQUEST_REANNOUNCE_INTERVAL_MS = 10_000;

type GCodeProtocolOutboundMessage = GCodeProtocolNotification | GCodeProtocolRequest;
...
async function notifyWorkspaceHookTrustGrantSessions(input: {
  ...
  sessions: Map<string, GCodeProtocolSessionRecord>;
}): Promise<void> {
...
interface GCodeProtocolPostResponseBatch {
  readonly messages: readonly GCodeProtocolOutboundMessage[];
  commit(): boolean;
}
...
export class GCodeProtocolAgentServer {
   ...
   */
  get officialMcpAuthRequestContext(): Pick<GCodeProtocolAgentServerContext, "requestClient"> {
    return this.context;
  }

  private messageSink?: (message: GCodeProtocolOutboundMessage) => void;
  private clientDisconnectError?: Error;
  private readonly context: GCodeProtocolAgentServerContext;
  ...
  private readonly postResponseOutbox = new Map<
    GCodeProtocolRequestId,
    GCodeProtocolPostResponseBatch
  >();
  private nextClientRequestId = 1;

  constructor(deps: GCodeProtocolAgentDependencies) {
    this.runtimeResources = new ProtocolRuntimeResources(deps.createGCodeApp);
    const resolvedDeps = {
      ...deps,
      createGCodeApp: this.runtimeResources.create,
      // 默认 turn 窗口保留策略。
    ...
    this.context = {
      ...
      sessions: new Map<string, GCodeProtocolSessionRecord>(),
  ...
  setNotificationSink(sink: (message: GCodeProtocolOutboundMessage) => void): void {
  ...
  shutdown(): Promise<void> {
    ...
    const error = new Error("GCode Protocol runtime stopping");
    ...
    for (const record of this.context.sessions.values()) {
      ...
      try {
        ...
        this.logger?.warn("Session unsubscribe failed during protocol shutdown", {
          event: "gcode_protocol.session.unsubscribe.failed",
        });
      }
    }
  ...
  takePostResponseMessages(requestId: GCodeProtocolRequestId): GCodeProtocolOutboundMessage[] {
  ...
  takePostResponseBatch(requestId: GCodeProtocolRequestId): GCodeProtocolPostResponseBatch | null {
  ...
  async handleMessage(
    message: GCodeProtocolMessage,
  ): Promise<GCodeProtocolError | GCodeProtocolResponse | undefined> {
    this.runtimeResources.assertServing();
    ...
    if (isNotification(message)) {
      this.logger?.debug("GCode Protocol notification ignored", {
        event: "gcode_protocol.notification.ignored",
  ...
  private async handleRequest(
    request: GCodeProtocolRequest,
  ): Promise<GCodeProtocolError | GCodeProtocolResponse> {
  ...
  private async dispatchRequest(request: GCodeProtocolRequest) {
      ...
      case gcodeProtocolMethods.workspaceHookTrustGrant: {
        ...
        if (grantResult.accepted) {
          await notifyWorkspaceHookTrustGrantSessions({
            ...
            grantedWorkspaceKey: gcodeWorkspaceHookTrustGrantParamsSchema.safeParse(request.params)
              .success
              ? gcodeWorkspaceHookTrustGrantParamsSchema.parse(request.params).workspace
                  .workspaceKey
              : undefined,
  ...
  private cancelPluginOperation(rawParams: unknown) {
    const params = parseParams(gcodePluginsCancelOperationParamsSchema, rawParams);
  ...
  private cancelWorkspaceGenerateText(rawParams: unknown) {
    const params = parseParams(gcodeWorkspaceCancelGenerateTextParamsSchema, rawParams);
  ...
  private ok(id: GCodeProtocolRequestId, result: unknown): GCodeProtocolResponse {
    return { id, result };
  }

  private fail(
    id: GCodeProtocolRequestId,
    code: number,
    message: string,
    data?: unknown,
  ): GCodeProtocolError {
  ...
  private requestClient<T>(
    method: GCodeProtocolMethod,
    params: unknown,
    resultSchema: ParamsSchema<T>,
    options?: GCodeProtocolClientRequestOptions,
  ): Promise<T> {
    if (this.clientDisconnectError) {
      throw this.clientDisconnectError;
    }
    if (!this.messageSink) {
      throw new ProtocolRequestError(-32020, `No GCode Protocol client is attached for ${method}`);
    }
  ...
  private resolveClientRequest(id: GCodeProtocolRequestId, result: unknown): void {
    const key = String(id);
  ...
  private rejectClientRequest(id: GCodeProtocolRequestId, error: Error): void {
    const key = String(id);
    const pending = this.pendingClientRequests.get(key);
    if (!pending) {
      return;
