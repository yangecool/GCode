import type { GCodeMessagePart, GCodeMessageWithParts, GCodeToolState } from "@gcode/shared";
import type { MessagePart, MessageWithParts, ToolState } from "@gcode/contracts";
import { shouldHideInvalidToolCallFromProduct } from "../tool-call-product-visibility.js";

export function mapMessageWithParts(message: MessageWithParts): GCodeMessageWithParts {
  return {
    info:
...
function mapMessagePart(part: MessagePart): GCodeMessagePart {
  const base = {
...
function mapToolState(state: ToolState): GCodeToolState {
  switch (state.status) {
    case "pending":
...
function mapCompletedToolStateMetadata(
  ...
  // GCode protocol 的 completed tool state 要求 metadata 必须是 object。
  return visibleMetadata;
}
