...
import {
  ...
  gcodeUserInputResponseSchema,
  type GCodePermissionOption,
  type GCodePermissionResponse,
  type GCodeUserInputQuestion,
...
import type { GCodeProtocolAgentServerContext } from "./server-types.js";
import {
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        gcodeProtocolMethods.interactionRequestPermission,
        {
          input: request.input,
    ...
    // v4 answer → GCodePermissionResponse：optionId 语义来自 v4 reducer 合成的
...
function v4AnswerToPermissionResponse(
  ...
  permissionOptions: GCodePermissionOption[],
  toolName: string,
  ...
  if (exact) {
    ...
    // option.response——wire 上 gcodePermissionUpdateSchema 是 strict，旧桌面多一个字段就丢事件。
...
async function requestUserInput(
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        gcodeProtocolMethods.interactionRequestUserInput,
        {
          input: request.input,
...
function v4AnswerToUserInputResponse(answer: V4InteractionAnswer): GCodeUserInputResponse {
...
function v4AnswerToPlanApprovalResponse(answer: V4InteractionAnswer): GCodeUserInputResponse {
...
function createExitPlanModeApprovalQuestion(): GCodeUserInputQuestion {
  return {
    header: "Plan",
    options: [
      {
...
function mapAskUserQuestion(question: AskUserQuestion): GCodeUserInputQuestion {
  return {
...
async function readPersistedAutoResolution(
  ...
  try {
    ...
    context.logger?.warn("Failed to restore user input auto-resolution state", {
      ...
      event: "gcode_protocol.user_input_auto_resolution_restore_failed",
      interactionId: request.requestId,
