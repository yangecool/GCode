...
import type { McpTelemetryTracker } from "@gcode/adapters";
import type { WorkspaceHookPolicyProvider } from "@gcode/core";
import type { AccountProviderConfigSnapshot } from "@gcode/provider";
import {
  gcodeProtocolErrorCodes,
  type GCodeDeliveryKind,
  type ModelSelection,
  type GCodeModelContextBudgetStrategy,
  type GCodeProtocolMessage,
  type GCodeProtocolMethod,
  type GCodeProtocolNotification,
  type GCodeProtocolRequest,
  type GCodeProtocolRequestId,
  type GCodeProtocolTrace,
  type GCodeSessionMode,
  type GCodeSessionPersistence,
  type GCodeWorkspaceRef,
} from "@gcode/shared";
import type { GCodeApp, GCodeAppOptions } from "../app/types.js";
import type { V4InteractionRegistry } from "../gcode-protocol-v4/interaction-registry.js";
import type { ConversationV4Gateway } from "../gcode-protocol-v4/v4-gateway.js";
...
export interface GCodeProtocolAgentDependencies {
  createGCodeApp(options?: Omit<GCodeAppOptions, "providerRegistry">): GCodeApp | Promise<GCodeApp>;
  /**
...
export type GCodeProtocolAgentResolvedDependencies = GCodeProtocolAgentDependencies & {
  createSessionEventStore(sessionId: string): SessionEventStorePort;
  workspaceHookPolicyProvider: WorkspaceHookPolicyProvider;
};

export interface GCodeProtocolEventSequenceState {
  lastSeq: number;
  seqBySourceEventKey: Map<string, number>;
}

export interface GCodeProtocolToolInputTransmissionState {
  streamedToolCallIdsWithInput: Set<string>;
}

export interface GCodeProtocolSessionRecord {
  app: GCodeApp;
  ...
  protocolEventSequences: Map<string, GCodeProtocolEventSequenceState>;
  protocolToolInputTransmissions: Map<string, GCodeProtocolToolInputTransmissionState>;
...
export interface GCodeProtocolClientRequestOptions {
  signal?: AbortSignal;
  timeoutMs?: number;
...
export interface GCodeProtocolAgentServerContext {
  /** 进程退出期间，迟到的异步物化不能重新登记 session。 */
  assertServing?: () => void;
  deps: GCodeProtocolAgentResolvedDependencies;
  logger?: Logger;
  ...
  notify(notification: GCodeProtocolNotification): void;
  requestClient<T>(
...
export function isRequest(message: GCodeProtocolMessage): message is GCodeProtocolRequest {
  return "method" in message && "id" in message;
}

export function isNotification(
  message: GCodeProtocolMessage,
): message is GCodeProtocolNotification {
  return "method" in message && !("id" in message);
}

export function isResponse(
  message: GCodeProtocolMessage,
): message is { id: GCodeProtocolRequestId; result: unknown } {
  return "id" in message && "result" in message;
}

export function isErrorResponse(message: GCodeProtocolMessage): message is {
...
export function protocolTraceFromTraceContext(context: TraceContext): GCodeProtocolTrace {
  return {
...
export function requireSession(
  ...
  sessionId: string,
  options: { deliveryKind?: GCodeDeliveryKind; operation?: string } = {},
  ...
  if (!record) {
    ...
    context.logger?.warn("GCode Protocol session runtime missing", {
      activeSessionCount: context.sessions.size,
      event: "gcode_protocol.session.require_missing",
...
export function createProtocolLogger(deps: GCodeProtocolAgentDependencies): Logger | undefined {
  return deps.loggerFactory?.createLogger("gcode").child({
    module: "bootstrap.gcode_protocol",
  });
}
