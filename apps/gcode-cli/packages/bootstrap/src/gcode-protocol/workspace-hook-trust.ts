...
import { workspaceHookPolicySchema } from "@gcode/contracts";
import type { WorkspaceHookPolicyProvider } from "@gcode/core";
import {
  gcodeWorkspaceHookTrustGrantParamsSchema,
  gcodeWorkspaceHookTrustGrantReasonCodeSchema,
  gcodeWorkspaceHookTrustGrantResultSchema,
  type GCodeWorkspaceHookTrustGrantReasonCode,
  type GCodeWorkspaceHookTrustGrantResult,
} from "@gcode/shared";
import {
...
export async function grantWorkspaceHookTrustForProtocol(
  ...
  const params = gcodeWorkspaceHookTrustGrantParamsSchema.parse(rawParams);
  ...
  if (policyRejection) {
    return gcodeWorkspaceHookTrustGrantResultSchema.parse({
      accepted: false,
...
): GCodeWorkspaceHookTrustGrantReasonCode | undefined {
  try {
...
function toPublicReasonCode(error: unknown): GCodeWorkspaceHookTrustGrantReasonCode {
  const candidate = error instanceof Error ? error.message : error;
  const parsed = gcodeWorkspaceHookTrustGrantReasonCodeSchema.safeParse(candidate);
