...
import {
  ...
  gcodeProviderRuntimeHeadersResponseSchema,
  type GCodeWorkspaceRef,
} from "@gcode/shared";
import type { GCodeAppOptions } from "../app/types.js";
import {
...
): NonNullable<GCodeAppOptions["providerRuntimeHeadersPort"]> {
  return {
    ...
    async refreshBeforeModelRequest(input) {
      ...
      try {
        result = await context.requestClient(
          gcodeProtocolMethods.interactionRequestProviderRuntimeHeaders,
          {
        ...
        if (input.abortSignal?.aborted || timedOut) {
          context.notify({
            method: gcodeProtocolNotifications.providerRuntimeHeadersCancelled,
            trace: protocolTraceFromTraceContext(input.traceContext),
      ...
      // gcode-plan 的账号鉴权材料是每次请求刷新的运行时配置，
