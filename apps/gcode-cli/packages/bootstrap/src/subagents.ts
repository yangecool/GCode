...
import { migrateUserSubagentMarkdown, migrateSubagentStateFile } from "@gcode/shared/node";
import {
  parseAgentProfileFromMarkdown,
  type AgentProfile,
  type AgentProfileParseDiagnostic,
} from "@gcode/core";
import type { Logger, PluginMetadata } from "@gcode/contracts";
import {
  createAgentStateId,
...
interface LoadGCodeAgentProfilesInput {
  logger?: Logger;
  storageRoot: string;
  workingDirectory: string;
}

interface LoadGCodeAgentProfilesResult {
...
export async function loadGCodeAgentProfiles(
  input: LoadGCodeAgentProfilesInput,
): Promise<LoadGCodeAgentProfilesResult> {
  ...
  const roots = [
    ...
    { path: join(input.workingDirectory, ".gcode", "agents"), source: "project" as const },
  ];
...
function sanitizeProjectAgentProfile(profile: AgentProfile): AgentProfile {
  ...
  // 项目级 .gcode/agents/*.md 是仓库内容，不能通过 frontmatter
