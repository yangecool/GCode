...
import { createNodeHttpClientAdapter } from "@gcode/adapters/http";
import type { EnvRecord } from "@gcode/adapters/model";
import { buildGCodeEndpointUrls, resolveRuntimeGCodeEndpointOrigin } from "@gcode/shared";
import {
...
export interface LoginGCodeCliOptions {
  providerId?: CodingPlanProviderId;
  abortSignal?: AbortSignal;
  apiKeyResolver?: ReturnType<typeof createCodingPlanApiKeyResolver>;
  baseUrl?: string;
  credentialStore?: SharedGCodeCredentialStore;
  env?: EnvRecord;
...
export interface LoginGCodeCliResult {
...
export type LoginBigmodelCodingPlanOptions = Omit<LoginGCodeCliOptions, "providerId">;
export type LoginBigmodelCodingPlanResult = LoginGCodeCliResult & { providerId: "bigmodel" };
...
export interface LogoutGCodeCliOptions {
  credentialStore?: SharedGCodeCredentialStore;
  env?: EnvRecord;
}

export interface LogoutGCodeCliResult {
  credentialsPath: string;
}
...
export async function hasConfiguredStandaloneCodingPlan(
  ...
  const credentialStore =
    options.credentialStore ?? createSharedGCodeCredentialStore({ env: options.env });
  return hasStandaloneCodingPlanAccess(credentialStore, options.env ?? process.env);
}

export class GCodeCliLoginError extends Error {
  readonly code:
    | "auth_failed"
    | "auth_timeout"
  ...
  constructor(
    code: GCodeCliLoginError["code"],
    message: string,
    options: { cause?: unknown } = {},
  ) {
    super(message, options);
    this.name = "GCodeCliLoginError";
    this.code = code;
  }
}

export async function loginGCodeCli(
  options: LoginGCodeCliOptions = {},
): Promise<LoginGCodeCliResult> {
  ...
  const timeoutError = () =>
    new GCodeCliLoginError("auth_timeout", "Authorization timed out. Please retry login.");
  ...
  try {
    ...
    const credentialStore = options.credentialStore ?? createSharedGCodeCredentialStore({ env });
    ...
    const readyData = await pollUntilReady({
      ...
      createError: (code) =>
        code === "auth_timeout"
          ? timeoutError()
          : new GCodeCliLoginError(code, "Authorization failed. Please retry login."),
    });
    ...
    try {
      if (providerId === "zai") {
        ...
        await credentialStore.saveMany({
          [SHARED_GCODE_CREDENTIAL_KEYS.activeProvider]: providerId,
          [SHARED_GCODE_CREDENTIAL_KEYS.gcodeJwtToken]: readyData.token,
          [SHARED_GCODE_CREDENTIAL_KEYS.bigmodelAccessToken]: readyData.accessToken,
          ...(readyData.refreshToken
            ? { [SHARED_GCODE_CREDENTIAL_KEYS.bigmodelRefreshToken]: readyData.refreshToken }
            : {}),
          [SHARED_GCODE_CREDENTIAL_KEYS.bigmodelUserInfo]: JSON.stringify({
    ...
    try {
      ...
      throw new GCodeCliLoginError(
        "config_update_failed",
        "Login succeeded but updating GCode config failed.",
        { cause: error },
      );
    }
    return {
...
export async function loginBigmodelCodingPlan(
  ...
  return {
    ...(await loginGCodeCli({ ...options, providerId: "bigmodel" })),
    providerId: "bigmodel",
  };
}
...
export async function configureCodingPlanApiKey(
  ...
  if (!apiKey) {
    throw new GCodeCliLoginError("config_update_failed", "API key must not be empty.");
  }
  const credentialStore =
    options.credentialStore ?? createSharedGCodeCredentialStore({ env: options.env });
...
export async function logoutGCodeCli(
  options: LogoutGCodeCliOptions = {},
): Promise<LogoutGCodeCliResult> {
  const credentialStore =
    options.credentialStore ?? createSharedGCodeCredentialStore({ env: options.env });
  ...
  const keys = [
    ...Object.values(SHARED_GCODE_CREDENTIAL_KEYS),
    ...identityKeys,
    ...dynamicApiKeyKeys,
  ];
...
async function persistStandaloneCodingPlanConnection(input: {
  ...
  const path =
    ...
    input.env[GCODE_PERSONAL_PROVIDER_CONFIG_FILE_ENV]?.trim() ??
...
function createOAuthClient(options: LoginGCodeCliOptions, env: EnvRecord): CliOAuthClient {
  return createCliOAuthClient({
    baseUrl:
      options.baseUrl ?? buildGCodeEndpointUrls(resolveCliGCodeEndpointOrigin(env)).apiBaseUrl,
    providerId: options.providerId ?? "zai",
    httpClient: options.httpClient ?? createDefaultHttpClient(env),
  });
}

function resolveCliGCodeEndpointOrigin(env: EnvRecord): string {
  return resolveRuntimeGCodeEndpointOrigin(env);
}
