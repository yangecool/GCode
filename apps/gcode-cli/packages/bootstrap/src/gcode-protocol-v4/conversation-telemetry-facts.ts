...
import { getModelUsageTotalTokens, SessionEventType } from "@gcode/contracts";
import { parseAutomationRunId } from "@gcode/shared";
...
export function streamingParentToolCallId(payload: Record<string, unknown>): string | undefined {
  ...
  const gcode = recordValue(meta.gcode);
  return (
    ...
    optionalString(gcode.parentToolCallId) ??
    optionalString(gcode.parentToolUseId)
  );
}
