...
import { verdictWorkspaceHookReviewRequest } from "@gcode/shared/workspace-hook-review-monotonicity";
import {
  extractPlanStepsFromToolInput,
  extractPlanStepsFromToolOutput,
  isGCodeModelRetryRecoveryProgressPayload,
  isGCodeFileStreamingToolInputPreviewTool,
  parseGCodeBackgroundTaskNotificationText,
  resolveGCodeBackgroundTaskControlKind,
...
export class ProductProjection {
  ...
  private applyBackgroundTaskNotification(fact: CanonicalUserIntentFact): ConversationDelta[] {
    const parsed = parseGCodeBackgroundTaskNotificationText(fact.input);
    if (!parsed) return [];
    const row = this.findToolRow(parsed.toolUseId);
    if (!row) return [];

    const notificationStatus = gcodeBackgroundTaskNotificationToolUpdateStatus(
  ...
  private openToolRow(
    ...
    if (isGCodeFileStreamingToolInputPreviewTool(row.toolName)) {
  ...
  private appendStreamingToolInput(
    ...
    if (
      state.lastPublishedAt !== null &&
      now - state.lastPublishedAt < GCODE_FILE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS
    ) {
      return [];
    }
  ...
  private onBackgroundTaskLifecycle(event: SessionEvent): ConversationDelta[] {
    ...
    const legacyKind = resolveGCodeBackgroundTaskControlKind(payload);
  ...
  //
  // 归约本体在 @gcode/shared 的 workflow-runs-reducer（与状态 schema 同居）：TUI 镜像要用
  // 同一份归约，两处各写一份就是两个时钟。
