...
import {
  ...
  type GCodeProcessResourceSample,
} from "@gcode/shared";

/** 采样周期与 app 侧聚合共用 shared 的同一个常量，避免两侧节拍各自漂移。 */
const GCODE_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS = GCODE_CLI_RESOURCE_SAMPLE_INTERVAL_MS;
...
interface CreateGCodeProcessResourceSamplerOptions {
  /**
   * 第二个参数是本周期完整的内存快照，供进程内本地诊断日志使用；
   * 协议样本自身只带 rss 与 heapUsed 两项内存字段。
   */
  onSample(sample: GCodeProcessResourceSample, memoryUsage: ProcessMemoryUsageSnapshot): void;
  platform?: GCodeProcessResourceSample["platform"];
  arch?: GCodeProcessResourceSample["arch"];
...
export interface GCodeProcessResourceSampler {
  start(): void;
  stop(): void;
}
...
export function createGCodeProcessResourceSampler(
  options: CreateGCodeProcessResourceSamplerOptions,
): GCodeProcessResourceSampler {
  const platform = options.platform ?? (process.platform as GCodeProcessResourceSample["platform"]);
  const arch = options.arch ?? (process.arch as GCodeProcessResourceSample["arch"]);
  ...
  const sample = (): void => {
    try {
      ...
      baseline = nextBaseline;
      const resourceSample: GCodeProcessResourceSample = {
        platform,
        arch,
        logicalCpuCount,
        intervalMs,
  ...
  return {
    start() {
      ...
      try {
        timerHandle = timer.setInterval(sample, GCODE_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS);
      } catch {
