...
import { GCODE_OFFICIAL_PLUGIN_MARKETPLACE, isOfficialMarketplaceId } from "@gcode/contracts";
import { GCODE_CUA_OFFICIAL_PLUGIN_ID, isGCodeCuaInternalFeatureEnabled } from "@gcode/shared";
...
export interface ResolveGCodePluginsOptions {
...
export interface ListGCodePluginsOptions extends ResolveGCodePluginsOptions {}

export interface SetGCodePluginEnabledOptions extends ResolveGCodePluginsOptions {
  enabled: boolean;
  plugin: string;
  scope?: "user" | "workspace";
}

export interface SetGCodePluginEnabledResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface GCodeMarketplaceSummaryData {
  id: string;
  name: string;
...
export interface GCodeAvailablePluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface GCodeInstalledPluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface GCodePluginsOverviewData {
  marketplaces: GCodeMarketplaceSummaryData[];
  availablePlugins: GCodeAvailablePluginData[];
  installedPlugins: GCodeInstalledPluginData[];
  restorableBuiltins: GCodeAvailablePluginData[];
  diagnostics: PluginLoadOutcome["diagnostics"];
}

export interface GCodeMarketplaceUpdateData {
  diagnostics: PluginLoadOutcome["diagnostics"];
  marketplaces: GCodeMarketplaceSummaryData[];
}

export interface AddGCodeMarketplaceOptions extends ResolveGCodePluginsOptions {
...
export interface RemoveGCodeMarketplaceOptions extends ResolveGCodePluginsOptions {
  marketplace: string;
}

export interface UpdateGCodeMarketplaceOptions extends ResolveGCodePluginsOptions {
  abortSignal?: AbortSignal;
  marketplace?: string;
}

export interface InstallGCodeMarketplacePluginOptions extends ResolveGCodePluginsOptions {
  abortSignal?: AbortSignal;
  dryRun?: boolean;
  marketplace: string;
  pluginName: string;
...
export interface UninstallGCodeMarketplacePluginOptions extends ResolveGCodePluginsOptions {
  pluginId?: string;
  pluginName?: string;
  marketplace?: string;
  removeCache?: boolean;
  /** 保留 data/<plugin-id> 用户数据目录（`gcode plugins uninstall --keep-data`）。 */
  keepData?: boolean;
}

export interface UpdateGCodeMarketplacePluginOptions extends ResolveGCodePluginsOptions {
  abortSignal?: AbortSignal;
  pluginId: string;
}

export interface ValidateGCodePluginPathOptions extends ResolveGCodePluginsOptions {
  abortSignal?: AbortSignal;
  path: string;
}

export interface GCodePluginUpdateData extends GCodePluginInstallData {
  previousVersion: string;
}

interface RestoreBuiltinPluginOptions extends ResolveGCodePluginsOptions {
  pluginId: string;
}

interface ConfigureGCodePluginOptions extends ResolveGCodePluginsOptions {
...
interface ResetGCodePluginConfigOptions extends ResolveGCodePluginsOptions {
  pluginId: string;
  scope?: "user" | "workspace";
}

interface ValidateGCodePluginOptions extends ResolveGCodePluginsOptions {
  marketplace?: string;
  pluginName?: string;
  source?: string;
}

interface DescribeGCodePluginOptions extends ResolveGCodePluginsOptions {
  marketplace: string;
  pluginName: string;
}

export interface GCodePluginInstallData {
...
function countVisibleMarketplacePlugins(
  ...
  if (marketplaceId !== GCODE_OFFICIAL_PLUGIN_MARKETPLACE) return plugins.length;
  return plugins.filter((entry) => entry.name !== OFFICIAL_NODE_REPL_HOST_PLUGIN_NAME).length;
}

export function resolveGCodePlugins(options: ResolveGCodePluginsOptions = {}): PluginLoadOutcome {
...
export function getGCodePluginsOverview(
  options: ResolveGCodePluginsOptions = {},
  ...
  const catalogs: Array<{
    summary: GCodeMarketplaceSummaryData;
  ...
  const restorableBuiltins: GCodeAvailablePluginData[] = OFFICIAL_PLUGIN_DEFINITIONS.filter(
    (def) =>
      suppressed.has(`${def.name}@${GCODE_OFFICIAL_PLUGIN_MARKETPLACE}`) &&
      // computer-use 的恢复入口需要 internal 特性开启（与 restoreBuiltinPluginCore 同口径）。
      (def.name !== "computer-use" || isGCodeCuaInternalFeatureEnabled(options.env ?? process.env)),
  ).map((def) => {
    const listing = def.listing
      ? parseEntryStoreListing({ name: def.name, ...def.listing })
      : undefined;
    return {
      id: `${def.name}@${GCODE_OFFICIAL_PLUGIN_MARKETPLACE}`,
      name: def.name,
...
export function listGCodePlugins(options: ListGCodePluginsOptions = {}): PluginLoadOutcome {
  const outcome = resolveGCodePlugins(options);
...
function loadPluginListingsById(storageRoot: string): Record<string, PluginStoreListing> {
  ...
  for (const definition of OFFICIAL_PLUGIN_DEFINITIONS) {
    ...
    if (listing) {
      listings.set(`${definition.name}@${GCODE_OFFICIAL_PLUGIN_MARKETPLACE}`, listing);
    }
  }
...
export async function setGCodePluginEnabled(
  options: SetGCodePluginEnabledOptions,
): Promise<SetGCodePluginEnabledResult> {
...
export async function addGCodePluginMarketplace(
  options: AddGCodeMarketplaceOptions,
): Promise<GCodeMarketplaceSummaryData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function removeGCodePluginMarketplace(
  options: RemoveGCodeMarketplaceOptions,
): Promise<void> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function updateGCodePluginMarketplace(
  options: UpdateGCodeMarketplaceOptions,
): Promise<GCodeMarketplaceUpdateData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
...
export async function installGCodeMarketplacePlugin(
  options: InstallGCodeMarketplacePluginOptions,
): Promise<GCodePluginInstallData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.dryRun === true) {
  ...
  const isSuppressedBundledOfficial =
    options.marketplace === GCODE_OFFICIAL_PLUGIN_MARKETPLACE &&
  ...
  if (options.marketplace === GCODE_OFFICIAL_PLUGIN_MARKETPLACE) {
...
export async function uninstallGCodeMarketplacePlugin(
  options: UninstallGCodeMarketplacePluginOptions,
): Promise<GCodeInstalledPluginData | null> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
...
/**
 * `gcode plugins update <plugin>`：先刷新所属 marketplace 目录，再按同一条目重装。
 * cacheMarketplacePlugin 对已存在的安装记录做原地覆盖并保留 installedAt；启用态只会给
 * 用户配置里尚未显式声明的 id 补默认值，因此更新不会改变用户已经做过的开关选择。
 */
export async function updateGCodeMarketplacePlugin(
  options: UpdateGCodeMarketplacePluginOptions,
): Promise<GCodePluginUpdateData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ...
  const refreshed = await updateGCodePluginMarketplace({
    ...options,
  ...
  const installed = await installGCodeMarketplacePlugin({
    ...options,
    marketplace: record.marketplace,
...
/** `gcode plugins validate <path>`：只读校验本地插件目录或 marketplace 目录。 */
export async function validateGCodePluginPath(
  options: ValidateGCodePluginPathOptions,
...
async function restoreBuiltinPluginCore(options: RestoreBuiltinPluginOptions): Promise<void> {
  const gcodeCuaPluginId = GCODE_CUA_OFFICIAL_PLUGIN_ID;
  if (
    options.pluginId === gcodeCuaPluginId &&
    !isGCodeCuaInternalFeatureEnabled(options.env ?? process.env)
  ) {
    // overview 虽然隐藏了恢复入口，但协议调用仍可绕过 UI 写用户配置。
    // 功能开关关闭时在写盘前失败，确保用户配置与插件缓存都保持零痕迹。
    throw new Error("computer-use built-in plugin requires GCODE_CUA_PRODUCT_HELPER to be enabled");
  }
...
export async function configureGCodePlugin(options: ConfigureGCodePluginOptions): Promise<void> {
...
export async function resetGCodePluginConfig(
  options: ResetGCodePluginConfigOptions,
...
export async function validateGCodePlugin(
  options: ValidateGCodePluginOptions,
): Promise<PluginLoadOutcome["diagnostics"]> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.source) {
...
export async function describeGCodePlugin(
  options: DescribeGCodePluginOptions,
...
function resolvePluginContext(options: ResolveGCodePluginsOptions): {
  configResult: ConfigResult;
  pluginStorageRoot: string;
  workingDirectory: string;
} {
...
function resolvePluginIdForMutation(options: UninstallGCodeMarketplacePluginOptions): string {
...
function resolvePluginConfigPath(
  options: ResolveGCodePluginsOptions & { scope?: "user" | "workspace" },
  configResult: ConfigResult,
  workingDirectory: string,
): string {
  if (options.scope !== "workspace") {
  ...
  // Workspace Plugin 配置固定落在当前 `<workspace>/.gcode/config.json`。嵌套 workspace
  // 可能同时发现仓库根与自身的配置，读取端 innermost 优先；写入端也必须锁定当前
  // workspace，不能用 project discovery 的第一个 outermost 文件。
  const workspaceConfigPath = join(workingDirectory, ".gcode", "config.json");
