import { updateUiLocaleInFileConfig, type ConfigResult } from "@gcode/adapters/config";
import type { AgentRuntime } from "@gcode/core";
import { resolveLocale } from "@gcode/i18n";
import { normalizeModelSelection, type ModelSelection } from "@gcode/provider";
import {
...
import type { PrepareUserExecutionBoundary, GCodeApp } from "./types.js";
...
export function createSessionFacade(deps: CreateSessionFacadeDeps): SessionFacade {
  ...
  return {
    ...
    setTarget: async (input) =>
      (await setTargetStatus("set", input)) as Awaited<ReturnType<GCodeApp["setTarget"]>>,
    updateTargetStatus: async (status) =>
      (await setTargetStatus("status_updated", { status })) as Awaited<
        ReturnType<GCodeApp["updateTargetStatus"]>
      >,
