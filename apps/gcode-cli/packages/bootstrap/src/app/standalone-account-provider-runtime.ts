...
import type { SharedGCodeCredentialStore } from "@gcode/adapters/auth";
import type { ProviderRuntimeHeadersPort } from "@gcode/core";
import {
...
async function readStandaloneCodingPlanCatalog(
  ...
  if (config)
    return {
      gcodeBuiltinRevision: config.revision,
  ...
  const filePath = env[GCODE_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  if (!filePath) {
    throw new Error(`${GCODE_BUILTIN_PROVIDER_CONFIG_FILE_ENV} is required for login`);
  }
...
export async function resolveStandaloneCodingPlanProvider(
  ...
  if (matches.length !== 1) {
    throw new Error(
      `GCode Built-in Config 必须为 ${family} 声明唯一 Individual Coding Plan Provider`,
    );
  }
  return matches[0]!;
}
...
export async function readStandaloneAccountProviderConfigSnapshot(
  credentialStore: Pick<SharedGCodeCredentialStore, "loadMany">,
  env: Readonly<Record<string, string | undefined>>,
  config?: Pick<ProviderConfigLayerSnapshot, "revision" | "providers">,
  ...
  return createAccountProviderConfigSnapshot(catalog.gcodeBuiltinRevision, providers);
}
...
export function createStandaloneProviderRuntimeHeadersPort(
  credentialStore: Pick<SharedGCodeCredentialStore, "load" | "loadMany">,
  env: Readonly<Record<string, string | undefined>>,
