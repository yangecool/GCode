...
import { SqliteSessionStore } from "@gcode/adapters/storage";
import { resolvePath, type ConfigResult } from "@gcode/adapters/config";
import {
...
import type { ModelSelection } from "@gcode/provider";
...
export async function openStartupSessionStore(
  ...
  startupTimer.start("GCode SQLite migration started", {
    context: { dbPath },
  ...
  startupTimer.mark("GCode SQLite migration completed", {
    context: { dbPath },
