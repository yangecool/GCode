...
import { detectLocale, resolveLocale } from "@gcode/i18n";
import type { RuntimeConfigPatch, SupportedLocale, UiLocale } from "@gcode/contracts";
import type { GCodeAppOptions } from "./types.js";

export function isMessageEnabled(env: NodeJS.ProcessEnv): boolean {
  return env.GCODE_MESSAGE_ENABLED === "1" || env.GCODE_MESSAGE_ENABLED === "true";
}

export function createConfigCliOverrides(options: GCodeAppOptions): RuntimeConfigPatch | undefined {
...
function detectAppLocale(options: GCodeAppOptions): SupportedLocale | undefined {
