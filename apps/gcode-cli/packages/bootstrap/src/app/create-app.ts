...
} from "@gcode/adapters/storage";
import { createNodeLoggerFactory } from "@gcode/adapters/logging";
import { createConfig, resolvePath } from "@gcode/adapters/config";
import {
  createNodeExecutionAdapter,
  resolveEffectiveBashShellSelection,
} from "@gcode/adapters/exec";
import { createNodeFileSystemAdapter } from "@gcode/adapters/fs";
import { createNodeWebFetchHttpClientAdapter } from "@gcode/adapters/http";
import { createJimpImageProcessorAdapter } from "@gcode/adapters/image";
import { createPopplerPdfDocumentAdapter } from "@gcode/adapters/pdf";
import { createNodeSessionMailboxAdapter } from "@gcode/adapters/mailbox";
import { createNodeContextSourceAdapter } from "@gcode/adapters/context";
import { createNodeSkillAdapter } from "@gcode/adapters/skills";
import { createMcpAdapter } from "@gcode/adapters/mcp";
import {
  AgentRuntime,
  PermissionService,
...
import type { AgentRuntimeDeps } from "@gcode/core";
import { createModelTelemetry } from "@gcode/telemetry";
import {
...
import { isRemoteWorkspaceIdentity, resolveGCodeRuntimeEnv } from "@gcode/shared";
import {
...
import { resolveGCodeCustomCommandPrompt } from "../custom-command-prompt.js";
import { resolveGCodeBuiltinPromptCommand } from "../builtin-prompt-command.js";
import { collectDisabledPaths } from "../skill-command-overrides.js";
import { loadPluginAgentProfiles, loadGCodeAgentProfiles } from "../subagents.js";
...
import { buildGrokEnginePersona } from "@gcode/adapters/grok-session";
import { grokMemoryV2PersonaPaths } from "@gcode/adapters/grok-session";
import {
  completeAppStartup,
...
export async function createGCodeApp(options: GCodeAppOptions): Promise<GCodeApp> {
  if (!options?.providerRegistry) {
    throw new Error("createGCodeApp requires a Provider Registry");
  }
  ...
  const modelLogger = loggerFactory.createLogger("gcode").child({
    ...traceContextToLogContext(traceContext),
  ...
  try {
    ...
    const modelIoDir = getModelIoDir(
      cliStorageRoot,
      resolveGCodeRuntimeEnv(options.env ?? process.env) === "development",
    );
    const gcodeSubagentProfileOutcome = await loadGCodeAgentProfiles({
      logger,
      storageRoot,
      workingDirectory,
    });
    const gcodeSubagentProfiles = gcodeSubagentProfileOutcome.profiles;
    ...
    const pluginSubagentProfiles = loadPluginAgentProfiles({
      ...
      reservedProfileNames: gcodeSubagentProfiles.map((profile) => profile.name),
      modelSelectionOverrides: gcodeSubagentProfileOutcome.pluginAgentModelSelectionOverrides,
    }).profiles;
    ...
    const subagentProfiles = [...gcodeSubagentProfiles, ...pluginSubagentProfiles];
    ...
    let { configuredMcpServers, runtimeConfig, untrustedProjectMcpServers } =
      resolveAppRuntimeConfig({
        ...
        builtInSubagentModelSelectionOverrides:
          gcodeSubagentProfileOutcome.builtInModelSelectionOverrides,
    ...
    startupTimer.mark("GCode runtime configuration resolved", {
      ...
      ...(options.workspaceHookReviewHost
        ? {
            emitReviewEvent: async (event) => {
              if (!runtime) throw new Error("GCode runtime is not initialized yet.");
    ...
    const sessionMailboxPort =
      ...
      (messageEnabled
        ? createNodeSessionMailboxAdapter({
            rootDir: resolvePath(
              (options.env ?? process.env).GCODE_MAILBOX_ROOT ?? "~/.gcode/mailbox",
            ),
          })
        : undefined);
    markStorageAdaptersInitialized({
    ...
    const inputFacade = createInputFacade({
      ...
      customCommandPromptResolver: async (text, resolverOptions) => {
        const builtinPrompt = resolveGCodeBuiltinPromptCommand(text, {
          workingDirectory,
        });
        if (builtinPrompt !== undefined) {
          return builtinPrompt;
        }
        return await resolveGCodeCustomCommandPrompt(text, {
      ...
      if (input.messageId && input.attachmentIndex !== undefined) {
        ...
        if (persistedAttachment?.type === "file") {
          ...
          artifactUri =
            ...
            (persistedAttachment.url.startsWith("gcode-artifact://")
    ...
    return {
      ...
      readPromptAttachment: async (input) => {
        ...
        if (ref.startsWith("gcode-artifact://")) {
      ...
      statPromptAttachment: async (input) => {
        ...
        if (artifactUri) {
          if (!artifactStore.statToolResultArtifact) {
            throw new GCodeAttachmentFaultError(GCODE_ATTACHMENT_FAULT_CODES.statUnsupported);
          }
        ...
        if (result.kind !== "file") {
          ...
          throw new GCodeAttachmentFaultError(GCODE_ATTACHMENT_FAULT_CODES.statNotFile);
        }
        return {
    ...
    startupTimer.fail("GCode app startup failed", error, {
