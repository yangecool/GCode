...
import {
  ...
  CoreErrorType,
  GCODE_DWF_CHILD_COMMAND,
  type CreateSessionTaskLinkInput,
  type DynamicWorkflowRunEvent,
  type DynamicWorkflowRunProgressPayload,
  type SessionId,
} from "@gcode/contracts";
import type { AgentRuntime } from "@gcode/core";
import { parseModelPickerValue, type ModelSelection } from "@gcode/shared/model-selection";
import {
...
export function launchDynamicWorkflowRun(
  ...
  return runWorkflowScript({
    ...
    makeDriver,
    // 入口文件写不进项目 `.gcode/` 时 harness 回落到 OS 临时目录并报一声——run 照常启动，
 ...
 * `@gcode/dynamic-workflow` 与 node 内建），既拿不到 contracts 的子命令常量，也不该知道
...
export function dynamicWorkflowChildSpawn(
  ...
  return isSea ? { argsPrefix: [GCODE_DWF_CHILD_COMMAND] } : undefined;
}
 ...
 * `gcode-protocol-v4/product-projection.ts` 的 `applyWorkflowRunEvent` 逐种类归约，其
