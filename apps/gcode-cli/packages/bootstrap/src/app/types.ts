import type { GCodeToolExecResource, BackgroundBashOutputResult } from "@gcode/shared";
import type { AiSdkModelAdapter } from "@gcode/adapters/model";
import type {
  AgentRuntime,
  AgentRuntimeConfig,
  ExecuteTurnOptions,
...
import type { GCodeModelOption } from "@gcode/shared";
import type { EffectiveModelSelectionResult } from "@gcode/shared/model-selection";
export type { GCodeModelOption } from "@gcode/shared";
import type { ModelProviderSourceTitle } from "../model-config.js";
import type { GCodeInstalledPluginData } from "../plugins.js";
import type {
  AutomationPort,
  OffPeakPort,
...
import type { AgentTelemetryRuntimeOwner, WorkspaceHookPolicy } from "@gcode/contracts";
...
export type GCodeAppRuntimeConfigInput = AgentRuntimeConfig;

export interface GCodeAppOptions {
  sessionId?: SessionId;
  resume?: boolean;
  version?: string;
  traceContext?: TraceContext;
  runtimeConfig?: GCodeAppRuntimeConfigInput;
  /**
  ...
  onToolExecResource?: (sample: GCodeToolExecResource) => void;
...
export type SubmitPromptOptions = SubmitPromptOptionsBase &
  import("@gcode/contracts").TurnBackgroundAttribution;
...
export interface GCodePluginSetResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface GCodePluginUninstallResult {
  // null 表示该 plugin id 当前未安装（幂等 no-op），调用方据此提示"未安装"。
  removed: GCodeInstalledPluginData | null;
}
...
export interface GCodeApp {
  ...
  getCurrentModelOption?(): GCodeModelOption | undefined;
  /** 只读 Registry 元数据；不要求选项完整，也不绑定执行模型。 */
  getModelOption?(selection: ModelSelection): GCodeModelOption | undefined;
  ...
  readSubagents(input?: {
    endedCursor?: string;
    endedLimit?: number;
  }): Promise<import("@gcode/shared").GCodeSessionSubagentsResult>;
  ...
  listModels(): GCodeModelOption[];
  listThoughtLevels(): string[];
  listPlugins(): Promise<PluginLoadOutcome>;
  setPluginEnabled(plugin: string, enabled: boolean): Promise<GCodePluginSetResult>;
  uninstallPlugin(plugin: string): Promise<GCodePluginUninstallResult>;
  /**
...
export interface RunGCodeProtocolAgentOptions {
...
export interface ListGCodeSessionsOptions {
  directory?: string;
  env?: NodeJS.ProcessEnv;
  limit?: number;
  sessionStore?: SessionStorePort;
