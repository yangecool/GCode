...
import { createNodeModelSelectionFacade } from "@gcode/provider-node";
import { createNodeLoggerFactory } from "@gcode/adapters/logging";
import {
  createMcpAdapterConnectionPool,
  createMcpTelemetryTracker,
  type McpConnectionPool,
  type McpTelemetryTracker,
} from "@gcode/adapters/mcp";
import {
  gcodeProtocolNotifications,
  type GCodeMcpResourceSample,
  type GCodeMcpTelemetryEvent,
} from "@gcode/shared";
import type { SqliteSessionStore } from "@gcode/adapters/storage";
import { traceContextToLogContext, createRootTraceContext } from "@gcode/contracts";
import type { McpPort, ModelSelection } from "@gcode/contracts";
import type { PresentationSurface } from "@gcode/core";
import type { RunGCodeProtocolAgentOptions, GCodeAppOptions } from "./app/types.js";
import { createGCodeApp } from "./app/create-app.js";
import {
...
} from "./gcode-protocol/storage-startup.js";
...
import { installGCodeProtocolAiSdkWarningLogger } from "./gcode-protocol/ai-sdk-warning-logger.js";
import {
  createOfficialMcpAuthHeadersPort,
  type OfficialMcpAuthRequestContext,
} from "./gcode-protocol/official-mcp-auth-port.js";
import {
  createOfficialMcpTrustedOriginRegistry,
  OFFICIAL_MCP_DEV_TRUSTED_ORIGINS_ENV,
  GCODE_WORKSPACE_IDENTITY_ENV,
  resolveRuntimeGCodeEndpointOrigin,
} from "@gcode/shared";
import { GCodeProtocolAgentServer } from "./gcode-protocol/server.js";
import { GCodeProtocolNdjsonConnection } from "./gcode-protocol/transport.js";
import { cleanupProtocolRuntime } from "./gcode-protocol/runtime-cleanup.js";
import { startProtocolResourceSampler } from "./gcode-protocol/resource-sampler.js";
import { acquireProtocolStartupResource } from "./gcode-protocol/startup-resource.js";
import type { GCodeProcessResourceSampler } from "./process-resource-sampler.js";
import { prepareGCodeTelemetryEnv, shutdownGCodeTelemetry } from "./telemetry-bootstrap.js";

function applyProtocolPresentationSurface(
  options: Omit<GCodeAppOptions, "providerRegistry">,
  presentationSurface: PresentationSurface,
): Omit<GCodeAppOptions, "providerRegistry"> {
  return {
    ...options,
    runtimeConfig: {
...
function applyProtocolProviderRegistry(
  options: Omit<GCodeAppOptions, "providerRegistry">,
  providerRegistry: GCodeAppOptions["providerRegistry"],
...
export async function runGCodeProtocolAgent(
  options: RunGCodeProtocolAgentOptions = {},
): Promise<void> {
  ...
  const traceContext = createRootTraceContext({
    attributes: {
      entrypoint: "gcode_protocol",
    },
  });
  const logger = loggerFactory.createLogger("gcode").child({
    ...traceContextToLogContext(traceContext),
    module: "bootstrap.gcode_protocol",
  });
  installGCodeProtocolAiSdkWarningLogger(logger);
  const startupTimer = new StartupTimer(
    logger,
    {
      ...traceContextToLogContext(traceContext),
      module: "bootstrap.gcode_protocol",
      startupKind: "gcode_protocol_agent",
    },
    startupStartedAt,
  );
  startupTimer.start("GCode Protocol agent startup started", {
    context: { version: options.version },
    event: "gcode_protocol.startup.started",
    stage: "start",
  });

  let sessionStore: SqliteSessionStore | undefined;
  let serverForCleanup: GCodeProtocolAgentServer | undefined;
  ...
  let mcpResourceSink: ((samples: GCodeMcpResourceSample[]) => void) | undefined;
  let mcpTelemetrySink: ((event: GCodeMcpTelemetryEvent) => void) | undefined;
  let processResourceSampler: GCodeProcessResourceSampler | undefined;
  ...
  try {
        ...
        openProtocolStartupStorage({
          ...
          onProgress: (progress) =>
            logger.info("SQLite startup state", {
              event: "gcode_protocol.startup.storage_state",
              ...progress,
            }),
        }),
    });
    ...
    logger.info("Worker Provider Registry 已就绪", {
      ...
      event: "gcode_protocol.provider_registry.ready",
      module: "bootstrap.gcode_protocol",
    ...
    const telemetryEnv = await acquireProtocolStartupResource({
      signal: options.lifecycle?.signal,
      logger,
      disposeLate: () => shutdownGCodeTelemetry(),
      create: () =>
        prepareGCodeTelemetryEnv(runtimeEnv, {
          cliVersion: options.version,
          productVersion: options.env?.GCODE_APP_VERSION,
          runtimeSurface,
        }),
    });
    const telemetryDeviceMid = telemetryEnv.GCODE_TELEMETRY_DEVICE_MID;
    mcpTelemetryTracker =
    ...
    // 与下面 trustedOrigins 的 resolveGCodeApiOrigin 必须是同一个表达式，否则两侧判定分叉。
    const resolveGCodeApiOrigin = (): string =>
      resolveRuntimeGCodeEndpointOrigin(options.env ?? process.env);
    const workspaceIdentity = (options.env ?? process.env)[GCODE_WORKSPACE_IDENTITY_ENV]?.trim();
    const officialMcpAuth = {
      ...
      resolveGCodeApiOrigin,
      ...(workspaceIdentity ? { workspaceIdentity } : {}),
      // 信任判定只看一条：目标 origin 等于当前 GCode API origin（https）。pluginId 不参与。
    ...
    options.lifecycle?.signal.throwIfAborted();
    const server = (serverForCleanup = new GCodeProtocolAgentServer({
      createGCodeApp: (appOptions = {}) =>
        createGCodeApp({
          ...
          env: {
            ...telemetryEnv,
            ...appOptions.env,
            ...(telemetryDeviceMid ? { GCODE_TELEMETRY_DEVICE_MID: telemetryDeviceMid } : {}),
          },
          ...
          onToolExecResource: (params) =>
            connection.send({ method: gcodeProtocolNotifications.toolExecResource, params }),
        }),
      cwd: options.cwd,
      env: options.env,
      loggerFactory,
    ...
    const connection = new GCodeProtocolNdjsonConnection({
      signal: options.lifecycle?.signal,
    ...
    mcpResourceSink = (samples) =>
      connection.send({
        method: gcodeProtocolNotifications.mcpResourceSamples,
        params: samples,
      });
    mcpTelemetrySink = (event) => {
      ...
      connection.send({
        method: gcodeProtocolNotifications.mcpTelemetry,
        params: event,
      });
    };
    connection.start();
    ...
    startupTimer.complete("GCode Protocol agent startup completed", {
      event: "gcode_protocol.startup.completed",
      stage: "total",
    });
    ...
    startupTimer.fail("GCode Protocol agent startup failed", error, {
      event: "gcode_protocol.startup.failed",
      stage: "total",
    });
    throw error;
  } finally {
    ...
    logger.info("GCode Protocol agent shutdown completed", {
      ...traceContextToLogContext(traceContext),
      event: "gcode_protocol.shutdown.completed",
      module: "bootstrap.gcode_protocol",
      status: "completed",
    });
  }
...
function resolveProtocolRuntimeSurface(
  ...
  return env.GCODE_TELEMETRY_RUNTIME_SURFACE?.trim() === "remote_workspace_host"
