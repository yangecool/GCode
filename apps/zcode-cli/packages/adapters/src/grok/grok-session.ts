 ...
 *
 * 组装原则（对齐用户裁决）：ZCode 成熟机制（四档权限、compact 机器、
...
export interface GrokEnginePersonaOverride {
  readonly kind: 'grok'
  /** 替换 ZCode cli_prefix 段的身份首段。 */
  readonly cliPrefix: string
  /** 替换 ZCode identity 段的 persona 主体。 */
...
/** 子代理身份首两段（buildGrokSubagentPrompt 的 identity 部分，ZCode cli_prefix 替换用）。 */
