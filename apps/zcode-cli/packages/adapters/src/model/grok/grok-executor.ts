/**
 * G Code — Grok ModelExecutor 核心映射（M1 第三批：接线 ZCode Model 面）。
 *
 * 定位：`executeGrokRequest`（引擎）与 ZCode `ModelExecutor` 契约之间的纯映射层。
 * 本文件只做 `import type`（剥离后无 @zcode 运行时依赖），保证 node 原生 TS
 * 剥离下可独立测试；GrokWireError → ModelProtocolError 的错误包装在 runner
 * 侧壳（grok-runner.ts）完成，那里本来就活在 ai/@zcode 的运行时世界里。
 *
 * 事件语义：GrokStreamEvent 与 ZCode ModelStreamEvent 同形（H5 双表示的
 * ZCode 侧就是流事件本身），stream 路径按原样透传；generateText 路径把流
