...
const defaultEntryPath = join(repoRoot, "apps/gcode-cli/packages/cli/dist/gcode.cjs");
...
export async function buildDesktopAgentBytecode({
  ...
  if (env.GCODE_E2E_COVERAGE === "1") throw new Error("coverage 构建不能启用字节码试验");
  const directory = dirname(entryPath);
  const temporary = join(directory, `.bytecode-${randomUUID()}`);
  const loaderPath = join(directory, "gcode.bytecode.cjs");
  try {
    ...
    const bytecodeFile = `gcode.bytecode-${metadata.bytecodeSha256}.jsc`;
    ...
    const runtimeFile = `gcode.bytecode-runtime-${runtimeHash}.cjs`;
