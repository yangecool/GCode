...
const agentEntry = join(root, "agent", "gcode.cjs");

function usage() {
  return `Usage:
  gcode --web [--host <host>] [--port <port>] [--workspace <path>] [--open|--no-open] [--token <token>|--no-token]
  gcode --version
`;
}
...
async function serve(options) {
  ...
  const child = spawn(process.execPath, [serverEntry], {
    ...
    env: {
      ...process.env,
      PORT: String(port),
      GCODE_AGENT_SERVER_ARGS_JSON: JSON.stringify([agentEntry, "app-server", "--stdio"]),
      GCODE_AGENT_SERVER_COMMAND: process.execPath,
      GCODE_SERVER_HOST: options.host,
      GCODE_SERVER_WORKSPACE: options.workspace,
      GCODE_WEB_STATIC_ROOT: webRoot,
      // 显式关闭 token 时必须清空继承值，否则 --no-token 仍会开启后端鉴权。
      GCODE_SERVER_AUTH_TOKEN: token,
    },
  ...
  console.log("");
  console.log("GCode Web is running");
...
try {
  ...
  if (argv.length === 1 && ["--version", "-v"].includes(argv[0])) {
    ...
    if (argv.length === 1 && ["--help", "-h"].includes(argv[0])) {
      console.log("Web mode: gcode --web [options] (gcode --web --help for details)\n");
    }
