...
function printUsageAndExit() {
  process.stderr.write(
    [
      "Usage: gcode-stdio-tap.mjs --workspace-key <key> [--log-dir <dir>] -- <command> [...args]",
      "",
    ].join("\n"),
  );
  process.exit(2);
...
function parseArgs(argv) {
  ...
  let workspaceKey = "";
  let logDir = process.env.GCODE_STDIO_TAP_LOG_DIR?.trim() || "";
  ...
  return {
    ...
    logDir: logDir || join(homedir(), ".gcode", "v2", "dev", "stdio-traffic"),
    workspaceKey,
  };
}
...
child.on("error", (error) => {
  ...
  process.stderr.write(`[gcode-stdio-tap] spawn failed: ${error.message}\n`);
});
