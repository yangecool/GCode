...
import {
  ...
  stageTuiRuntime,
} from "./gcode-distribution/assets.mjs";
import { installScriptSource } from "./gcode-distribution/installer.mjs";

const root = resolve(import.meta.dirname, "..");
const defaultOutDir = resolve(root, "dist", "gcode");
const defaultBaseUrl = (await loadEndpointEnv()).GCODE_DIST_BASE_URL?.trim() || "";
const packageDirName = "gcode";
const usage = `Usage:
  pnpm build:gcode
  node scripts/build-gcode.mjs --skip-build
  node scripts/build-gcode.mjs --version 3.3.3-dev.1
  node scripts/build-gcode.mjs --out-dir dist/gcode
  node scripts/build-gcode.mjs --base-url http://host/gcode/deps/gcode/
...
Options:
  ...
  --out-dir <path>    Output directory. Defaults to dist/gcode.
...
function run(command, args, options = {}) {
  console.log(`[gcode] ${commandText(command, args)}`);
...
async function buildOutputs(skipBuild) {
  if (skipBuild) {
    console.log("[gcode] skipping build; reusing existing outputs");
    return;
  }

  run("pnpm", ["--filter", "@gcode/cli...", "build"]);
  await rm(resolve(root, "packages", "server", "dist"), {
    force: true,
    recursive: true,
  });
  run("pnpm", ["--filter", "@gcode/server", "build"]);
  run("pnpm", ["--filter", "@gcode/web", "build"]);
}

async function stageGCodePackage({ packageRoot, version }) {
  const webDist = resolve(root, "packages", "web", "dist");
  const serverDist = resolve(root, "packages", "server", "dist");
  const agentBundle = resolve(root, "apps", "gcode-cli", "packages", "cli", "dist", "gcode.cjs");
  const agentProvider = resolve(root, "apps/gcode-cli/packages/cli/dist/provider");
  ...
  await assertFile(resolve(agentProvider, "gcode-builtin.json"), "Agent provider config");
  ...
  await cp(agentBundle, resolve(packageRoot, "agent", "gcode.cjs"));
  // TUI 入口通过真正的 CLI 路径定位伴随配置；只复制 JS 会在仓库外启动失败。
  await cp(agentProvider, resolve(packageRoot, "agent/provider"), { recursive: true });
  await cp(
    resolve(root, "apps/gcode-cli/packages/cli/dist/THIRD-PARTY-NOTICES.md"),
    resolve(packageRoot, "agent/THIRD-PARTY-NOTICES.md"),
  );
  await chmod(resolve(packageRoot, "agent", "gcode.cjs"), 0o755);
  ...
  const runner = resolve(packageRoot, "bin", "gcode.mjs");
  await cp(resolve(root, "scripts/gcode-distribution/runner.mjs"), runner);
...
async function main() {
  ...
  if (!options.help && !options.baseUrl)
    throw new Error("Configure GCODE_DIST_BASE_URL in .env or pass --base-url");
  if (options.help) {
    console.log(usage);
    return;
  }
  ...
  if (!version || typeof version !== "string") {
    throw new Error("Unable to resolve GCode version.");
  }
  ...
  console.log(`[gcode] release directory: ${outDir}`);
  console.log(`[gcode] tarball: ${tarball}`);
  console.log(`[gcode] sha256: ${sha256}`);
}
