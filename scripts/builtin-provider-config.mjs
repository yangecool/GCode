...
export async function resolveBuiltinProviderBuildEnvironment({
  ...
  if (!value?.trim()) {
    ...
    for (const file of files) {
      ...
      if (parsed.GCODE_ENV !== undefined) value = parsed.GCODE_ENV;
    }
  }
  const normalized = value?.trim().toLowerCase() || "test";
  if (normalized !== "test" && normalized !== "production") {
    throw new Error(`Invalid GCODE_ENV for Built-in Provider build: ${normalized}`);
  }
  return normalized;
}
...
export async function loadBuiltinProviderConfig({ root = repositoryRoot, env = process.env } = {}) {
  ...
  const sourcePath = resolve(
    root,
    env.GCODE_BUILTIN_PROVIDER_CONFIG_FILE?.trim() || "config/provider/gcode-builtin.json",
  );
  try {
    ...
    const { decodeGCodeBuiltinRelease } = await tsImport(
      pathToFileURL(resolve(repositoryRoot, "packages/provider-node/src/gcode-builtin-release.ts"))
        .href,
      import.meta.url,
    );
    decodeGCodeBuiltinRelease(JSON.parse(content));
...
export async function stageBuiltinProviderConfig({ directory, ...options }) {
  ...
  await writeFile(resolve(directory, "gcode-builtin.json"), config.content, "utf8");
  return config;
}
