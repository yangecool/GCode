...
function runBootstrapServerBuild() {
  ...
  // bootstrap:with-remote 的最终构建过去复用 build:bootstrap，导致 @gcode/server build
...
function runBootstrapWithRemoteBuild() {
  for (const filter of ["@gcode/rpc", "@gcode/web", "@gcode/formal-proof"]) {
...
runGit(["submodule", "update", "--init", "--recursive", "apps/gcode-cli"]);
