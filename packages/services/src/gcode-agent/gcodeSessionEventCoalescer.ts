import type { GCodeSessionEvent } from "@gcode/shared";
import type { GCodeAgentServiceEvent } from "#src/gcode-agent/gcodeAgent.js";
...
type SessionServiceEvent = Extract<GCodeAgentServiceEvent, { type: "session.event" }>;
...
interface BackgroundSessionEventCoalescer {
  accept(event: GCodeAgentServiceEvent): void;
  flush(): void;
  dispose(): void;
}
...
function getBackgroundSessionEventCoalesceKey(event: GCodeSessionEvent): string | null {
...
function mergeBackgroundSessionEvents(
  current: GCodeSessionEvent,
  next: GCodeSessionEvent,
): GCodeSessionEvent {
...
export function createBackgroundSessionEventCoalescer(params: {
  emit: (event: GCodeAgentServiceEvent) => void;
  flushDelayMs?: number;
  maxItems?: number;
