...
import { Emitter, Event as RpcEvent, type Event, type IDisposable } from "@gcode/rpc";
import {
...
export type GCodeAgentV4ClientMode = "desktop-continuous" | "web-remote-replayable";

export interface GCodeAgentV4ConnectionContext {
  connectionId: string;
  clientMode: GCodeAgentV4ClientMode;
  role?: "terminal-client" | "trusted-host-relay";
}

const TRUSTED_CONNECTION_FIELD = "__gcodeTrustedV4Connection";
const TRUSTED_UNSUBSCRIBE_ROUTE_FIELD = "__gcodeTrustedV4UnsubscribeRoute";

type TrustedConnectionCarrier = {
  [TRUSTED_CONNECTION_FIELD]?: GCodeAgentV4ConnectionContext;
};

interface TrustedGCodeAgentV4UnsubscribeRoute {
  topic: string;
  connectionId: string;
}

type TrustedUnsubscribeRouteCarrier = {
  [TRUSTED_UNSUBSCRIBE_ROUTE_FIELD]?: TrustedGCodeAgentV4UnsubscribeRoute;
};

export function readTrustedGCodeAgentV4UnsubscribeRoute(
  value: unknown,
): TrustedGCodeAgentV4UnsubscribeRoute | null {
 ...
 */
export function readTrustedGCodeAgentV4Connection(
  value: unknown,
): GCodeAgentV4ConnectionContext | null {
  if (typeof value !== "object" || value === null) return null;
...
function withTrustedConnection<T extends object>(
  value: T,
  context: GCodeAgentV4ConnectionContext,
): T {
...
function withTrustedUnsubscribeRoute<T extends object>(
  value: T,
  route: TrustedGCodeAgentV4UnsubscribeRoute,
): T {
  const forwarded = {
    ...(value as unknown as Record<string, unknown>),
  };
...
function workspaceKey(target: GCodeAgentWorkspaceTarget): string {
  return target.workspaceIdentity?.trim() || target.workspacePath;
}

function workspaceTarget(target: GCodeAgentWorkspaceTarget): GCodeAgentWorkspaceTarget {
  return {
...
interface ConnectionFlowRoute {
  key: string;
  target: GCodeAgentWorkspaceTarget;
  connection: GCodeAgentV4ConnectionContext;
}
...
function deliveryProfileFor(clientMode: GCodeAgentV4ClientMode): HelloMessage["deliveryProfile"] {
  return clientMode === "desktop-continuous" ? "continuous" : "replayable";
}

function createHello(context: GCodeAgentV4ConnectionContext): HelloMessage {
...
export interface GCodeAgentConnectionScope {
  readonly service: IGCodeAgentService;
...
export function createGCodeAgentConnectionScope(
  base: IGCodeAgentService,
  context: GCodeAgentV4ConnectionContext,
  ...
  const forwardedConnection = (params: unknown): GCodeAgentV4ConnectionContext => {
    const downstream = readTrustedGCodeAgentV4Connection(params);
  ...
  const ownershipKey = (kind: SubscriptionKind, target: GCodeAgentWorkspaceTarget, topic: string) =>
    `${kind}\0${workspaceKey(target)}\0${topic}`;
  const routedEventKey = (kind: SubscriptionKind, target: GCodeAgentWorkspaceTarget) =>
    `${kind}\0${workspaceKey(target)}`;
  const flowRouteKey = (target: GCodeAgentWorkspaceTarget, connectionId: string) =>
  ...
  const flowRouteForEntry = (entry: OwnedSubscription): ConnectionFlowRoute => ({
    ...
    } satisfies GCodeAgentV4ConnectionContext,
  });
  ...
  const findBySubscription = (
    ...
    target: GCodeAgentWorkspaceTarget,
    subscriptionId: string,
    expectedRoute: TrustedGCodeAgentV4UnsubscribeRoute | null,
  ...
  const forwardedUnsubscribeRoute = (
    ...
    const downstream = readTrustedGCodeAgentV4UnsubscribeRoute(params);
  ...
  const unsubscribeBase = (
    ...
    const params: GCodeAgentConversationUnsubscribeParams = {
      ...entry.target,
  ...
  const resyncBase = (entry: OwnedSubscription, params: GCodeAgentConversationResyncParams) => {
    const forwarded = withTrustedUnsubscribeRoute(params, {
      topic: entry.topic,
      connectionId: entry.connectionId,
    });
  ...
  const resyncOwned = (kind: SubscriptionKind, params: GCodeAgentConversationResyncParams) => {
    assertReady();
  ...
  const overrides: Partial<IGCodeAgentService> = {
