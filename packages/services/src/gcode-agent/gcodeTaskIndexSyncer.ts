...
import { repairSubagentTaskIndex } from "#src/gcode-agent/repairSubagentTaskIndex.js";
import {
  deriveGCodeTaskStatusFromSessionSnapshot,
  generateTraceId,
  getGCodeUserVisibleMessages,
  isGCodeGoalContinuationReminderText,
  isGCodeModelOnlySyntheticUserMessage,
  resolveWorkspaceKey,
...
import { Emitter, type Event, type IDisposable } from "@gcode/rpc";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { GCodeWorkspaceEventSubscriptionParams } from "#src/session/gcodeTaskListTypes.js";
import type {
  IGCodeAgentService,
...
import { formatTaskMetaModelSelectionFromSnapshot } from "./gcodeConfigOptions.js";

const logger = createServiceLogger("gcode-task-index-syncer");
...
export interface GCodeTaskIndexTerminalEvent {
...
export interface GCodeTaskIndexReadyEvent {
  target: GCodeAgentSessionTarget;
  /** v4 phase 终态映射：agent 收口后可接受下一条输入的 ready 边界。 */
  reason: "prompt_completed" | "prompt_failed";
}

export interface GCodeTaskIndexSyncer {
  /**
   ...
   */
  ensureWorkspaceSubscription(target: GCodeAgentWorkspaceTarget): void;
  /**
   ...
   * desktop-continuous 路径上，gcodeSessionService 在 createSession/resumeSession/setModel
  ...
  syncSnapshotAndBroadcast(
    ...
    options: {
      ...
      unreadSignal?: GCodeWorkspaceTaskListChanged["unreadSignal"];
      /**
       ...
       */
      broadcastReason: GCodeWorkspaceTaskListChanged["reason"];
    },
   ...
   */
  syncTaskModel(target: GCodeAgentSessionTarget, model: string): Promise<GCodeTaskMeta | null>;
  /**
  ...
  emitWorkspaceTaskListChanged(
    ...
    meta: GCodeTaskMeta | undefined,
    reason: GCodeWorkspaceTaskListChanged["reason"],
    options?: Pick<GCodeWorkspaceTaskListChanged, "unreadSignal">,
  ): void;
  /**
...
): GCodeWorkspaceTaskListChanged["unreadSignal"] {
...
function taskStatusFromSummaryPhase(phase: SessionPhase): GCodeTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
export function createGCodeTaskIndexSyncer(
  options: CreateGCodeTaskIndexSyncerOptions,
  ...
  const workspaceEmitters = new Map<string, Emitter<GCodeWorkspaceEvent>>();
  const terminalEventEmitter = new Emitter<GCodeTaskIndexTerminalEvent>();
  const readyEventEmitter = new Emitter<GCodeTaskIndexReadyEvent>();
  let disposed = false;
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<GCodeWorkspaceEvent> {
    ...
    if (!emitter) {
      emitter = new Emitter<GCodeWorkspaceEvent>();
  ...
  function broadcastTargetFrom(target: GCodeAgentSessionTarget): WorkspaceBroadcastTarget {
    return {
      workspacePath: target.workspacePath,
      workspaceIdentity: target.workspaceIdentity,
  ...
  function emitTerminalAndReady(target: GCodeAgentSessionTarget, summary: SessionSummary): void {
  ...
  function applyTerminalTransition(
    ...
    void taskIndexRepo
      ...
      .then((meta) => {
        ...
        // 不经过 gcodeSessionService 的 op 驱动 snapshot 同步，行缺失/正文搜索/lastError
  ...
  function applyTitleChange(target: GCodeAgentSessionTarget, title: string): void {
    const updatedAt = Date.now();
    void taskIndexRepo
      .applyAgentPatch({
        workspacePath: target.workspacePath,
      ...
      .then((meta) => {
        if (meta) {
          ...
          // v4 createSession 不经过 gcodeSessionService.createSession；首个标题到达且
  ...
  function processSummary(
    ...
    if (becameVisibleTask) {
      ...
      // 不经过 gcodeSessionService.createSession。此处是最早且不依赖标题时序的新任务边界；
  ...
  async function seedMissingRowsFromInitialSnapshot(
    ...
    // 纯 V4 UI 不经过 gcodeSessionService.initializeWorkspace；若把首帧
  ...
  function applyWorkspaceConfigFrame(
    ...
    if (!config || config.configOptions.length === 0) {
      // 空目录（无 live session 的订阅种子）不下发：下游 useGCodeConfig 收到空
      // configOptions 会把聊天工具栏的模型目录清掉。
      completeConfigRecoveryFrame(state, deliveryKind);
      return;
    }
    // v4 载荷与 GCodeConfigOption 结构对齐（shared 黄金测试背书），零映射直通，
    // 下游 workspace_config_options_update 消费面（useGCodeConfig 等）不改。
    getWorkspaceEmitter({
  ...
  function ensureWorkspaceSubscription(target: GCodeAgentWorkspaceTarget): void {
    if (disposed) {
      return;
    }
...
function hasUserVisibleContent(snapshot: GCodeSessionStateSnapshot): boolean {
  const title = snapshot.session.title?.trim() ?? "";
  if (title && !isGCodeGoalContinuationReminderText(title)) {
    return true;
  }
  ...
  return snapshot.messages.some((message) => {
    ...
    if (isGCodeModelOnlySyntheticUserMessage(message)) return false;
...
// 与 UI 默认折叠后展示的范围一致。
function buildSearchableTextFromSnapshot(snapshot: GCodeSessionStateSnapshot): string {
  const parts: string[] = [];
  let total = 0;
  // /goal 自动续跑会把内部 system-reminder 作为 runtime user turn 持久化；
  // 这条内容是 model-only 输入，不是用户可见 query，不能写进侧边栏搜索正文。
  for (const message of getGCodeUserVisibleMessages(snapshot.messages, {
    target: snapshot.projection.target,
  })) {
    const textParts = message.parts.filter(
      (part): part is Extract<GCodeMessagePart, { type: "text" }> => part.type === "text",
    );
...
function fromGCodeGoal(
  goal: NonNullable<GCodeSessionStateSnapshot["projection"]["target"]>,
): GCodeTaskGoal {
  return {
