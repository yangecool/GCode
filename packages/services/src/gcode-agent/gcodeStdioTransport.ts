...
import { gcodeProtocolMessageSchema } from "@gcode/shared";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import type {
  GCodeProtocolTransport,
  GCodeProtocolTransportClosedEvent,
} from "./gcodeProtocolTransport.js";
import {
...
interface GCodeStdioTransportOptions {
...
const processTreeLogger = createServiceLogger("gcode-agent-process-tree");

export class GCodeStdioTransport implements GCodeProtocolTransport {
  readonly kind = "stdio" as const;

  private readonly messageEmitter = new Emitter<GCodeProtocolMessage>();
  private readonly closeEmitter = new Emitter<GCodeProtocolTransportClosedEvent>();
  ...
  constructor(
    ...
    private readonly options?: GCodeStdioTransportOptions,
  ) {
    this.stderrCollector = new AgentStderrCollector(child.stderr, options?.onStderrLine);

    // GCode Protocol stdio 帧边界只认 LF。Node readline 会把 U+2028/U+2029
  ...
  async send(message: GCodeProtocolMessage): Promise<void> {
    if (this.disposed || this.closed || this.child.killed || !this.child.stdin.writable) {
      throw new Error("GCode agent stdio transport is closed");
    }
  ...
  private async disposeAndWaitOnce(): Promise<void> {
    ...
    if (!this.hasChildExited()) {
      ...
      const configuredEofWaitMs =
        process.env.GCODE_E2E_COVERAGE === "1"
  ...
  private handleStdoutFrame(frame: string): void {
    ...
    try {
      const parsed = gcodeProtocolMessageSchema.parse(JSON.parse(line));
  ...
  private handleStreamError(stream: "stdin" | "stdout", error: Error): void {
    ...
    // gcode-server 会因未处理的 EPIPE 直接崩溃，UI 只能看到远端连接断开而不是协议请求失败。
    // 这里把 stream error 视为 transport 已关闭，阻止后续继续向失效 agent 写入。
    this.fireClose({ reason: `${stream}_error: ${error.message}` });
  }

  private fireClose(event: GCodeProtocolTransportClosedEvent): void {
    if (this.closed) {
      return;
    }
    this.closed = true;
