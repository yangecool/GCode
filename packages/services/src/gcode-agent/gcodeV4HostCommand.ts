...
class GCodeV4CommandRejectedError extends Error {
  readonly code = "GCODE_V4_COMMAND_REJECTED";
  ...
  constructor(
    ...
    this.name = "GCodeV4CommandRejectedError";
  }
}
...
export function assertV4CommandAckOk(
  ...
  throw new GCodeV4CommandRejectedError(commandType, ack, contextMessage);
}
...
export async function sendHostCasCommandV4<T extends CommandType>(input: {
  ...
  throw new GCodeV4CommandRejectedError(
    input.type,
    lastAck ?? {
      commandId: "",
      status: "stale",
