/* oxlint-disable eslint(max-lines) -- 迁移期需要在一个门面里集中维护旧 task projection 到 GCode session 的协议适配。 */
...
import {
  coalesceConsecutiveGCodeAssistants,
  ...
  generateTraceId,
  getGCodeGoalActiveIterationCount,
  getGCodeGoalIterationByAssistantMessageId,
  getGCodeUserVisibleMessages,
  isMainAgentToolProjectionSource,
  normalizeGCodeApiRetryStatus,
  attachGCodeBackgroundTaskNotificationToRaw,
  collectGCodeBackgroundTaskNotificationsByToolUseId,
  mergeGCodeBackgroundTaskControlItems,
  parseGCodeBackgroundTaskControlItems,
  parseGCodeBackgroundTaskNotificationText,
  parseModelPickerValue as parseSharedModelSelection,
  resolveWorkspaceKey,
  resolveGCodeVisibleSessionTitle,
  textFromGCodeMessageParts,
  GCODE_AGENT_PROVIDER,
  gcodeBackgroundTaskNotificationToolUpdateStatus,
  appendGCodeStreamingToolInputDelta,
  buildGCodeStreamingToolInputPreview,
  createGCodeToolProjectionMemory,
  finalizeGCodeToolProjectionInput,
  forgetGCodeToolProjectionMetadata,
  isGCodeModelRetryRecoveryProgressPayload,
  markGCodeStreamingToolInputPreviewMaterialized,
  resolveGCodeToolProjectionMetadata,
  gcodeApiRetryFromModelNetworkStatusPayload,
  gcodeApiRetryFromStreamRecoveryPayload,
  gcodeTaskNetworkDebugStatusFromPayload,
  shouldMaterializeGCodeStreamingToolInputPreview,
  type GCodeApiRetryStatus,
  type GCodeAssistantMessageFeedback,
  type GCodeBackgroundTaskNotificationInfo,
  type GCodeBackgroundTaskControlItem,
  type GCodeBackgroundTurnAttribution,
  type GCodeCancelTaskCommandResult,
  type GCodeConfigOption,
  type GCodeEnqueueTaskCommandResult,
  type GCodeError,
  type GCodeGoalVerificationTimelineMeta,
  type GCodeImportSessionsResult,
  type GCodeImportableSessionCandidate,
  type GCodeUsage,
  type GCodePersistedMessage,
  type GCodePersistedMessagePart,
  type GCodePersistedToolCall,
  type GCodePromptAttachment,
  type GCodeProvider,
  type GCodeSessionFile,
  type GCodeTaskGoal,
  type GCodeTaskGoalStats,
  type GCodeTaskMode,
  type GCodePlanStep,
  type GCodeSlashCommand,
  type GCodeStreamEvent,
  type GCodeTaskCreateResult,
  type GCodeTaskMeta,
  type GCodeTaskClientMode,
  type GCodeTaskRuntimeCommand,
  type GCodeTaskSnapshot,
  type GCodeTaskSnapshotBody,
  type GCodeTaskSnapshotRefContent,
  type GCodeTaskSnapshotToolCallsSlice,
  type GCodeTaskTokenUsageResult,
  type GCodeTodoGroup,
  type GCodeTurnSteerCommandKind,
  type GCodeTurnSteerSource,
  type GCodeWorkspaceEvent,
  type GCodeWorkspaceTaskListChanged,
  type InputId,
  type TraceId,
  gcodeContextUsageBreakdownSchema,
  gcodeSessionSettingsStateSchema,
  type GCodeDeliveryKind,
  type GCodeMessagePart,
  type GCodeMessageWithParts,
  type ModelSelection,
  type GCodePermissionOption,
  type GCodePermissionRequestParams,
  type GCodePermissionRequest,
  type GCodeSessionEvent,
  type GCodeSessionMode,
  type GCodeSessionSettingsState,
  type GCodeSessionStateSnapshot,
  type GCodeStateUpdatedNotification,
  type GCodeContextCompactionTimelineMeta,
  type GCodeTimelineMeta,
  type GCodeTimelineStatus,
  type GCodeTimelineTrigger,
  type GCodeToolProjectionMemory,
  type GCodeUserInputRequestParams,
  type GCodeUserInputResponse,
  type GCodeAgentMcpServer,
} from "@gcode/shared";
import type {
  GCodeTaskListQuery,
  GCodeTaskListResult,
  GCodeWorkspaceEventSubscriptionParams,
  IGCodeTaskService,
  GCodeArchivedTaskDeletionResult,
  GCodeTaskReadyOutcome,
  GCodeTaskTerminalOutcome,
...
import type {
  GCodeTaskIndexReadyEvent,
  GCodeTaskIndexSyncer,
  GCodeTaskIndexTerminalEvent,
} from "./gcodeTaskIndexSyncer.js";
import { readModelTrajectory } from "./modelTrajectory.js";
import { errorAttributionSchema, type CommandPayloadMap } from "@gcode/shared/gcode-protocol-v4";
import {
  assertV4CommandAckOk,
  createHostCommandEnvelope,
  sendHostCasCommandV4,
} from "./gcodeV4HostCommand.js";
...
import {
  ...
  getGCodeAgentAvailableModes,
  normalizeAvailableGCodeMode,
  settingsToConfigOptions,
} from "./gcodeConfigOptions.js";
...
interface CreateGCodeTaskServiceAdapterOptions {
  ...
  taskIndexSyncer: GCodeTaskIndexSyncer;
...
type WorkspaceEventInput = string | GCodeWorkspaceEventSubscriptionParams;
type GCodeSendPromptRuntimeCommand = Extract<GCodeTaskRuntimeCommand, { type: "send_prompt" }>;
type GCodeTerminalStreamEvent =
  | Extract<GCodeStreamEvent, { type: "task_complete" }>
  | Extract<GCodeStreamEvent, { type: "task_error" }>;

const GLM_PROVIDER: GCodeProvider = GCODE_AGENT_PROVIDER;
const EMPTY_SLASH_COMMANDS: GCodeSlashCommand[] = [];
const logger = createServiceLogger("gcode-task-service");
...
function formatGCodeAgentLogDate(now: Date): string {
  return [now.getFullYear(), padDatePart(now.getMonth() + 1), padDatePart(now.getDate())].join("-");
}

function resolveGCodeAgentCurrentLogFilePath(now = new Date()): string {
  const configuredLogDir = process.env.GCODE_LOG_DIR?.trim();
  const logDir = configuredLogDir || join(homedir(), ".gcode", "cli", "log");
  return join(logDir, `gcode-${formatGCodeAgentLogDate(now)}.jsonl`);
}

export function createGCodeTaskServiceAdapter(
  options: CreateGCodeTaskServiceAdapterOptions,
): IGCodeTaskService {
  const errorEmitter = new Emitter<GCodeError>();
  const taskEmitters = new Map<string, Emitter<GCodeStreamEvent>>();
  const globalTaskEmitters = new Map<string, Emitter<GCodeStreamEvent>>();
  const overlays = new Map<string, TaskOverlay>();
  const taskTargets = new Map<string, TaskTarget>();
  const runtimeCommands = new Map<string, GCodeTaskRuntimeCommand[]>();
  const runtimeCommandDrains = new Map<string, Promise<void>>();
  const apiRetryByTaskKey = new Map<string, GCodeApiRetryStatus | null>();
  const backgroundTaskControlsByTaskKey = new Map<string, GCodeBackgroundTaskControlItem[]>();
  const streamedTurnKeys = new Set<string>();
  const activePromptInputIds = new Map<string, InputId>();
  const toolProjectionMemoryByTaskKey = new Map<string, GCodeToolProjectionMemory>();
  ...
  function unsupported(name: string): never {
    throw Object.assign(
      new Error(`GCode task service adapter does not support IGCodeTaskService.${name} yet.`),
      {
        code: "GCODE_AGENT_UNSUPPORTED_LEGACY_TASK_METHOD",
      },
    );
  }
  ...
  function turnAttributionOf(
    params: GCodeBackgroundTurnAttribution,
  ...
  async function resolveProductMcpServers(
    servers: GCodeAgentMcpServer[] | undefined,
  ): Promise<GCodeAgentMcpServer[] | undefined> {
  ...
  async function sendPromptToAgent(
    ...
    // GCode task wrapper 的字段仍叫 traceId，但这里语义已经是单次输入 inputId。
    // 先记录 inputId，后续 GCode session 事件回投 GCode Agent 时才能让 UI 终态按输入轮次收口。
    activePromptInputIds.set(taskKey(target), params.traceId);
    logger.info(params.traceId, "GCode task facade sendPrompt 开始", {
    ...
    try {
      ...
      if (params.attachments?.length) {
        ...
        await options.gcodeAgentService.sendPrompt({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
        ...
        const ack = await options.gcodeAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          ...(target.remoteSessionId ? { remoteSessionId: target.remoteSessionId } : {}),
          ...(params.clientMode ? { clientMode: params.clientMode } : {}),
      ...
      logger.info(params.traceId, "GCode task facade sendPrompt ACK", {
      ...
      logger.warn(params.traceId, "GCode task facade sendPrompt 失败", {
        durationMs: Date.now() - startedAt,
  ...
  function setRuntimeCommands(params: TaskTarget, commands: GCodeTaskRuntimeCommand[]): void {
    const key = taskKey(params);
  ...
  function markRuntimeCommandRunning(
    params: TaskTarget,
    command: GCodeSendPromptRuntimeCommand,
  ): GCodeSendPromptRuntimeCommand {
    const key = taskKey(params);
    const runningCommand: GCodeSendPromptRuntimeCommand = {
      ...command,
      status: "running",
      updatedAt: Date.now(),
    };
  ...
  function markRuntimeCommandFailed(
    ...
    const key = taskKey(params);
    const failedCommand: GCodeSendPromptRuntimeCommand = {
      ...command,
      status: "failed",
      updatedAt: Date.now(),
      error: error instanceof Error ? error.message : String(error),
  ...
  function completeRuntimeCommandByInputId(
    ...
    logger.info(command.traceId, "GCode task command 终态收口", {
  ...
  function completeRuntimeCommandForTerminalEvent(
    params: TaskTarget,
    event: GCodeTerminalStreamEvent,
  ): void {
  ...
  async function drainRuntimeCommands(params: TaskTarget, reason: string): Promise<void> {
    ...
    const command = commands.find(
      (candidate): candidate is GCodeSendPromptRuntimeCommand =>
    ...
    logger.info(runningCommand.traceId, "GCode task command drain 开始", {
    ...
    try {
      ...
      logger.warn(runningCommand.traceId, "GCode task command drain 失败", {
        commandId: runningCommand.commandId,
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
  ...
  async function drainRuntimeCommandsSafely(params: TaskTarget, reason: string): Promise<void> {
    try {
      ...
      logger.warn(undefined, "GCode task command drain 调度失败", {
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
  ...
  function persistedStatusFromToolUpdate(
    status: Extract<GCodeStreamEvent, { type: "tool_call_update" }>["status"],
  ): GCodePersistedToolCall["status"] | undefined {
  ...
  function rememberLiveToolProjection(params: TaskTarget, event: GCodeStreamEvent): void {
    ...
    const nextTool: GCodePersistedToolCall =
  ...
  function persistedToolId(tool: GCodePersistedToolCall): string | undefined {
  ...
  function mergePersistedToolCall(
    snapshotTool: GCodePersistedToolCall | undefined,
    liveTool: GCodePersistedToolCall,
  ): GCodePersistedToolCall {
    if (!snapshotTool) {
      return liveTool;
    }
    return {
  ...
  function findToolLocation(
    messages: readonly GCodePersistedMessage[],
    toolId: string,
  ...
  function latestAssistantMessageIndex(messages: readonly GCodePersistedMessage[]): number {
  ...
  function upsertPersistedToolPart(
    parts: readonly GCodePersistedMessagePart[] | undefined,
    toolIndex: number,
  ): GCodePersistedMessagePart[] {
  ...
  function updateSnapshotMessageTool(
    message: GCodePersistedMessage,
    toolIndex: number,
    tool: GCodePersistedToolCall,
  ): GCodePersistedMessage {
  ...
  function mergeLiveToolProjectionIntoSnapshotMessages(
    ...
    if (changed) {
      logger.debug(undefined, "GCode snapshot 合并 live tool projection", {
        event: "gcode_task.snapshot.live_tool_projection.merged",
  ...
  function getTaskTarget(taskId: string): TaskTarget {
    ...
    if (!target) {
      throw Object.assign(new Error(`GCode session target is not loaded: ${taskId}`), {
        code: "GCODE_SESSION_TARGET_NOT_FOUND",
      });
    }
    return target;
  }
  ...
  function getToolProjectionMemory(params: TaskTarget): GCodeToolProjectionMemory {
    const key = taskKey(params);
    let memory = toolProjectionMemoryByTaskKey.get(key);
    if (!memory) {
      memory = createGCodeToolProjectionMemory();
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<GCodeWorkspaceEvent> {
    return taskIndexSyncer.getWorkspaceEmitter(workspace);
  }

  function getTaskEmitter(params: TaskTarget): Emitter<GCodeStreamEvent> {
    const key = taskKey(params);
    let emitter = taskEmitters.get(key);
    if (!emitter) {
      emitter = new Emitter<GCodeStreamEvent>();
      taskEmitters.set(key, emitter);
    }
    return emitter;
  }

  function getGlobalTaskEmitter(taskId: string): Emitter<GCodeStreamEvent> {
  ...
  function emitTaskEvent(params: TaskTarget, event: GCodeStreamEvent): void {
  ...
  function emitWorkspaceTaskListChanged(
    ...
    taskMeta: GCodeTaskMeta | undefined,
    reason: GCodeWorkspaceTaskListChanged["reason"],
  ) {
  ...
  async function resumeSnapshot(
    ...
    return options.gcodeAgentService.resumeSession({
      workspacePath: params.workspacePath,
      workspaceIdentity: params.workspaceIdentity,
  ...
  async function sendConfigCasCommandV4<T extends "switchModelConfig" | "switchCollaborationMode">(
    ...
    await sendHostCasCommandV4({
      send: (envelope) =>
        options.gcodeAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          envelope,
        }),
   ...
   * command.ts 裁决），而旧协议 GCodeSessionMode 含 auto 且旧 op 接受它——为 UI 行为
  ...
  async function switchCollaborationModeViaProtocol(
    ...
    if (mode === "auto") {
      await options.gcodeAgentService.setMode({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: target.taskId,
        mode,
  ...
  async function repairEmptyImportedClaudeSnapshot(
    ...
    const repaired = await repairImportedClaudeSessionSnapshot({
      snapshot,
      target: params,
      createSession: (input) => options.gcodeAgentService.createSession(input),
      onRepair: (history) => {
        logger.warn(
          undefined,
  ...
  function snapshotToMeta(snapshot: GCodeSessionStateSnapshot): GCodeTaskMeta {
    const target = {
  ...
  function rememberIndexedTaskMeta(meta: GCodeTaskMeta): GCodeTaskMeta {
  ...
  async function resolveTaskIndexResumeHints(
    ...
    if (model || thoughtLevel) {
      ...
      logger.info(undefined, "从 task index 回填 GCode session resume 配置", {
        model: model || null,
  ...
  async function syncTaskIndexMeta(meta: GCodeTaskMeta): Promise<GCodeTaskMeta> {
  ...
  async function updateIndexedTaskState(
    ...
    try {
      ...
      // 旧 GCode session 可能还没有轻量 task index 行。
  ...
  function updateTaskIndexFromStreamEvent(params: TaskTarget, event: GCodeStreamEvent): void {
  ...
  function updateTaskApiRetryFromStreamEvent(params: TaskTarget, event: GCodeStreamEvent): void {
    const key = taskKey(params);
  ...
  function snapshotToGCode(
    ...
    const goalIterationByAssistantMessageId = getGCodeGoalIterationByAssistantMessageId(
      snapshot.messages,
      {
    ...
    const backgroundTaskNotifications = collectGCodeBackgroundTaskNotificationsByToolUseId(
      snapshot.messages,
    );
      ...
      normalizeGoalVerificationTimelineMessageOrder(
        addGoalVerificationTimelineSnapshotFallback(
          addSessionForkSnapshotFallback(
            coalesceConsecutiveGCodeAssistants(
              getGCodeUserVisibleMessages(snapshot.messages, {
                target: snapshot.projection.target,
              }).map((message) =>
                mapMessage(
                  message,
    ...
    const pendingPermissions: GCodePermissionRequest[] = snapshot.projection.pendingPermissions
    ...
    const pendingElicitations = snapshot.projection.pendingPermissions
      ...
      .filter(
        (event): event is Extract<GCodeStreamEvent, { type: "elicitation_request" }> =>
          event !== null,
      );
    const backgroundTaskControls = parseGCodeBackgroundTaskControlItems(
  ...
  function mapServiceEvent(params: TaskTarget, event: GCodeAgentServiceEvent): void {
    if (event.type === "snapshot") {
      void syncTaskIndexSnapshot(event.snapshot).catch((error) => {
        logger.warn(undefined, "同步 GCode snapshot 到 task index 失败", error);
      });
      const snapshotEvent: GCodeStreamEvent = {
        type: "task_snapshot_updated",
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: workspaceKey(params),
  ...
  function handleTaskIndexTerminalEvent(event: GCodeTaskIndexTerminalEvent): void {
  ...
  function handleTaskIndexReadyEvent(event: GCodeTaskIndexReadyEvent): void {
    const params: TaskTarget = {
      taskId: event.target.sessionId,
      workspacePath: event.target.workspacePath,
      workspaceIdentity: event.target.workspaceIdentity,
  ...
  async function disposeGCodeAgentServiceAndWait(): Promise<void> {
    const agentService = options.gcodeAgentService as IGCodeAgentService & {
  ...
  const service: IGCodeTaskService & {
    disposeAll(): void;
    disposeAllAndWait(): Promise<void>;
  } = {
    async initialize(params) {
      const result = await options.gcodeAgentService.initialize(params);
      return {
    ...
    async releaseWorkspacePreparation(params): Promise<void> {
      ...
      await options.gcodeAgentService.disposeWorkspace(normalizeWorkspaceParams(params));
    },

    async createTask(params): Promise<GCodeTaskCreateResult> {
      ...
      const mcpServers = await resolveProductMcpServers(params.mcpServers);
      let snapshot: GCodeSessionStateSnapshot | null = null;
      if (draftSessionId && !mcpServers) {
        try {
          snapshot = await options.gcodeAgentService.readSession({
            ...target,
          ...
          if (requestedSelection) {
            if (!sameModelSelection(snapshot.settings.model.current, requestedSelection)) {
              ...
              snapshot = await options.gcodeAgentService.setModel({
                ...target,
                sessionId: draftSessionId,
          ...
          if (
            ...
            snapshot = await options.gcodeAgentService.setThoughtLevel({
              ...target,
              sessionId: draftSessionId,
      ...
      if (!snapshot) {
        ...
        // 迁移属 v4 生命周期收口。
        snapshot = await options.gcodeAgentService.createSession({
          ...target,
          sessionTraceId: createSessionTraceId(),
          mode: toGCodeMode(params.mode),
          model: requestedSelection,
          thoughtLevel: requestedSelection?.options?.reasoningLevel,
    ...
    async enqueueTaskCommand(params): Promise<GCodeEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const workspaceKeyValue = workspaceKey(params);
      const command: GCodeTaskRuntimeCommand = {
    ...
    async promoteTaskCommand(params): Promise<GCodeEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
    ...
    async cancelTaskCommand(params): Promise<GCodeCancelTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
      const command = commands.find((candidate) => candidate.commandId === params.commandId);
      if (!command) {
        logger.info(undefined, "GCode task command 取消时已不存在", {
          commandId: params.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      if (command.status === "running") {
        logger.info(command.traceId, "GCode task command 已开始运行，跳过取消", {
          commandId: command.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      logger.info(command.traceId, "GCode task command 已取消", {
        commandId: command.commandId,
        status: command.status,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
    ...
    async stopGeneration(params): Promise<void> {
      ...
      logger.info(params.runId, "GCode task facade stopGeneration 开始", {
      ...
      logger.info(params.runId, "GCode task facade stopGeneration ACK", {
        durationMs: Date.now() - startedAt,
        taskId: params.taskId,
        workspaceIdentity: target.workspaceIdentity ?? null,
        workspaceKey: resolveWorkspaceKey(target),
    ...
    async compactSession(params) {
      ...
      try {
        const result = await options.gcodeAgentService.compactSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
    ...
    async goalSession(params) {
      ...
      logger.info(params.inputId, "[gcode-task-service] goalSession start", {
      ...
      const result = await options.gcodeAgentService.goalSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
        inputId: params.inputId,
      ...
      logger.info(params.inputId, "[gcode-task-service] goalSession agent 返回", {
        action: params.action,
        durationMs: Date.now() - startedAt,
      ...
      emitWorkspaceTaskListChanged(target, meta, "task_status_changed");
      logger.info(params.inputId, "[gcode-task-service] goalSession task index 同步完成", {
        action: params.action,
        durationMs: Date.now() - startedAt,
        startedTurn: result.startedTurn,
        status: result.snapshot.session.status,
    ...
    async closeTask(params): Promise<void> {
      ...
      await options.gcodeAgentService.closeSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async resumeTask(params): Promise<GCodeTaskMeta> {
    ...
    async listTasks(params): Promise<GCodeTaskMeta[]> {
    ...
    async listPinnedTasks(params): Promise<GCodeTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async listTaskList(params: GCodeTaskListQuery): Promise<GCodeTaskListResult> {
    ...
    async listArchivedTasks(params): Promise<GCodeTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async archiveStaleTasks(params): Promise<GCodeTaskMeta[]> {
    ...
    async archiveWorkspaceTasks(params): Promise<GCodeTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async getTaskSnapshot(params): Promise<GCodeTaskSnapshot | null> {
      const startedAt = Date.now();
      ...
      const gcodeSnapshot = limitTaskSnapshotMessages(
        snapshotToGCode(snapshot, {
      ...
      const indexedMeta = await syncTaskIndexMeta(gcodeSnapshot.meta);
      const indexDurationMs = Date.now() - indexStartedAt;
      logger.info(undefined, "[gcode-task-service] 历史快照读取完成", {
        ...
        stats: getTaskSnapshotMessageDiagnostics(gcodeSnapshot),
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
      return { ...gcodeSnapshot, meta: indexedMeta };
    },
    ...
    async getTaskSnapshotWithEtag(params) {
      ...
      if (!snapshot) {
        logger.info(undefined, "[gcode-task-service] 历史快照 ETag 读取为空", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          messageLimit: params.messageLimit ?? null,
          taskId: params.taskId,
      ...
      if (params.ifNoneMatch && params.ifNoneMatch === etag) {
        logger.info(undefined, "[gcode-task-service] 历史快照 ETag 命中缓存", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          etagDurationMs,
          messageLimit: params.messageLimit ?? null,
      ...
      logger.info(undefined, "[gcode-task-service] 历史快照 ETag 生成完成", {
        clientMode: params.clientMode ?? "unknown",
        durationMs: Date.now() - startedAt,
        etagDurationMs,
        messageLimit: params.messageLimit ?? null,
    ...
    async getTaskSnapshotBody(): Promise<GCodeTaskSnapshotBody | null> {
      return null;
    },

    async getTaskSnapshotRef(): Promise<GCodeTaskSnapshotRefContent | null> {
      return null;
    },

    async getTaskSnapshotToolCallsSlice(): Promise<GCodeTaskSnapshotToolCallsSlice | null> {
      return null;
    },

    async getTaskMeta(params): Promise<GCodeTaskMeta | null> {
      const meta = await taskIndexRepo.getTaskMeta(params);
      return meta ? rememberIndexedTaskMeta(meta) : null;
    },

    async getTaskConfigOptions(params): Promise<GCodeConfigOption[]> {
    ...
    async getTaskModelSelection(params): Promise<ModelSelection | null> {
      const target = getTaskTarget(params.taskId);
      const snapshot = await options.gcodeAgentService.readSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async setAssistantMessageFeedback(params): Promise<GCodeSessionFile> {
      const snapshot = await service.getTaskSnapshot(params);
      if (!snapshot) {
      ...
      if (targetMessage) {
        targetMessage.feedback = params.feedback as GCodeAssistantMessageFeedback | undefined;
      }
      return snapshot;
    },
    ...
    async importClaudeSessions(params: {
      ...
      workspaceIdentity?: string;
      sessionIds: string[];
    }): Promise<GCodeImportSessionsResult> {
      return importClaudeNativeSessions({
        ...
        createImportedSession: async (source) => {
          const targetWorkspaceIdentity = params.workspacePath
            ...
            : undefined;
          const snapshot = await options.gcodeAgentService.createSession({
          ...
          // 导入后的任务必须是真实 GCode session，setModel/sendPrompt 才能继续命中 runtime。
    ...
    async setMode(params): Promise<void> {
      ...
      const target = getTaskTarget(params.taskId);
      await switchCollaborationModeViaProtocol(target, toGCodeMode(params.mode) ?? "build");
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    },

    async setConfigOption(params): Promise<GCodeConfigOption[]> {
      const target = getTaskTarget(params.taskId);
      ...
      if (params.configId === THOUGHT_LEVEL_CONFIG_ID) {
        ...
        const current = await options.gcodeAgentService.readSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
        });
        ...
        await switchCollaborationModeViaProtocol(
          target,
          toGCodeMode(params.value as GCodeTaskMode) ?? "build",
        );
      }
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    ...
    async setModel(params): Promise<GCodeConfigOption[]> {
      ...
      const target = getTaskTarget(params.taskId);
      await options.gcodeAgentService.setModel({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
    ...
    async setAutomationSessionConfig(params): Promise<GCodeConfigOption[]> {
      const target = getTaskTarget(params.taskId);
    ...
    async getTaskNativeSessionLogFile() {
      const path = resolveGCodeAgentCurrentLogFilePath();
      // 返回 GCode Agent 的结构化日志 JSONL；日志行中的 sessionId 用于按当前任务排查。
      return { provider: GLM_PROVIDER, path, exists: existsSync(path) };
    },

    async getModelTrajectory(params) {
      // GCode Agent 把 taskId 当作 sessionId 落盘 model-io（见本文件其它 sessionId: params.taskId 用法），
      // 这里按 sessionId 还原该 task 的模型调用轨迹，供 UI 侧边栏可视化。
      const trajectory = await readModelTrajectory(params.taskId, params.limit);
      logger.info(
        `[GCodeTaskService] getModelTrajectory taskId=${params.taskId} records=${trajectory.records.length} files=${trajectory.sourceFiles.length} truncated=${trajectory.truncated}`,
      );
      return trajectory;
    },

    async getTaskTokenUsage(params): Promise<GCodeTaskTokenUsageResult> {
      // 摘要面板需要展示 task 的累计模型消耗，不能复用 usage_update 的 context window。
      // 这里通过 GCode Protocol 读 agent SQLite 的 model_usage 聚合，保持桌面和远控同一事实源。
      return options.gcodeAgentService.getTaskTokenUsage({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async getTaskSessionFilePath(params) {
      return {
        path: `${params.workspacePath}/${params.taskId}.gcode-session`,
        exists: false,
      };
    },
    ...
    async deleteArchivedTasks(params): Promise<GCodeArchivedTaskDeletionResult> {
      const result: GCodeArchivedTaskDeletionResult = {
        deletedTaskIds: [],
        skippedTaskIds: [],
        failedTaskIds: [],
      };
    ...
    async renameTask(params): Promise<GCodeTaskMeta> {
      const renamedAt = Date.now();
      logger.info(undefined, "[GCodeTaskService] renameTask start", {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: resolveWorkspaceKey(params),
      ...
      try {
        ...
        logger.info(undefined, "[GCodeTaskService] renameTask overlay set", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        ...
        logger.info(undefined, "[GCodeTaskService] renameTask index updated", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        ...
        logger.info(undefined, "[GCodeTaskService] renameTask event emitted", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        return meta;
      } catch (error) {
        logger.error(undefined, "[GCodeTaskService] renameTask failed", {
          taskId: params.taskId,
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          workspaceKey: resolveWorkspaceKey(params),
    ...
    async setTaskPinned(params): Promise<GCodeTaskMeta> {
    ...
    async setTaskUnread(params): Promise<GCodeTaskMeta> {
    ...
    async archiveTask(params): Promise<GCodeTaskMeta> {
    ...
    async unarchiveTask(params): Promise<GCodeTaskMeta> {
    ...
    async branchTaskFromPrompt(): Promise<GCodeTaskCreateResult> {
      unsupported("branchTaskFromPrompt");
    },

    onDynamicStreamEvent(taskId: string): Event<GCodeStreamEvent> {
      return getGlobalTaskEmitter(taskId).event;
    },

    onDynamicTaskTerminalOutcome(taskId: string): Event<GCodeTaskTerminalOutcome> {
    ...
    onDynamicTaskReady(taskId: string): Event<GCodeTaskReadyOutcome> {
      return (listener) =>
    ...
    onDynamicTaskEvent(params): Event<GCodeStreamEvent> {
      const target = {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
      ...
      return (listener) => {
        ...
        // 已收敛 v4 命令；读路径消费方是
        // host 镜像 taskRealtimePort（词表同为 GCodeStreamEvent），镜像换 v4 帧 = relay
        // 协议与手机 store 整链重做。
        // 过渡归宿 = replayable 读路径 v4 store，与 host/index.ts 镜像、
        // mapStateUpdated/mapServiceEvent、agentService.onDynamicSessionEvent 同批摘除。
        const upstreamDisposable = options.gcodeAgentService.onDynamicSessionEvent({
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          sessionId: params.taskId,
          deliveryKind: toGCodeDeliveryKind(params.deliveryKind),
    ...
    onDynamicWorkspaceEvent(workspace): Event<GCodeWorkspaceEvent> {
    ...
    disposeAll(): void {
      ...
      options.gcodeAgentService.disposeAll();
    ...
    async disposeAllAndWait(): Promise<void> {
      ...
      // 否则 host 退出时会把 gcode-cli 的 SIGKILL 兜底 timer 一起带走。
      taskIndexSyncer.disposeAll();
      await disposeGCodeAgentServiceAndWait();
      disposeLocalTaskState();
    },
  };
...
function applyOverlayToMeta(meta: GCodeTaskMeta, overlay: TaskOverlay): GCodeTaskMeta {
  return {
    ...meta,
...
function buildSnapshotHistory(
  ...
  messageLimit: number,
): NonNullable<GCodeTaskSnapshot["history"]> {
  return {
...
function getTaskSnapshotMessageDiagnostics(snapshot: GCodeTaskSnapshot) {
...
function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    // UI 下拉的 custom:provider:model 只是展示态，不能原样传给 gcode-cli。
...
function toGCodeMode(mode: GCodeTaskMode | undefined): GCodeSessionMode | undefined {
  switch (mode) {
    case "plan":
      return "plan";
    case "edit":
...
function addSessionForkSnapshotFallback(
  ...
  return [
    ...messages,
    {
      id: `gcode-timeline-fork-${parentSessionId}-`,
      role: "user",
      content: "",
...
function addGoalVerificationTimelineSnapshotFallback(
  ...
  const timelineMessages = timeline
    ...
    .map<GCodePersistedMessage>((item) => ({
...
function goalVerificationTimelineIdentityKey(
  item: Extract<GCodeGoalVerificationTimelineMeta, { type: "goal_verification" }>,
): string {
...
function goalVerificationTimelineMessageId(
  ...
  if (typeof item.goalIteration === "number") {
    return `gcode-goal-verification-${item.targetId}-${item.goalIteration}`;
  }
  return `gcode-goal-verification-${item.verificationId}`;
}

function insertGoalVerificationTimelineMessages(
  messages: GCodePersistedMessage[],
  timelineMessages: GCodePersistedMessage[],
): GCodePersistedMessage[] {
...
function getSnapshotGoalActiveIterationCount(snapshot: GCodeSessionStateSnapshot): number {
  ...
  return getGCodeGoalActiveIterationCount({
    targetStatus: snapshot.projection.target?.status ?? null,
    timeline,
  });
}

function toGCodeDeliveryKind(
...
function backgroundTaskNotificationToolUpdateFromInput(params: {
  ...
  taskId: string;
  traceId: TraceId;
}): Extract<GCodeStreamEvent, { type: "tool_call_update" }> | null {
  const parsed = parseGCodeBackgroundTaskNotificationText(params.input);
  if (!parsed) {
    return null;
  }
  const status = gcodeBackgroundTaskNotificationToolUpdateStatus(parsed.notification.status);
  return {
    ...
    ...(status === "failed" && parsed.notification.error
      ...
      : {}),
    raw: attachGCodeBackgroundTaskNotificationToRaw(
...
function mapMessage(
  message: GCodeMessageWithParts,
  goalIteration?: number,
  backgroundTaskNotifications?: ReadonlyMap<string, GCodeBackgroundTaskNotificationInfo>,
): GCodePersistedMessage {
  const tools: GCodePersistedToolCall[] = [];
  const parts: GCodePersistedMessagePart[] = [];
  const attachments =
    message.info.role === "user" ? mapPromptAttachmentsFromParts(message.parts) : undefined;
  let syntheticTimeline: GCodeTimelineMeta | undefined;
...
function mapPromptAttachmentsFromParts(
  parts: readonly GCodeMessagePart[],
): GCodePromptAttachment[] | undefined {
  const attachments = parts
    .filter((part): part is Extract<GCodeMessagePart, { type: "file" }> => part.type === "file")
    .map(mapPromptAttachmentFromFilePart)
    .filter((attachment): attachment is GCodePromptAttachment => attachment !== undefined);
  return attachments.length > 0 ? attachments : undefined;
}

function mapPromptAttachmentFromFilePart(
  part: Extract<GCodeMessagePart, { type: "file" }>,
): GCodePromptAttachment | undefined {
...
function extractSyntheticTimelineFromTextPart(
  part: Extract<GCodeMessagePart, { type: "text" }>,
): GCodeTimelineMeta | undefined {
...
function synthesizeCompactionTimeline(
  part: Extract<GCodeMessagePart, { type: "compaction" }>,
): GCodeTimelineMeta | undefined {
  const metadata = asRecord(part.metadata);
...
function mapToolPart(
  part: Extract<GCodeMessagePart, { type: "tool" }>,
  backgroundTaskNotifications?: ReadonlyMap<string, GCodeBackgroundTaskNotificationInfo>,
): GCodePersistedToolCall {
  const state = part.state;
  const taskNotification = backgroundTaskNotifications?.get(part.callId);
  // GCode Protocol 的 part.callId 是实时流和终态 snapshot 共同的工具身份。
  // 以前只保存 metadata 会丢掉 toolCallId，手机 replayable 里 result-only 临时工具就无法被终态快照覆盖。
  const raw = attachGCodeBackgroundTaskNotificationToRaw(
  ...
  if (state.status === "completed") {
    ...
    const notificationStatus = taskNotification?.status
      ? gcodeBackgroundTaskNotificationToolUpdateStatus(taskNotification.status)
      : undefined;
...
function textFromParts(parts: readonly GCodeMessagePart[]): string {
  return textFromGCodeMessageParts(parts);
}

function reasoningFromParts(parts: readonly GCodeMessagePart[]): string | undefined {
  const text = parts
    .filter(
      (part): part is Extract<GCodeMessagePart, { type: "reasoning" }> => part.type === "reasoning",
    )
    .map((part) => part.text)
    .join("");
...
function sessionTodosToPlanSteps(
  todos: GCodeSessionStateSnapshot["todos"],
): GCodePlanStep[] | null {
...
function sessionGoalStatsToRuntime(
  stats: GCodeSessionStateSnapshot["goalStats"],
): GCodeTaskGoalStats | null {
  return stats ? { ...stats } : null;
}

function sessionTodoGroupsToRuntime(
  groups: GCodeSessionStateSnapshot["todoGroups"],
): GCodeTodoGroup[] | null {
...
function mapStateUpdated(
  ...
  const parsedSettings = gcodeSessionSettingsStateSchema.safeParse(notification.patch);
  ...
  return [
    {
      ...
      taskId: params.taskId,
      traceId,
      currentModeId: normalizeAvailableGCodeMode(parsedSettings.data.mode.current),
      availableModes: getGCodeAgentAvailableModes(),
    },
    {
...
function mapSessionEvent(
  ...
  toolProjectionMemory?: GCodeToolProjectionMemory,
  backgroundTaskControlsByTaskKey?: Map<string, GCodeBackgroundTaskControlItem[]>,
  ...
  // GCode Protocol runtime trace 只在事件没有 inputId 时兜底，避免同一轮 chunk/tool/complete 被拆成不同 trace。
  ...
  if (eventInputId && eventInputId !== protocolTraceId) {
    logger.debug(
      eventInputId,
      `对齐 GCode prompt inputId eventType=${event.type} protocolTrace=${protocolTraceId}`,
    );
  }
  ...
  if (event.type === "turn.started") {
    ...
    const runStartedEvent: GCodeStreamEvent = {
    ...
    if (stringValue(payload.turnPhase) === "compact") {
      return [
        compactFailureToTimelineEvent(
          ...
          traceId,
          eventInputId,
          stringValue(errorPayload.message) ?? "GCode compact failed",
        ),
      ];
    }
    ...
    return [
      {
        ...
        traceId,
        ...(eventInputId ? { inputId: eventInputId } : {}),
        error: stringValue(errorPayload.message) ?? "GCode session failed",
...
): Extract<GCodeStreamEvent, { type: "session_info_update" }> | null {
  if (!hasActiveApiRetry || !isGCodeModelRetryRecoveryProgressPayload(payload)) {
    return null;
  }
...
function mapToolInputStreaming(
  ...
  if (kind === "tool_input_delta") {
    const state = appendGCodeStreamingToolInputDelta(
      streamingToolInputById?.get(toolId),
      stringValue(payload.delta) ?? "",
    );
    streamingToolInputById?.set(toolId, state);
    if (!shouldMaterializeGCodeStreamingToolInputPreview(state, { toolName })) {
    ...
    const preview = buildGCodeStreamingToolInputPreview(state.rawInput);
    markGCodeStreamingToolInputPreviewMaterialized(state);
  ...
  if (kind === "tool_call") {
    ...
    const preview = buildGCodeStreamingToolInputPreview(
      rawInput,
    ...
    if ((hasCompleteInput || preview.complete) && toolProjectionMemory) {
      finalizeGCodeToolProjectionInput(toolId, preview.input, toolProjectionMemory);
    }
...
function logStreamingToolInputProjection(
  ...
  logger.debug(traceId, "GCode streaming tool input projected", {
    ...details,
    event: "gcode.task.streaming_tool_input.projected",
  });
}
...
function mapCompactTimelinePayload(
  ...
  inputId: InputId | undefined,
  payload: Record<string, unknown>,
): Extract<GCodeStreamEvent, { type: "agent_message_chunk" }> | null {
...
function mapSyntheticTimelinePartPayload(
  ...
  const timeline = extractSyntheticTimelineFromTextPart(
    part as Extract<GCodeMessagePart, { type: "text" }>,
  );
  if (!timeline) {
    return null;
  }
...
function compactTimelineMetaFromPayload(
  payload: Record<string, unknown>,
  inputId: InputId | undefined,
): GCodeContextCompactionTimelineMeta | null {
...
function compactFailureToTimelineEvent(
  ...
  inputId: InputId | undefined,
  reason: string,
): Extract<GCodeStreamEvent, { type: "agent_message_chunk" }> {
  return {
    type: "agent_message_chunk",
    taskId,
    traceId,
...
function mapToolUpdated(
  ...
  const rememberedTool = resolveGCodeToolProjectionMetadata(payload, toolId, memory);
  ...
  if ("input" in payload && "toolName" in payload) {
    ...
    toolNameById?.set(toolId, toolName);
    finalizeGCodeToolProjectionInput(toolId, payload.input, memory);
    const toolEvent: GCodeStreamEvent = {
      type: "tool_call",
      taskId,
      traceId,
      ...(inputId ? { inputId } : {}),
  ...
  if ("result" in payload) {
    ...
    // GCode Protocol 的 ToolCallResult 只有 toolCallId/result，不再重复带 toolName。
    // 去掉 GCode Agent 后如果不记住前序 ToolCallScheduled 的 TodoWrite 名称，result 里的 todos
    ...
    if (status !== "in_progress") {
      forgetGCodeToolProjectionMetadata(toolId, memory);
    }
    const planSteps = isMainAgentToolProjectionSource(payload)
      ? extractPlanStepsFromToolOutput({
          title: toolName,
...
function parentToolUseIdFromToolPayload(payload: Record<string, unknown>): string | null {
  // GCode Protocol 发送的父级字段叫 parentToolCallId；
...
function permissionRequestToStreamEvent(
  taskId: string,
  request: GCodePermissionRequestParams,
): GCodePermissionRequest {
  return {
...
function pendingPermissionToStreamEvent(
  taskId: string,
  permission: GCodeSessionStateSnapshot["projection"]["pendingPermissions"][number],
): GCodePermissionRequest {
  return {
    type: "permission_request",
    taskId,
...
function userInputRequestToElicitationStreamEvent(
  taskId: string,
  request: GCodeUserInputRequestParams,
): GCodeStreamEvent {
  const questions =
...
): Extract<GCodeStreamEvent, { type: "elicitation_response" }> {
  return {
...
function pendingUserInputBackedPermissionToElicitationEvent(
  taskId: string,
  permission: GCodeSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<GCodeStreamEvent, { type: "elicitation_request" }> | null {
...
function pendingExitPlanModeToElicitationEvent(
  taskId: string,
  permission: GCodeSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<GCodeStreamEvent, { type: "elicitation_request" }> {
...
function setTaskBackgroundTaskControlCache(
  cache: Map<string, GCodeBackgroundTaskControlItem[]> | undefined,
  cacheKey: string,
  jobs: GCodeBackgroundTaskControlItem[],
) {
...
function updateTaskBackgroundTaskControlCacheFromPayload(
  ...
  cacheKey: string,
  payload: Record<string, unknown>,
): GCodeBackgroundTaskControlItem[] | null {
  const parsedJobs = parseGCodeBackgroundTaskControlItems([payload]);
  ...
  const nextJobs = mergeGCodeBackgroundTaskControlItems(cache.get(cacheKey) ?? [], parsedJobs);
...
function permissionOptionsFromPayload(payload: Record<string, unknown>): GCodePermissionOption[] {
  return Array.isArray(payload.options) ? (payload.options as GCodePermissionOption[]) : [];
}
...
function permissionResolvedPayloadToStreamEvents(
  ...
  } as Extract<GCodeStreamEvent, { type: "permission_response" }>;
...
function mapSessionInfoLikePayload(
  ...
  const networkDebugStatus = gcodeTaskNetworkDebugStatusFromPayload({
    taskId,
    traceId,
    ...(inputId ? { inputId } : {}),
    ...(eventId ? { eventId } : {}),
  ...
  if ("target" in payload && ("action" in payload || "source" in payload)) {
    events.push({
      ...
      target: {
        ...
        target: payload.target ? fromGCodeGoal(payload.target as never) : null,
        previousTarget: payload.previousTarget
          ? fromGCodeGoal(payload.previousTarget as never)
          : undefined,
      },
    });
  }
  ...
  if (Array.isArray(projection.backgroundJobs)) {
    const jobs = parseGCodeBackgroundTaskControlItems(projection.backgroundJobs);
    setTaskBackgroundTaskControlCache(
      backgroundTaskControlsByTaskKey,
...
function taskTokenUsageDeltaFromPayload(
  ...
  eventId: string | undefined,
  payload: Record<string, unknown>,
): Extract<GCodeStreamEvent, { type: "task_token_usage_delta" }> | null {
...
function apiRetryFromSessionInfoPayload(
  payload: Record<string, unknown>,
): GCodeApiRetryStatus | null | undefined {
  if ("apiRetry" in payload) {
    return normalizeGCodeApiRetryStatus(payload.apiRetry);
  }

  const runtimeRetry = normalizeGCodeApiRetryStatus(asRecord(payload.runtime).apiRetry);
  if (runtimeRetry !== undefined) {
    return runtimeRetry;
  }

  const metaRetry = normalizeGCodeApiRetryStatus(asRecord(asRecord(payload._meta).gcode).apiRetry);
  if (metaRetry !== undefined) {
    return metaRetry;
  }

  return (
    gcodeApiRetryFromStreamRecoveryPayload(payload) ??
    gcodeApiRetryFromModelNetworkStatusPayload(payload)
  );
}

function recordAgentModelNetworkTelemetry(event: GCodeSessionEvent): void {
...
function buildAgentModelNetworkInterface(payload: Record<string, unknown>): string {
  ...
  return `gcode_agent.model.${providerKind}.${transport}.${base}`;
}
...
type ContextUsageUpdate = Pick<
  Extract<GCodeStreamEvent, { type: "usage_update" }>,
  "used" | "size" | "cost" | "cache" | "breakdown"
>;

function contextUsageFromProjection(
  projection: GCodeSessionStateSnapshot["projection"],
...
function contextUsageFromRuntime(
  runtimeUsage: GCodeSessionStateSnapshot["runtime"]["contextUsage"] | undefined,
): ContextUsageUpdate | null {
...
function contextUsageFromPayload(payload: Record<string, unknown>): ContextUsageUpdate | null {
  ...
  // GCode Protocol 的 session.updated 里 contextUsed/contextWindow 是 projection 事实源；
...
function optionalContextUsageBreakdownFromPayload(
  ...
  const parsed = gcodeContextUsageBreakdownSchema.safeParse(payload.contextUsageBreakdown);
...
function usageFromPayload(value: unknown): GCodeUsage | undefined {
...
function timelineStatusValue(value: unknown): GCodeTimelineStatus | undefined {
...
function timelineTriggerValue(value: unknown): GCodeTimelineTrigger | undefined {
  return value === "manual" ||
    value === "auto" ||
    value === "reactive" ||
    value === "partial" ||
...
function turnSteerSourceValue(value: unknown): GCodeTurnSteerSource | undefined {
  return value === "plan_approval_feedback" || value === "workflow_refine_feedback"
    ? value
    : undefined;
}

function turnSteerCommandKindValue(value: unknown): GCodeTurnSteerCommandKind | undefined {
