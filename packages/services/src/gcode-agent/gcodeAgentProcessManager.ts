import { resolveGCodeAgentSpawnCwd } from "#src/gcode-agent/gcodeAgentSpawnCwd.js";
import type { GCodeAgentStorageStartupSnapshot } from "#src/gcode-agent/gcodeAgent.js";
/* eslint-disable max-lines -- gcodeAgentProcessManager 集中维护 agent 子进程启动、复用、超时回收和 runtime identity，拆分会扩大进程生命周期状态同步面 */
...
import {
  parseGCodeProcessDiagnostic,
  GCODE_AGENT_LIFECYCLE_LOG_MARKER,
  GCODE_PROCESS_DIAGNOSTIC_NAME_MAX_CHARS,
  GCODE_PROCESS_DIAGNOSTIC_MESSAGE_MAX_CHARS,
  GCODE_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS,
} from "@gcode/shared/process-diagnostic";
import {
  GCODE_AGENT_RUNTIME,
  GCODE_AGENT_PROVIDER,
  GCODE_RUNTIME_ENV_KEY,
  resolveWorkspaceKey,
  resolveGCodeRuntimeEnv,
  sanitizeGCodeRuntimeEnv,
} from "@gcode/shared";
import {
  findGCodeAgentRuntimeBinary,
  findGCodeAgentRuntimeNodeBundle,
...
import { GCodeProtocolClient } from "./gcodeProtocolClient.js";
import { GCodeStdioTransport } from "./gcodeStdioTransport.js";
import { readGCodeStdioTapDevState } from "./gcodeStdioTapDevConfig.js";
import type { GCodeAgentPresentationSurface } from "./gcodeAgentPresentationSurface.js";
...
export interface GCodeAgentCommand {
...
export interface GCodeAgentCommandResolverContext {
  presentationSurface?: GCodeAgentPresentationSurface;
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
}

export type GCodeAgentCommandResolver = (
  context: GCodeAgentCommandResolverContext,
) => Promise<GCodeAgentCommand | null> | GCodeAgentCommand | null;

export interface GCodeAgentProcessManagerOptions {
  commandResolver?: GCodeAgentCommandResolver;
  presentationSurface?: GCodeAgentPresentationSurface;
...
interface ManagedGCodeAgentProcess {
  ...
  runtimeIdentity: GCodeAgentRuntimeIdentity;
...
function buildE2EAgentCoverageEnv(env: NodeJS.ProcessEnv = process.env): Record<string, string> {
  ...
  if (env.GCODE_E2E_COVERAGE !== "1" || !artifactDir) {
    return {};
  }
  const directory = resolve(artifactDir, "coverage", "raw", "cli");
  mkdirSync(directory, { recursive: true });
  const preloadPath = resolve(directory, "gcode-e2e-coverage-preload.cjs");
 ...
 */
interface GCodeAgentRuntimeRestartedEvent {
  workspaceKey: string;
  runtimeIdentity: GCodeAgentRuntimeIdentity;
}

export interface GCodeAgentRuntimeLifecycleEvent {
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
  runtimeIdentity: GCodeAgentRuntimeIdentity;
...
interface GCodeAgentRuntimeIdentity {
  generation: number;
  identity: string;
  processId?: number;
  workspaceKey: string;
...
interface GCodeAgentSpawnPreflight {
  command: string;
  args: string[];
  requestedCwd: string;
  cwd: string;
...
const serviceLog = createServiceLogger("gcode-agent");
...
function parseArgsJson(raw: string | undefined): string[] | undefined {
  ...
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string")) {
    throw new Error("GCODE_AGENT_SERVER_ARGS_JSON must be a JSON string array");
  }
  return parsed;
}
...
async function buildGCodeAgentSpawnPreflight(
  command: GCodeAgentCommand,
  workspacePath: string,
  spawnFallbackCwd?: string,
): Promise<GCodeAgentSpawnPreflight> {
  ...
  const {
    ...
    cwdExists,
  } = await resolveGCodeAgentSpawnCwd({ requestedCwd, workspacePath, spawnFallbackCwd });
  return {
...
function resolveBundledWorkspaceGCodeAgentCommand(
  context: GCodeAgentCommandResolverContext,
): GCodeAgentCommand | null {
  const distEntrypoint = findUpward("apps/gcode-cli/packages/cli/dist/gcode.cjs");
  if (distEntrypoint) {
    const useBytecode =
      process.versions.electron && process.env.GCODE_DESKTOP_AGENT_BYTECODE === "1";
    const entrypoint = useBytecode
      ? join(dirname(distEntrypoint), "gcode.bytecode.cjs")
      : distEntrypoint;
    ...
    return {
      ...
      // 这里显式启用 Node 运行模式，避免内置 gcode-agent 被当成 Electron/Chromium 子进程启动并卡在 GPU 初始化。
      env: { ELECTRON_RUN_AS_NODE: "1" },
    };
  }

  const sourceEntrypoint = findUpward("apps/gcode-cli/packages/cli/src/main.ts");
...
function resolveDeployedGCodeAgentBinaryCommand(
  context: GCodeAgentCommandResolverContext,
): GCodeAgentCommand | null {
  // 旧 resolver 只识别 GCODE_AGENT_SERVER_COMMAND env 和 monorepo 源码树。
  // SSH 远端把 gcode-server.cjs 单文件部署到 ~/.gcode/server/，宿主进程的 cwd 不在仓库内、
  // env 也不会被 ssh exec 继承，即使 gcode-agent 已经部署到 ~/.gcode/server/agents/glm/，
  // resolver 也找不到，第一次 getClient 就抛 "GCode agent server command is not configured"。
  // 这里复用 findGCodeAgentRuntimeBinary 的候选链（含 GLM_BINARY_PATH env、
  // packagedResourcesPath、~/.gcode/server/agents/glm、bundled-agents 等），
  // 把已部署的原生 binary 当成最终兜底，远端/桌面打包形态都能命中。
  const binaryPath = findGCodeAgentRuntimeBinary();
  if (!binaryPath) {
    return null;
  }
  return {
    command: binaryPath,
    args: GCODE_AGENT_RUNTIME.spawnArgs,
    cwd: context.workspacePath,
  };
}

function resolveElectronRuntimeGCodeAgentCommand(
  context: GCodeAgentCommandResolverContext,
): GCodeAgentCommand | null {
  // 桌面打包态：host 跑在 Electron utility process 里，process.execPath 指向 Electron Helper，
  // 它内置的 Node runtime 与 gcode-cli 目标版本一致（Electron 41 = Node 24.x）。
  // 这里直接用 app 自带的 Electron Node 执行打进 resources/glm 的 gcode.cjs，
  ...
  const bundlePath = findGCodeAgentRuntimeNodeBundle();
  if (!bundlePath) {
    return null;
  }
  return {
    command: process.execPath,
    args: [bundlePath, ...GCODE_AGENT_RUNTIME.spawnArgs],
...
export function resolveDefaultGCodeAgentCommand(
  context: GCodeAgentCommandResolverContext,
): GCodeAgentCommand | null {
  const command = process.env.GCODE_AGENT_SERVER_COMMAND?.trim();
  if (command) {
    return applyPresentationSurfaceToCommand(
      {
        command,
        args: parseArgsJson(process.env.GCODE_AGENT_SERVER_ARGS_JSON) ?? ["app-server", "--stdio"],
        cwd: process.env.GCODE_AGENT_SERVER_CWD?.trim() || context.workspacePath,
      },
  ...
  // 抢先匹配）→ 桌面打包态 Electron Node runtime 跑 gcode.cjs → 已部署 native binary（远端 SSH 兜底）。
  const bundled =
    resolveBundledWorkspaceGCodeAgentCommand(context) ??
    resolveElectronRuntimeGCodeAgentCommand(context);
  return applyPresentationSurfaceToCommand(
    bundled
      ? { ...bundled, supportsStorageStartup: true }
      : resolveDeployedGCodeAgentBinaryCommand(context),
    context.presentationSurface,
  );
}

function applyPresentationSurfaceToCommand(
  command: GCodeAgentCommand | null,
  presentationSurface: GCodeAgentCommandResolverContext["presentationSurface"],
): GCodeAgentCommand | null {
...
function wrapGCodeAgentCommandWithStdioTapDevProxy(
  command: GCodeAgentCommand,
  workspaceKey: string,
): GCodeAgentCommand {
  const tapState = readGCodeStdioTapDevState();
  if (!tapState.enabled) {
    return command;
  }

  const tapScript = findUpward("scripts/dev/gcode-stdio-tap.mjs");
  if (!tapScript) {
    debugLog("GCode stdio tap proxy enabled but script not found");
    return command;
  }
...
export class GCodeAgentProcessManager {
  private readonly processesByWorkspaceKey = new Map<string, ManagedGCodeAgentProcess>();
  private readonly ownedProcesses = new Set<ManagedGCodeAgentProcess>();
  private readonly startingByWorkspaceKey = new Map<string, Promise<GCodeProtocolClient>>();
  ...
  private readonly storageStartupEmitter = new Emitter<{
    workspaceKey: string;
    snapshot: GCodeAgentStorageStartupSnapshot;
  }>();
  ...
  getStorageStartupState(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): GCodeAgentStorageStartupSnapshot | null {
  ...
  private readonly commandResolver: GCodeAgentCommandResolver;
  private readonly presentationSurface: GCodeAgentProcessManagerOptions["presentationSurface"];
  private readonly requestTimeoutMs: number | undefined;
  private readonly processLifecycleReporter: RuntimeProcessLifecycleReporter | undefined;
  private readonly resolveSpawnEnv: GCodeAgentProcessManagerOptions["resolveSpawnEnv"];
  private readonly waitForSpawnAdmission: GCodeAgentProcessManagerOptions["waitForSpawnAdmission"];
  ...
  private readonly runtimeRestartedEmitter = new Emitter<GCodeAgentRuntimeRestartedEvent>();
  private readonly runtimeLifecycleEmitter = new Emitter<GCodeAgentRuntimeLifecycleEvent>();
  private disposeAllInFlight: Promise<void> | undefined;
  private disposed = false;

  /** 进程换代通知（generation>1 时触发）；v4 订阅方据此重订，见 GCodeAgentRuntimeRestartedEvent。 */
  ...
  constructor(options?: GCodeAgentProcessManagerOptions) {
    this.commandResolver = options?.commandResolver ?? resolveDefaultGCodeAgentCommand;
  ...
  private reportProcessLifecycle(
    ...
    try {
      ...
      warnLog("GCode agent process lifecycle reporter failed", error);
    }
  }

  private clearIdleTimer(managed: ManagedGCodeAgentProcess): void {
   ...
   */
  private scheduleIdleReclaim(workspaceKey: string, managed: ManagedGCodeAgentProcess): void {
    ...
    const timer = setTimeout(() => {
      ...
      log("GCode agent process idle timeout; reclaiming", {
        workspaceKey,
  ...
  private recordTerminationIntent(
    managed: ManagedGCodeAgentProcess,
  ...
  private reportRuntimeUnavailable(managed: ManagedGCodeAgentProcess): void {
  ...
  private reportRuntimeReady(managed: ManagedGCodeAgentProcess): void {
    if (
      !managed.spawned ||
      managed.exited ||
  ...
  private cleanupManagedProcess(
    ...
    const cleanupPromise = managed.client
      ...
      .then(() => {
        ...
        log("GCode agent process cleanup completed", {
    ...
    if (options.reportError !== false) {
      void cleanupPromise.catch((error) => {
        errorLog("GCode agent process cleanup failed", {
          workspaceKey: managed.runtimeIdentity.workspaceKey,
          pid: managed.child.pid,
          runtimeIdentity: managed.runtimeIdentity.identity,
          reason,
  ...
  private async cleanupManagedProcessForShutdown(managed: ManagedGCodeAgentProcess): Promise<void> {
  ...
  private async cleanupManagedProcessWithRetry(
    ...
    try {
      ...
      warnLog(`GCode agent process cleanup retrying during ${retryScope}`, {
        workspaceKey: managed.runtimeIdentity.workspaceKey,
        pid: managed.child.pid,
        runtimeIdentity: managed.runtimeIdentity.identity,
        cleanupStage: reason,
  ...
  async getClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<GCodeProtocolClient> {
    if (this.disposed) {
      throw new Error("GCode agent process manager is disposed.");
    }
    ...
    if (starting) {
      ...
      log("GCode agent process start already in progress", {
        workspaceKey,
      });
      const client = await starting;
      log("GCode agent process start wait completed", {
        workspaceKey,
  ...
  private async startClient(
    ...
    if (!command) {
      throw new Error(
        "GCode agent server command is not configured. Set GCODE_AGENT_SERVER_COMMAND before integration.",
      );
    }
    if (admissionSignal.aborted) {
      throw admissionSignal.reason ?? new Error("GCode agent process start was cancelled.");
    }
    const effectiveCommand = wrapGCodeAgentCommandWithStdioTapDevProxy(command, workspaceKey);
    log("GCode agent command resolved", {
      workspaceKey,
      command: command.command,
    ...
    if ((this.restartGenerationByWorkspaceKey.get(workspaceKey) ?? 0) !== startGeneration) {
      ...
      throw new Error("GCode agent process start was cancelled.");
    }
    // cwd 探测也让出事件循环，必须放在最终 admission 与销毁/代际检查之前。
    const spawnPreflight = await buildGCodeAgentSpawnPreflight(
      effectiveCommand,
    ...
    // app 以本地开发方式启动时，让 agent 子进程也带上 GCODE_RUNTIME_ENV=development；
    // 不再传 NODE_ENV，避免用户 shell/runtime 变量影响 GCode 运行模式或泄漏到 Bash 工具。
    const runtimeEnv = resolveGCodeRuntimeEnv(process.env);
    log("GCode agent spawn preflight", {
      workspaceKey,
      spawnPreflight,
    });
    ...
    const child = spawn(effectiveCommand.command, spawnPreflight.args, {
      ...
      env: {
        ...sanitizeGCodeRuntimeEnv(process.env),
        [GCODE_RUNTIME_ENV_KEY]: runtimeEnv,
        ...spawnEnv,
    ...
    const transport = new GCodeStdioTransport(child, {
      onStderrLine: (line) => {
        const diagnostic = parseGCodeProcessDiagnostic(line);
    ...
    const client = new GCodeProtocolClient(transport, {
    ...
    const runtimeIdentity: GCodeAgentRuntimeIdentity = {
    ...
    const managed: ManagedGCodeAgentProcess = {
      child,
      client,
      exited: false,
      readyReported: false,
    ...
    child.once("spawn", () => {
      ...
      this.reportRuntimeReady(managed);
      log("GCode agent process started", {
        workspaceKey,
        command: effectiveCommand.command,
        cwd: spawnPreflight.cwd,
        pid: child.pid,
    ...
    child.once("error", (error) => {
      errorLog(
        `GCode agent process error${this.processLifecycleReporter?.onError ? ` ${GCODE_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
        {
          workspaceKey,
          pid: child.pid,
    ...
    child.once("exit", async (code, signal) => {
      ...
      log("GCode agent process exited", exitContext);
      if (terminationKind === "unexpected") {
        ...
        errorLog(
          `GCode agent process exited unexpectedly${this.processLifecycleReporter ? ` ${GCODE_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
          {
            ...exitContext,
            stderr,
          },
    ...
    client.onRequestTimeout((event) => {
      ...
      if (event.method === "workspace/cancelGenerateText") {
        warnLog(
          "GCode agent cancel notification timed out; keeping client (best-effort control plane)",
          {
            workspaceKey,
            method: event.method,
      ...
      warnLog("GCode agent request timed out; disposing stale protocol client", {
        workspaceKey,
        method: event.method,
        requestId: event.requestId,
        timeoutMs: event.timeoutMs,
    ...
    client.onClose(() => {
      ...
      log("GCode agent protocol client closed", {
        workspaceKey,
        pid: child.pid,
        runtimeIdentity: runtimeIdentity.identity,
        wasActiveClient,
  ...
  async getRuntimeIdentity(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<GCodeAgentRuntimeIdentity> {
    const workspaceKey = resolveWorkspaceKey(params);
    ...
    if (!managed || managed.exited || managed.child.killed) {
      throw new Error("GCode agent runtime identity is unavailable.");
    }
    ...
    try {
      ...
      return command
        ...
        : {
            available: false,
            workspaceKey,
            reason: "GCODE_AGENT_SERVER_COMMAND is not configured",
          };
    } catch (error) {
      return {
        available: false,
  ...
  private abortPendingStarts(
    workspaceKey: string,
    reason = new Error("GCode agent process start was cancelled."),
  ): void {
  ...
  private abortAllPendingStarts(
    reason = new Error("GCode agent process manager is disposed."),
  ): void {
  ...
  async disposeAllAndWait(): Promise<void> {
    ...
    // 依赖 unref timer，host 自己退出后 timer 不会再执行，gcode-cli 会残留为孤儿进程。
