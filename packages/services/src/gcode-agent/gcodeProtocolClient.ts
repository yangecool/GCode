import { GCodeStorageStartupGate } from "#src/gcode-agent/gcodeStorageStartupGate.js";
...
import type { V4Method } from "@gcode/shared/gcode-protocol-v4";
import type { z } from "zod";
import type { GCodeProtocolTransport } from "./gcodeProtocolTransport.js";

/** 客户端可发的方法名：旧 gcodeProtocolMethods + v4/*（并存，收敛为 v4）。 */
type GCodeProtocolClientMethod = GCodeProtocolMethod | V4Method;

interface GCodeProtocolClientOptions {
  requireStorageStartup?: boolean;
  requestTimeoutMs?: number;
}

interface GCodeProtocolRequestTimeoutEvent {
  method: GCodeProtocolClientMethod;
  requestId: GCodeProtocolRequestId;
  timeoutMs: number;
}
...
interface GCodeProtocolClientRequestOptions {
...
const DEFAULT_GCODE_PROTOCOL_REQUEST_TIMEOUT_MS = 3 * 60_000;

class GCodeProtocolClientError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = "GCodeProtocolClientError";
  }
}

export class GCodeProtocolRequestTimeoutError extends Error {
  constructor(
    readonly method: GCodeProtocolClientMethod,
    readonly requestId: GCodeProtocolRequestId,
    readonly timeoutMs: number,
  ) {
    super(`GCode Protocol request timed out: ${method}`);
    this.name = "GCodeProtocolRequestTimeoutError";
  }
}

export class GCodeProtocolClient implements IDisposable {
  readonly storageStartup: GCodeStorageStartupGate;
  private readonly pending = new Map<string, PendingRequest<unknown>>();
  private readonly notificationEmitter = new Emitter<GCodeProtocolNotification>();
  private readonly requestEmitter = new Emitter<GCodeProtocolRequest>();
  private readonly requestTimeoutEmitter = new Emitter<GCodeProtocolRequestTimeoutEvent>();
   ...
   * 否则会立即抛 "GCode Protocol client is disposed"。
   */
  ...
  constructor(
    private readonly transport: GCodeProtocolTransport,
    options?: GCodeProtocolClientOptions,
  ) {
    this.storageStartup = new GCodeStorageStartupGate(options?.requireStorageStartup ?? false);
    this.requestTimeoutMs = options?.requestTimeoutMs ?? DEFAULT_GCODE_PROTOCOL_REQUEST_TIMEOUT_MS;
    this.disposables.push(
      transport.onMessage((message) => this.handleMessage(message)),
      transport.onClose((event) => {
        this.storageStartup.dispose();
        const suffix = event.reason ? `: ${event.reason}` : "";
        this.rejectAll(new Error(`GCode agent transport closed${suffix}`));
  ...
  async request<T = unknown>(
    ...
    const resultPromise = new Promise<T>((resolve, reject) => {
      const expire = () => {
        ...
        const error = new GCodeProtocolRequestTimeoutError(method, id, requestTimeoutMs);
  ...
  async notify(method: GCodeProtocolClientMethod, params?: unknown): Promise<void> {
    this.assertNotDisposed();
    await this.transport.send({ method, params });
  }

  async respond(id: GCodeProtocolRequestId, result: unknown): Promise<void> {
    this.assertNotDisposed();
  ...
  private handleMessage(message: unknown): void {
    ...
    if ("result" in message && "id" in message) {
      this.resolveResponse(
        (message as { id: GCodeProtocolRequestId; result: unknown }).id,
    ...
    if ("method" in message && "id" in message) {
      this.requestEmitter.fire(message as GCodeProtocolRequest);
      return;
    }
    ...
    if ("method" in message) {
      if (
        ...
        this.storageStartup.accept((message as GCodeProtocolNotification).params)
      ) {
      ...
      this.notificationEmitter.fire(message as GCodeProtocolNotification);
    }
  }

  private resolveResponse(id: GCodeProtocolRequestId, result: unknown): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
    ...
    try {
      ...
      pending.reject(
        error instanceof Error
          ? error
          : new Error(`GCode Protocol response parse failed: ${pending.method}`),
      );
    }
  }

  private rejectResponse(id: GCodeProtocolRequestId, error: Error): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
  ...
  private disposeLocalResources(): void {
    this.rejectAll(new Error("GCode Protocol client disposed"));
  ...
  private assertNotDisposed(): void {
    if (this.disposed) {
      throw new Error("GCode Protocol client is disposed");
    }
  }
}
