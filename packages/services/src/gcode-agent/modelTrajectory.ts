...
} from "#src/session/gcodeTaskService.js";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import {
  readTrajectoryFileTail,
  resolveModelIODirs,
  sanitizeSessionSegment,
} from "#src/gcode-agent/modelTrajectoryFileTail.js";
import type { TrajectoryFileTail } from "#src/gcode-agent/modelTrajectoryFileTail.js";
...
/**
 * 解析 ~/.gcode/cli/{debug,rollout} 下的 model-io JSONL，按 sessionId 还原某个 task 的模型调用轨迹。
 *
 * 设计说明：
 * - model-io 由 adapters/model/runner-debug.ts 落盘；一个 session 一个
 *   `model-io-<sanitizedSessionId>.jsonl`。
 * - GCode Agent 把 taskId 当作 sessionId（见 gcodeTaskServiceAdapter），所以这里只读取
...
function mapRecord(record: Record<string, unknown>): GCodeModelTrajectoryRecord {
  ...
  const mapped: GCodeModelTrajectoryRecord = {
...
function mapMessages(value: unknown): GCodeModelTrajectoryMessage[] {
...
function mapResponseToolCall(rawToolCall: unknown): GCodeModelTrajectoryContentPart {
...
function mapUsage(value: unknown): GCodeModelTrajectoryUsage | undefined {
