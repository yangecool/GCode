...
const GCODE_PLUGIN_MANIFEST_PATH = [".gcode-plugin", "plugin.json"] as const;
...
function getWorkspaceGcodeCommandRoot(workspacePath: string): string {
  return join(workspacePath, ".gcode", "commands");
}

function getUserGcodeCommandRoot(): string {
  return join(resolveUserHomeDir(), ".gcode", "commands");
}

function getWorkspaceGcodePluginRoot(workspacePath: string): string {
  return join(workspacePath, ".gcode", "plugins");
}
...
function getUserGcodeCliConfigPath(): string {
  return join(resolveUserHomeDir(), ".gcode", "cli", "config.json");
}

function getWorkspaceGcodeConfigPath(workspacePath: string): string {
  return join(workspacePath, ".gcode", "config.json");
}
...
function getUserGcodeAgentsFilePath(): string {
  return join(resolveUserHomeDir(), ".gcode", "AGENTS.md");
}
...
function resolveTargetRootForScope(
  ...
  if (targetScope === "global") {
    return getUserGcodeSkillRoot();
  }
  return workspacePath ? getWorkspaceGcodeSkillRoot(workspacePath) : null;
}
...
function resolveCandidateTargetRoot(
  ...
  return sourceScope === "workspace"
    ? workspacePath
      ? getWorkspaceGcodeSkillRoot(workspacePath)
      : null
...
function resolveCommandTargetRootForScope(
  ...
  if (targetScope === "global") {
    return getUserGcodeCommandRoot();
  }
  return workspacePath ? getWorkspaceGcodeCommandRoot(workspacePath) : null;
}
...
function resolveCommandCandidateTargetRoot(
  ...
  return sourceScope === "workspace"
    ? workspacePath
      ? getWorkspaceGcodeCommandRoot(workspacePath)
      : null
    : getUserGcodeCommandRoot();
}
...
function resolvePluginTargetRootForScope(
  ...
  if (targetScope === "global") {
    return getUserGcodePluginRoot();
  }
  return workspacePath ? getWorkspaceGcodePluginRoot(workspacePath) : null;
}
...
function resolvePluginCandidateTargetRoot(
  ...
  return sourceScope === "workspace"
    ? workspacePath
      ? getWorkspaceGcodePluginRoot(workspacePath)
      : null
...
function resolvePluginConfigPathForScope(
  ...
  if (targetScope === "global") {
    return getUserGcodeCliConfigPath();
  }
  return workspacePath ? getWorkspaceGcodeConfigPath(workspacePath) : null;
}
...
async function findPluginManifestPath(pluginPath: string): Promise<string | null> {
  const gcodeManifestPath = join(pluginPath, ...GCODE_PLUGIN_MANIFEST_PATH);
  if (await pathExists(gcodeManifestPath)) {
    return gcodeManifestPath;
  }
...
function readGcodeMcpServers(parsed: Record<string, unknown>): Record<string, McpServerConfig> {
...
async function collectExistingMcpServerNameKeys(
  ...
  if (targetConfigPath) {
    for (const name of Object.keys(
      readGcodeMcpServers(await readJsonFileOrEmpty(targetConfigPath)),
    )) {
...
async function addMcpServerToGcodeConfig(
  filePath: string,
  name: string,
  config: McpServerConfig,
): Promise<void> {
  ...
  const servers = readGcodeMcpServers(parsed);
...
async function importMcpServersForAgent(
  ...
  for (const candidate of candidates) {
    ...
    try {
      await addMcpServerToGcodeConfig(selectedConfigPath, candidate.name, candidate.config);
      importedCount += 1;
...
async function getClaudeAgentsFileMigrationStatus(): Promise<SettingsSyncClaudeAgentsFileMigrationStatus> {
  ...
  const targetPath = getUserGcodeAgentsFilePath();
...
async function copyClaudeAgentsFileToGcodeAgentsFile(params: {
  overwrite?: boolean;
...
export function createSettingsSyncService(
  ...
  return {
    ...
    async copyClaudeAgentsFileToGcodeAgentsFile(
      request = {},
    ): Promise<SettingsSyncClaudeAgentsFileCopyResult> {
      const result = await copyClaudeAgentsFileToGcodeAgentsFile({
