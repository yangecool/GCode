/* oxlint-disable eslint(max-lines) -- 已发布旧 GCode config.json 的多版 Provider 结构读取集中在同一边界。 */
...
function normalizeBigModelCodingPlanAnthropicBaseUrlForEnv(
  ...
  const fallbackBaseUrl =
    resolveRuntimeGCodeEnv(env) === "production"
  ...
  if (resolveRuntimeGCodeEnv(env) === "production") {
...
function getGCodeConfigFilePath(): string {
...
interface GCodeConfigFile {
  $schema?: string;
  provider?: Record<string, GCodeOpenCodeProviderConfig>;
  [key: string]: unknown;
}

interface GCodeOpenCodeProviderConfig {
  api?: string;
  name?: string;
  env?: string[];
  id?: string;
  ...
  createdAt?: number;
  updatedAt?: number;
  models?: Record<string, GCodeOpenCodeModelConfig>;
  gcode?: GCodeProviderConfigExtension;
  [key: string]: unknown;
}

interface GCodeOpenCodeModelConfig {
  id?: string;
  name?: string;
  family?: string;
  release_date?: string;
  attachment?: boolean;
  reasoning?: boolean | GCodeReasoningConfigExtension;
  temperature?: boolean;
  tool_call?: boolean;
  interleaved?: unknown;
  cost?: JsonObject;
  ...
  modified?: boolean;
  deleted?: boolean;
  gcode?: GCodeModelConfigExtension;
  [key: string]: unknown;
}

interface GCodeReasoningConfigExtension {
  enabled?: boolean;
  variants?: string[];
...
interface GCodeProviderConfigExtension {
  enabled?: boolean;
  systemDisabledReason?: ModelProviderSystemDisabledReason;
  endpoints?: ModelProviderEndpoints;
  apiFormat?: ModelProviderApiFormat;
...
interface GCodeModelConfigExtension {
  kinds?: ModelProviderKind[];
  defaultKind?: ModelProviderKind;
  modelIdByKind?: Partial<Record<ModelProviderKind, string>>;
  disabledReason?: string;
...
const gcodeModelProviderModalitySchema = z.enum(["text", "image", "video", "audio", "pdf"]);
const gcodeProviderModelMappingsSchema = z.record(z.string(), z.unknown());

const gcodeReasoningConfigExtensionSchema = z
  .object({
...
const gcodeModelConfigExtensionSchema = z
  .object({
...
const gcodeOpenCodeModelConfigSchema: z.ZodType<GCodeOpenCodeModelConfig> = z
  .object({
    id: z.string().optional(),
    name: z.string().optional(),
    reasoning: z.union([z.boolean(), gcodeReasoningConfigExtensionSchema]).optional(),
    limit: z
      .object({
    ...
    modalities: z
      .object({
        input: z.array(gcodeModelProviderModalitySchema).optional(),
        output: z.array(gcodeModelProviderModalitySchema).optional(),
      })
      .optional(),
    ...
    modified: z.boolean().optional(),
    deleted: z.boolean().optional(),
    gcode: gcodeModelConfigExtensionSchema.optional(),
  })
  .passthrough();

const gcodeProviderConfigExtensionSchema = z
  .object({
    enabled: z.boolean().optional(),
    ...
    defaultKind: modelProviderKindSchema.optional(),
    providerMappings: gcodeProviderModelMappingsSchema.optional(),
...
const gcodeOpenCodeProviderConfigSchema: z.ZodType<GCodeOpenCodeProviderConfig> = z
  .object({
    ...
    createdAt: z.number().optional(),
    updatedAt: z.number().optional(),
    models: z.record(z.string(), gcodeOpenCodeModelConfigSchema).optional(),
    gcode: gcodeProviderConfigExtensionSchema.optional(),
  })
  .passthrough();

const gcodeConfigFileSchema = z
  .object({
    $schema: z.string().optional(),
    provider: z.record(z.string(), gcodeOpenCodeProviderConfigSchema).optional(),
  })
  .passthrough();
...
function inferModelProviderKindFromOpenCodeProvider(
  provider: GCodeOpenCodeProviderConfig,
): ModelProviderKind {
  if (provider.kind) {
...
function hasOpenCodeProviderRuntimeFields(provider: GCodeOpenCodeProviderConfig): boolean {
  // config.json 的权威运行态字段是 kind/options.baseURL。
  // 旧 endpoints/apiFormat/defaultKind/gcode 可能是历史迁移残留，若继续优先读取会覆盖新配置。
  return Boolean(
    provider.kind ||
...
function resolvePresetProviderKindFromRuntimeBaseUrl(
  ...
  if (path.includes("/api/anthropic") || path.includes("/gcode-plan/anthropic")) {
    return "anthropic";
  }
  return undefined;
}
...
function resolveOpenCodeProviderEndpoints(
  ...
  return provider.endpoints ?? provider.gcode?.endpoints ?? {};
}

function stripGCodePrefixedProviderMappings(
  ...
  const entries = Object.entries(providerMappings ?? {}).filter(
    ([key]) => !key.toLowerCase().startsWith("gcode"),
  );
  return Object.fromEntries(entries);
}

function openCodeReasoningToModelReasoning(
  model: GCodeOpenCodeModelConfig,
  ...
  if (!options.preferOpenCodeFields && model.gcode?.reasoning) {
    return stripModelProviderReasoningPatches(model.gcode.reasoning);
  }
...
function openCodeModelToModelProviderModel(
  ...
  const gcode = options.preferOpenCodeFields ? undefined : model.gcode;
  const gcodeExtensions = model.gcode;
  const contextWindow =
  ...
  return createModelProviderModelConfig({
    ...
    priority: readFiniteNumber(model.priority) ?? readFiniteNumber(gcodeExtensions?.priority),
    disabledReason: model.disabledReason ?? gcode?.disabledReason,
    supportsTools: model.supportsTools ?? gcode?.supportsTools,
    supportsStructuredOutput: model.supportsStructuredOutput ?? gcode?.supportsStructuredOutput,
    modified: model.modified ?? gcodeExtensions?.modified,
    deleted: model.deleted ?? gcodeExtensions?.deleted,
  });
}
...
function openCodeProviderToModelProviderConfig(
  ...
  for (const deletedModelId of normalizeDeletedModelIds(gcode?.deletedModels)) {
  ...
  const apiKeyRequired =
    provider.apiKeyRequired ?? gcode?.apiKeyRequired ?? readBoolean(options.apiKeyRequired);
  const providerMappings = preferOpenCodeFields
    ? undefined
    : stripGCodePrefixedProviderMappings(provider.providerMappings ?? gcode?.providerMappings);
  return normalizeProviderForStore({
    ...
    ...(provider.enabled !== undefined
      ...
      : gcode?.enabled !== undefined
        ? { enabled: gcode.enabled }
        : {}),
    ...((provider.systemDisabledReason ?? gcode?.systemDisabledReason)
      ? {
          systemDisabledReason: provider.systemDisabledReason ?? gcode?.systemDisabledReason,
        }
      : {}),
    endpoints,
    apiFormat: preferOpenCodeFields
      ? resolveModelProviderKindApiFormat(defaultKind)
      : (provider.apiFormat ?? gcode?.apiFormat ?? resolveModelProviderKindApiFormat(defaultKind)),
    ...((provider.source ?? gcode?.source)
      ? { source: provider.source ?? gcode?.source }
      : { source: "custom" }),
    ...((provider.catalogSourceId ?? gcode?.catalogSourceId)
      ? { catalogSourceId: provider.catalogSourceId ?? gcode?.catalogSourceId }
      : {}),
    ...((provider.catalogProviderId ?? gcode?.catalogProviderId)
      ? {
          catalogProviderId: provider.catalogProviderId ?? gcode?.catalogProviderId,
        }
      : {}),
    ...((provider.modelsDevProviderId ?? gcode?.modelsDevProviderId)
      ? {
          modelsDevProviderId: provider.modelsDevProviderId ?? gcode?.modelsDevProviderId,
        }
      : {}),
    ...(apiKeyRequired !== undefined ? { apiKeyRequired } : {}),
    ...((provider.headers ?? gcode?.headers ?? readStringRecord(options.headers))
      ? {
          headers: provider.headers ?? gcode?.headers ?? readStringRecord(options.headers),
        }
      : {}),
    ...((provider.logoUrl ?? gcode?.logoUrl)
      ? { logoUrl: provider.logoUrl ?? gcode?.logoUrl }
      : {}),
    apiKey,
    ...((provider.apiKeyUrl ?? gcode?.apiKeyUrl)
      ? { apiKeyUrl: provider.apiKeyUrl ?? gcode?.apiKeyUrl }
      : {}),
    defaultKind,
    models,
    ...(providerMappings ? { providerMappings } : {}),
    createdAt:
      (!preferOpenCodeFields ? (provider.createdAt ?? gcode?.createdAt) : undefined) ?? now,
    updatedAt:
      (!preferOpenCodeFields ? (provider.updatedAt ?? gcode?.updatedAt) : undefined) ?? now,
  });
}

async function readRawGCodeConfigFile(): Promise<GCodeConfigFile | null> {
  const filePath = getGCodeConfigFilePath();
  let raw: string;
  try {
  ...
  const parsed = gcodeConfigFileSchema.safeParse(json);
  if (!parsed.success) {
...
async function readGCodeConfigProviders(): Promise<ModelProviderConfig[] | null> {
  const config = await readRawGCodeConfigFile();
  if (!config?.provider) {
    return null;
  }
...
function normalizeProviderForStore(provider: ModelProviderConfig): ModelProviderConfig {
  ...
  return {
    ...
    providerMappings: stripGCodePrefixedProviderMappings(
...
function applyProviderStoreMigrations(providers: ModelProviderConfig[]): ModelProviderConfig[] {
  ...
  // reasoning mapping 由 GCode Built-in/Personal ModelConfigRules 在 Registry 中解析；
...
export async function readLegacyGCodeConfigProviders(): Promise<ModelProviderConfig[]> {
  const configProviders = await readGCodeConfigProviders();
