...
import { DATA_BASE_DIR_FORBIDDEN_WINDOWS_INSTALL_DIR_ERROR_CODE } from "@gcode/shared";

let _dataBaseDir: string | null = null;
export const GCODE_WINDOWS_APP_INSTALL_DIR_ENV = "GCODE_WINDOWS_APP_INSTALL_DIR";
const envDataBaseDir = process.env.GCODE_DATA_BASE_DIR?.trim() || null;
...
/** Get the current base directory. Priority: setDataBaseDir() > env GCODE_DATA_BASE_DIR > homedir(). */
...
/** {dataBaseDir}/.gcode */
export function getGCodeDataRootDir(): string {
  return join(getDataBaseDir(), ".gcode");
}

/** 非项目对话共享的真实工作目录；默认 ~/.gcode/workspace/default。 */
export function getConversationWorkspaceDir(): string {
  return join(getGCodeDataRootDir(), "workspace", "default");
}

/** {dataBaseDir}/.gcode/v2 */
export function getAppConfigDir(): string {
  return join(getGCodeDataRootDir(), "v2");
}
...
function collectWindowsForbiddenAppInstallDirs(
  ...
  const candidates = [
    ...
    readEnvValue(env, GCODE_WINDOWS_APP_INSTALL_DIR_ENV),
    programFiles ? win32.join(programFiles, "GCode") : null,
    programFilesX86 ? win32.join(programFilesX86, "GCode") : null,
    programW6432 ? win32.join(programW6432, "GCode") : null,
    localAppData ? win32.join(localAppData, "Programs", "GCode") : null,
  ];
...
export function getExportLogStageDir(): string {
  return join(getGCodeDataRootDir(), "export-log-stage");
}

export function getExportLogDir(): string {
  return join(getGCodeDataRootDir(), "export-log");
}

export function getFeedbackRootDir(): string {
  return join(getGCodeDataRootDir(), "feedback");
}
...
export function getGitCheckpointIndexRootDir(): string {
  return join(getGCodeDataRootDir(), "git-checkpoint-index");
}

/** ~/.gcode/v2/tasks-index.sqlite */
...
/** 与 GCode session 持久化一致：使用 workspaceKey 的 SHA-256 前 12 位 */
...
/** ~/.gcode/v2/sessions/{workspaceHash} */
...
/** ~/.gcode/v2/sessions/{workspaceHash}/{taskId}.json */
...
/** ~/.gcode/v2/sessions/{workspaceHash}/{taskId}.deleted.json */
...
/**
 * Copy the .gcode/v2 data directory from one base dir to another.
...
export async function copyDataDirectory(oldBaseDir: string, newBaseDir: string): Promise<void> {
  const oldDir = join(oldBaseDir, ".gcode", "v2");
  const newDir = join(newBaseDir, ".gcode", "v2");
