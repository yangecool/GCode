...
import type { GCodeAgentMcpServer } from "@gcode/shared";
...
function isFilesystemServer(
  server: GCodeAgentMcpServer,
): server is Extract<GCodeAgentMcpServer, { command: string }> {
  return (
    "command" in server &&
...
export function appendWorkspaceToFilesystemMcpServers(
  mcpServers: GCodeAgentMcpServer[] | undefined,
  workspacePath: string,
): GCodeAgentMcpServer[] | undefined {
