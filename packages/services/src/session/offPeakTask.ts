...
export interface IOffPeakTaskService {
  ...
  createTask(params: GCodeOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult>;
  cancelTask(offPeakTaskId: string): Promise<GCodeOffPeakTask | null>;
  pauseTask(offPeakTaskId: string): Promise<GCodeOffPeakTask | null>;
  continueTask(offPeakTaskId: string): Promise<GCodeOffPeakTask | null>;
  deleteTask(offPeakTaskId: string): Promise<void>;
  /** 仅隐藏本地 History 行；不删除 task/session/执行字段。 */
  deleteHistory(offPeakTaskId: string): Promise<GCodeOffPeakTask | null>;
  updateTask(
    offPeakTaskId: string,
    params: OffPeakUpdateTaskParams,
  ): Promise<GCodeOffPeakTask | null>;
  list(): Promise<GCodeOffPeakTask[]>;
  get(offPeakTaskId: string): Promise<GCodeOffPeakTask | null>;
}
