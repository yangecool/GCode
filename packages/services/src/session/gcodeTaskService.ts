/* eslint-disable max-lines -- GCode task wrapper service 接口集中承载 app/runtime API，拆散会让替换阶段更难追踪。 */
import type { Event } from "@gcode/rpc";
import { ServiceChannels } from "@gcode/shared";
import type { CommandPayloadMap } from "@gcode/shared/gcode-protocol-v4";
...
import type {
  ...
  GCodeTaskCreateResult,
  GCodeTaskMeta,
  GCodeStreamEvent,
  GCodeTaskMode,
  GCodeConfigOption,
  GCodeError,
  GCodeAssistantMessageFeedback,
  GCodePromptAttachment,
  GCodeProvider,
  GCodeWorkspaceEvent,
  TraceId,
  GCodeSessionFile,
  GCodeAgentMcpServer,
  GCodeTaskSnapshot,
  GCodeTaskSnapshotBody,
  GCodeTaskSnapshotToolCallsSlice,
  GCodeTaskSnapshotRefContent,
  GCodeEnqueueTaskCommandResult,
  GCodeCancelTaskCommandResult,
  GCodeTaskClientMode,
  GCodeTaskTokenUsageResult,
...
export interface GCodeTaskSnapshotWithEtagResult {
  snapshot: GCodeTaskSnapshot | null;
  etag?: string;
  notModified?: boolean;
}

/** 同一 workspace 的固定归档删除集合；每个去重后的目标恰好属于一种结果。 */
export interface GCodeArchivedTaskDeletionResult {
...
export type GCodeModelTrajectoryContentPart =
...
export interface GCodeModelTrajectoryMessage {
  role: string;
  parts: GCodeModelTrajectoryContentPart[];
}

/** model_io 记录里归一化出来的 token 用量。 */
export interface GCodeModelTrajectoryUsage {
  inputTokens?: number;
  outputTokens?: number;
  totalTokens?: number;
...
export type GCodeModelTrajectoryCallSourceKind =
  | "main"
  | "sidecar"
  | "subagent"
  | "compact"
...
export interface GCodeModelTrajectoryCallSource {
  kind: GCodeModelTrajectoryCallSourceKind;
  querySource?: string;
}
 ...
 */
export interface GCodeModelTrajectoryRecord {
  requestId: string;
  attempt: number;
  startedAt: string;
  completedAt?: string;
  ...
  turnId?: string;
  traceId?: string;
  callSource?: GCodeModelTrajectoryCallSource;
  model: {
    modelId?: string;
    providerId?: string;
    role?: string;
  ...
  request: {
    messages: GCodeModelTrajectoryMessage[];
    toolNames: string[];
  };
  response?: {
    finishReason?: string;
    ...
    toolCalls: GCodeModelTrajectoryContentPart[];
...
export interface GCodeModelTrajectory {
  taskId: string;
  /** runtime 是否支持读取 model-io 轨迹（仅 GCode Agent 落盘 model-io）。 */
  available: boolean;
  records: GCodeModelTrajectoryRecord[];
  /** 命中的源文件绝对路径，便于排查。 */
  sourceFiles: string[];
...
export interface GCodeTaskTerminalOutcome {
  taskId: string;
...
export interface GCodeTaskReadyOutcome {
  taskId: string;
  reason: "prompt_completed" | "prompt_failed";
}

/**
 * IGCodeTaskService — GCode task wrapper API 服务接口
 *
 * UI、remote controller 通过这层访问 task wrapper 状态；核心 session 状态由
 * GCode Agent server 维护，新功能应优先走 IGCodeSessionService。
 */
export interface IGCodeTaskService {
  // ---- 生命周期 ----
  ...
  enqueueTaskCommand(params: {
    ...
    automationId?: string;
    ownerRunId?: TraceId;
  }): Promise<GCodeEnqueueTaskCommandResult>;
  ...
  cancelTaskCommand(params: {
    ...
    ownerRunId?: TraceId;
    clientMode: GCodeTaskClientMode;
  }): Promise<GCodeCancelTaskCommandResult>;
  ...
  }): Promise<GCodeSessionCompactResult>;
  ...
  goalSession(params: {
    ...
    objective?: string;
    expectedRevision?: number;
  }): Promise<GCodeSessionGoalResult>;
   ...
   */
  listTaskList(params: GCodeTaskListQuery): Promise<GCodeTaskListResult>;
  ...
  applyGroupedTaskViewOrder(params: GCodeGroupedTaskViewOrderInput): Promise<GCodeGroupedTaskView>;
  ...
  getTaskSnapshot(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<GCodeTaskSnapshot | null>;
  ...
  getTaskSnapshotWithEtag(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<GCodeTaskSnapshotWithEtagResult>;
  ...
  getTaskSnapshotBody(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<GCodeTaskSnapshotBody | null>;
  ...
  getTaskSnapshotRef(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<GCodeTaskSnapshotRefContent | null>;
  ...
  getTaskSnapshotToolCallsSlice(params: {
    ...
    limit: number;
  }): Promise<GCodeTaskSnapshotToolCallsSlice | null>;
  ...
  getTaskConfigOptions(params: { taskId: string }): Promise<GCodeConfigOption[]>;
  ...
  setAssistantMessageFeedback(params: {
    ...
    workspaceIdentity?: string;
    turnIndex: number;
    feedback: GCodeAssistantMessageFeedback | null;
  }): Promise<GCodeSessionFile>;
  ...
  /** 切换 task 模式 */
  setMode(params: { taskId: string; mode: GCodeTaskMode }): Promise<void>;
  ...
  /**
   * 读取 task 对应的模型调用轨迹（来自 ~/.gcode/cli/{debug,rollout} 的 model-io JSONL）。
   * taskId 即 GCode Agent 的 sessionId，按 sessionId 匹配 model-io 记录。
   */
  getModelTrajectory(params: {
    taskId: string;
    /** 最多返回的调用条数（按时间倒序保留最近 N 条），默认 200。 */
    limit?: number;
  }): Promise<GCodeModelTrajectory>;
  ...
  getTaskTokenUsage(params: {
    ...
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<GCodeTaskTokenUsageResult>;
  ...
  /**
   * 重启指定 workspace 的 GCode Agent 共享进程。
   * 配置变更需要重新读取进程环境时使用。
  ...
  onDynamicWorkspaceEvent(
    workspace: string | GCodeWorkspaceEventSubscriptionParams,
  ): Event<GCodeWorkspaceEvent>;

  /** 全局错误事件 */
  onError: Event<GCodeError>;
}

export const IGCodeTaskService = createServiceDescriptor<IGCodeTaskService>(
  ServiceChannels.GCodeTask,
);
