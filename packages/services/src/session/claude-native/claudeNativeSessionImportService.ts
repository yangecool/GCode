...
import type { GCodeImportSessionsResult, GCodeTaskMeta } from "@gcode/shared";
...
export async function importClaudeNativeSessions(params: {
  ...
  sessionIds: string[];
  createImportedSession?: (source: ClaudeNativeImportedSessionSource) => Promise<GCodeTaskMeta>;
  onTaskImported: (meta: GCodeTaskMeta) => void;
}): Promise<GCodeImportSessionsResult> {
  const normalizedSessionIds = [
    ...new Set(params.sessionIds.map((item) => item.trim()).filter(Boolean)),
  ];
  const result: GCodeImportSessionsResult = {
    imported: [],
    skipped: [],
    failed: [],
  };
