...
import type { ApiClient, FeedbackDeviceInfo } from "@gcode/shared";
import {
...
function resolveApiBaseUrl(explicit?: string): string {
  return (
    explicit?.trim() ||
    process.env.GCODE_FEEDBACK_API_BASE?.trim() ||
    buildRuntimeGCodeApiUrl(process.env, "/api/v1")
  );
}
...
function buildDeviceSnapshot(): FeedbackDeviceInfo {
  return {
    ...
    buildCommitId: GCODE_COMMIT,
    buildTime: GCODE_BUILD_TIME,
...
export function createFeedbackService(options: CreateFeedbackServiceOptions): IFeedbackService {
  ...
  async function getGcodeJwtToken(): Promise<string | undefined> {
    return (await options.credentialService.load(GCODE_JWT_TOKEN_KEY))?.trim() || undefined;
  }

  async function hasGcodeJwtToken(): Promise<boolean> {
    return Boolean(await getGcodeJwtToken());
  }
  ...
  const httpClient = new FeedbackHttpClient({
    ...
    getAuthHeaders: async () => {
      ...
      const jwtToken = await getGcodeJwtToken();
      if (jwtToken) {
