/* eslint-disable max-lines -- gcodeSessionService 聚合 desktop-continuous session 操作、draft lifecycle 和 task index 同步边界，拆分需要单独设计。 */
import type { IGCodeAgentService } from "#src/gcode-agent/gcodeAgent.js";
import type { GCodeTaskIndexSyncer } from "#src/gcode-agent/gcodeTaskIndexSyncer.js";
...
import type {
  ...
  GCodeSessionReadWorkspacePresentationParams,
...
import { formatModelPickerValue } from "#src/gcode-agent/gcodeConfigOptions.js";
import { createGCodeSessionApiRetryRuntimeTracker } from "#src/gcode-session/gcodeSessionApiRetry.js";
import { appendWorkspaceToFilesystemMcpServers } from "#src/session/mcpWorkspaceScope.js";
import { repairEmptyImportedClaudeSessionSnapshot } from "#src/gcode-session/importedClaudeSessionRepair.js";
import { createGCodeDeferredDraftRegistry } from "#src/gcode-session/gcodeSessionDraftRegistry.js";
...
interface CreateGCodeSessionServiceOptions {
...
export function createGCodeSessionService({
  agentService,
  taskIndexSyncer,
  cuaProductMcpServerResolver,
}: CreateGCodeSessionServiceOptions): IGCodeSessionService {
  const { withApiRetryRuntime } = createGCodeSessionApiRetryRuntimeTracker();
  const deferredDraftSessions = createGCodeDeferredDraftRegistry();

  function notifySyncer(
    target: GCodeTaskTarget,
    options?: {
  ...
  function getSessionSnapshotDiagnostics(snapshot: GCodeSessionStateSnapshot) {
  ...
  // gcodeSessionService 是 desktop-continuous 主路径，状态变更后必须
  ...
  async function broadcastSnapshot(
    ...
    try {
      ...
      logger.warn(
        undefined,
        `[gcode-session-service] ${tag} syncSnapshotAndBroadcast 失败 taskId=${snapshot.session.sessionId}`,
        error,
      );
    }
  }
  ...
  async function repairEmptyImportedClaudeSession(
    snapshot: GCodeSessionStateSnapshot,
    params: GCodeSessionResumeParams | GCodeSessionReadParams,
  ...
  async function withResolvedMcpServers<
    T extends GCodeSessionCreateParams | GCodeSessionResumeParams,
  >(params: T): Promise<T> {
  ...
  return {
    async initializeWorkspace(params: GCodeSessionWorkspaceTarget) {
    ...
    getWorkspaceRuntimeIdentity(params: GCodeSessionWorkspaceTarget) {
      return agentService.getWorkspaceRuntimeIdentity(params);
    },

    readWorkspacePresentation(params: GCodeSessionReadWorkspacePresentationParams) {
      return agentService.readWorkspacePresentation(params);
    },

    async createSession(params: GCodeSessionCreateParams) {
      ...
      logger.info(sessionTraceId, "[gcode-session-service] createSession 分配 session trace", {
      ...
      logger.info(sessionTraceId, "[gcode-session-service] createSession agent 返回", {
      ...
      // desktop-continuous 路径不会经过 GCode task adapter，sqlite 的 task index 全靠
      ...
      logger.info(sessionTraceId, "[gcode-session-service] createSession task index 同步完成", {
    ...
    async resumeSession(params: GCodeSessionResumeParams) {
      const startedAt = Date.now();
      ...
      if (
        ...
        if (
          ...
          logger.warn(
            undefined,
            "[gcode-session-service] resumeSession 跳过不支持的 task 思考强度",
            {
      ...
      if (thoughtLevelOverride && snapshot.settings.thoughtLevel.current !== thoughtLevelOverride) {
        logger.info(undefined, "[gcode-session-service] resumeSession 重放 task 思考强度", {
      ...
      logger.info(undefined, "[gcode-session-service] resumeSession 历史恢复完成", {
        agentDurationMs,
    ...
    listSessions(params: GCodeSessionListParams) {
      return agentService.listSessions(params);
    },

    async readSession(params: GCodeSessionReadParams) {
      const startedAt = Date.now();
      const snapshot = await repairEmptyImportedClaudeSession(
        withApiRetryRuntime(await agentService.readSession(params)),
        params,
      );
      logger.info(undefined, "[gcode-session-service] readSession 历史快照读取完成", {
    ...
    readSessionMessages(params: GCodeSessionMessagesParams) {
      return agentService.readSessionMessages(params);
    },

    readSessionEvents(params: GCodeSessionEventsParams) {
      return agentService.readSessionEvents(params);
    },

    promoteDeferredDraftSession(params: GCodeTaskTarget) {
      ...
      notifySyncer(params);
      logger.info(undefined, "[gcode-session-service] deferred draft session 已提升为 task", {
        sessionId: params.sessionId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
    ...
    closeSession(params: GCodeTaskTarget) {
      deferredDraftSessions.forget(params);
      return agentService.closeSession(params).then(() => undefined);
    },

    async closeDeferredDraftSession(params: GCodeTaskTarget) {
      try {
        ...
        logger.warn(
          undefined,
          "[gcode-session-service] 条件关闭 deferred draft 失败，保留旧 session",
          {
    ...
    async setModel(params: GCodeSessionSetModelParams) {
    ...
    async setThoughtLevel(params: GCodeSessionSetThoughtLevelParams) {
      return withApiRetryRuntime(await agentService.setThoughtLevel(params));
    },

    async setMode(params: GCodeSessionSetModeParams) {
