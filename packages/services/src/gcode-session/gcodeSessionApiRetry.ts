import {
  ...
  resolveWorkspaceKey,
  type GCodeSessionApiRetryStatus,
  type GCodeSessionStateSnapshot,
  gcodeApiRetryFromModelNetworkStatusPayload,
  gcodeApiRetryFromStreamRecoveryPayload,
} from "@gcode/shared";
import type { GCodeSessionServiceEvent, GCodeTaskTarget } from "#src/gcode-session/gcodeSession.js";

export function createGCodeSessionApiRetryRuntimeTracker(): {
  trackApiRetryFromSessionEvent: (
    params: GCodeTaskTarget,
    event: GCodeSessionServiceEvent,
  ) => GCodeSessionServiceEvent;
  withApiRetryRuntime: (snapshot: GCodeSessionStateSnapshot) => GCodeSessionStateSnapshot;
} {
  const apiRetryBySessionKey = new Map<string, GCodeSessionApiRetryStatus | null>();
  ...
  function trackApiRetryFromSessionEvent(
    ...
    if (
      ...
      if (apiRetry !== undefined) {
        ...
        isGCodeModelRetryRecoveryProgressPayload(asRecord(event.event.payload))
      ) {
  ...
  function withApiRetryRuntime(snapshot: GCodeSessionStateSnapshot): GCodeSessionStateSnapshot {
  ...
  function sessionKey(params: GCodeTaskTarget): string {
...
): GCodeSessionApiRetryStatus | null | undefined {
