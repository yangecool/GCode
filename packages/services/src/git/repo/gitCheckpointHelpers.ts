...
import type { GitCheckpointDiff, GitCheckpointFileDiff } from "@gcode/shared";
...
export function getCheckpointRefName(workspacePath: string, checkpointId: string): string {
  ...
  return `refs/gcode/checkpoints/${workspaceHash}/${checkpointId}`;
}
...
export function buildCheckpointEnv(tempIndexPath: string): NodeJS.ProcessEnv {
  return {
    ...
    GIT_AUTHOR_NAME: "GCode Checkpoint",
    GIT_AUTHOR_EMAIL: "checkpoint@gcode.local",
    GIT_COMMITTER_NAME: "GCode Checkpoint",
    GIT_COMMITTER_EMAIL: "checkpoint@gcode.local",
  };
}
