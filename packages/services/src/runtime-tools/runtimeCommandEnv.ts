...
import { buildGCodeToolEnvPassthroughEnv, sanitizeGCodeRuntimeEnvInPlace } from "@gcode/shared";
...
function buildLoginShellEnvPatch(
  ...
  return {
    ...patch,
    ...buildGCodeToolEnvPassthroughEnv(snapshot),
  };
}
...
export function buildRuntimeProcessEnvPatch(
  ...
  const toolEnvPassthroughPatch = buildGCodeToolEnvPassthroughEnv({
    ...normalizedBaseEnv,
    ...loginShellEnvPatch,
  });
  ...
  const envPatch: Record<string, string> = {
    ...
    // encoding is set. GCode/Bash tool output is consumed as UTF-8, so force Python
...
export function initializeRuntimeProcessEnv(
  ...
  // 网络变量会先封存为 GCODE_TOOL_ENV_PASSTHROUGH_JSON，只有 Bash/tool 子进程边界才恢复原名。
  sanitizeGCodeRuntimeEnvInPlace(process.env);
