...
export function normalizeSettingsPatch(patch: Partial<AppSettings>): Partial<AppSettings> {
  ...
  if (
    "gcodeEndpointOrigin" in normalizedPatch &&
    typeof normalizedPatch.gcodeEndpointOrigin === "string"
  ) {
    // 非生产 endpoint override 需要支持 Reset 清空；RPC/JSON 对 undefined 不稳定时，用空串也能回到默认生产域。
    const trimmedGCodeEndpointOrigin = normalizedPatch.gcodeEndpointOrigin.trim();
    normalizedPatch.gcodeEndpointOrigin =
      trimmedGCodeEndpointOrigin.length > 0 ? trimmedGCodeEndpointOrigin : undefined;
  }
