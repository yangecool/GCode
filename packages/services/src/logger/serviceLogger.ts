import { formatLogPrefix, type TraceId } from "@gcode/shared";
...
export function createServiceLogger(
  ...
  function write(
    ...
    // 服务层日志过去常被复用到 GCode Agent 命名 logger，导致新 GCode 路径继续依赖 GCode Agent 目录。
    // 这里把通用分级日志抽到独立模块，后续删除 GCode Agent runtime 时不会牵连非 GCode Agent 服务。
