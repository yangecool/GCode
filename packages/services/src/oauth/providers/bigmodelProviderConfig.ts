import { BIGMODEL_PROVIDER_ID, buildBigModelApiUrl } from "@gcode/shared";
...
const BIGMODEL_OAUTH_PROVIDER_CONFIG: Omit<OAuthProviderRuntimeConfig, "appSecret"> = {
  ...
  userinfoUrl: buildBigModelApiUrl({ GCODE_ENV: "production" }, BIGMODEL_USERINFO_PATH),
  appId: "gcode",
...
export function createBigModelProviderRuntimeConfig(
  ...
  return {
    ...
    tokenUrl:
      ...
      buildGCodeApiUrlFromEnv(env, "/api/v1/oauth/token"),
    ...
    // 当前 BigModel callback 只消费 gcode OAuth token 路由，显式 appSecret 仅保留给
