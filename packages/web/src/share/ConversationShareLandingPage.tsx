...
import type { ConversationSharePreview } from "@gcode/shared";
import { ConversationShareReadonlyTimeline } from "@gcode/ui/conversation-share-readonly";
import { renderOAuthProviderIcon } from "@gcode/ui/oauth-provider-icon";
import { applyTheme, resolveTheme, type Theme } from "@gcode/ui/useTheme";
...
interface Copy {
  ...
  retry: string;
  continueInGCode: string;
  switchToDarkTheme: string;
  switchToLightTheme: string;
  continueHelp: string;
  downloadGCode: string;
  retryOpen: string;
...
const GCODE_DOWNLOAD_URL = "https://zcode.z.ai";
...
const COPY: Record<ConversationShareLandingLocale, Copy> = {
  "zh-CN": {
    ...
    outdatedTitle: "需要更新 GCode",
    outdatedDescription: "这个分享由更新版本的 GCode 创建，请升级后再查看。",
    unavailableTitle: "分享不可访问",
    unavailableDescription: "当前账号没有权限，或者分享内容已不存在。",
    retry: "重试",
    continueInGCode: "去 GCode 继续",
    switchToDarkTheme: "切换到深色主题",
    switchToLightTheme: "切换到浅色主题",
    continueHelp: "如果没有自动打开 GCode，请先下载客户端，或再次尝试打开。",
  ...
  "en-US": {
    brand: "GCode Conversation Share",
    ...
    outdatedTitle: "Update GCode to continue",
    outdatedDescription:
      "This share was created by a newer version of GCode. Please update to view it.",
    unavailableTitle: "Share unavailable",
    unavailableDescription:
      "This account is not allowed to view the share, or it no longer exists.",
    retry: "Try again",
    continueInGCode: "Continue in GCode",
    switchToDarkTheme: "Switch to dark theme",
    switchToLightTheme: "Switch to light theme",
    continueHelp: "If GCode did not open, download the app or try opening it again.",
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {importLink && headerView.continueVisible ? (
                      <a
                        ...
                        href={importLink}
                        aria-label={copy.continueInGCode}
                        title={copy.continueInGCode}
                        onClick={handleContinue}
                      >
                        <span className={headerView.continueCompact ? "sr-only" : "truncate"}>
                          {copy.continueInGCode}
                        </span>
                        <ArrowUpRightIcon
                          className={
                            headerView.continueCompact
                  ...
                  >
                    ...
                    {importLink ? (
                      <span className={SHARE_CONTINUE_MEASURE_CLASS}>
                        <span>{copy.continueInGCode}</span>
                        <ArrowUpRightIcon
        ...
        {importLink && showContinueHelp ? (
          ...
          <ShareContentInset>
            ...
            >
              ...
              <a className="text-brand underline underline-offset-2" href={GCODE_DOWNLOAD_URL}>
                {copy.downloadGCode}
              </a>
            </div>
          </ShareContentInset>
          ...
          <ShareContentInset>
            ...
            >
              <a
                ...
                onClick={handleContinue}
              >
                <span className="truncate">{copy.continueInGCode}</span>
...
export function ConversationShareLandingLoader({
  ...
  const handleThemeChange = useCallback((nextTheme: Theme) => {
    localStorage.setItem("gcode-theme", nextTheme);
