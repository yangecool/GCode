...
// Vite 配置在 Node 加载期执行，不能导入 @gcode/shared 根入口。
...
} from "@gcode/shared/gcodeEndpoint";
...
export default defineConfig(({ mode }) => {
  ...
  const gcodeEndpointOrigin = resolveRuntimeGCodeEndpointOrigin(endpointEnv);
  ...
  return {
    ...
    define: {
      ...
      __GCODE_VERSION__: JSON.stringify(version),
      __GCODE_COMMIT__: JSON.stringify(env.GCODE_COMMIT || "unknown"),
      __GCODE_ENV__: JSON.stringify(gcodeEnv),
      "import.meta.env.VITE_GCODE_BASE_URL": JSON.stringify(gcodeEndpointOrigin),
      // 兼容旧 Web runtime 读取名；新代码统一读 VITE_GCODE_BASE_URL。
