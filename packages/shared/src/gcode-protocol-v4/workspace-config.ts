...
// 下发，host 侧 gcodeTaskIndexSyncer 转成 workspace_config_options_update 广播给 UI；
...
// 与 host 侧 GCodeConfigSelectValue（gcode-task-types-core）结构对齐：
// syncer 转发 workspace_config_options_update 时零映射直通，下游 useGCodeConfig 消费面不改。
...
// 与 host 侧 GCodeConfigOption 结构对齐（id: model / mode / thought_level / 自定义）。
...
// slash 命令目录（workspace 级；与 host 侧 GCodeSlashCommand 对齐）。
