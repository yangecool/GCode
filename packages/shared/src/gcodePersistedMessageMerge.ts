import type { GCodePersistedMessage, GCodePersistedMessagePart } from "./gcode-task-types-core.js";

// GCode runtime 把同一个 user turn 里**每一轮 LLM 调用**落成独立的 assistant 消息（各带 time.created/completed）。
...
export function coalesceConsecutiveGCodeAssistants(
  messages: readonly GCodePersistedMessage[],
): GCodePersistedMessage[] {
  const merged: GCodePersistedMessage[] = [];
...
function mergeAssistantPair(
  first: GCodePersistedMessage,
  next: GCodePersistedMessage,
): GCodePersistedMessage {
  ...
  const shiftedNextParts: GCodePersistedMessagePart[] = (next.parts ?? []).map((part) =>
    part.type === "tool-call"
