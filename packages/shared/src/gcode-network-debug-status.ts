import type {
  ...
  TraceId,
  GCodeStreamEvent,
  GCodeTaskNetworkDebugStatusType,
} from "./gcode-task-types-core.js";

const MODEL_NETWORK_STATUS_TYPES = new Set<GCodeTaskNetworkDebugStatusType>([
...
export function gcodeTaskNetworkDebugStatusFromPayload(params: {
  taskId: string;
  traceId: TraceId;
  inputId?: InputId;
  queryId?: QueryId;
...
}): Extract<GCodeStreamEvent, { type: "task_network_debug_status" }> | null {
...
function networkStatusTypeValue(value: unknown): GCodeTaskNetworkDebugStatusType | undefined {
  return typeof value === "string" && MODEL_NETWORK_STATUS_TYPES.has(value as never)
    ? (value as GCodeTaskNetworkDebugStatusType)
    : undefined;
}
