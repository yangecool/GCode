import type { GCodeApiRetryStatus } from "./gcode-task-types-core.js";

export function normalizeGCodeApiRetryStatus(
  value: unknown,
...
export function gcodeApiRetryFromModelNetworkStatusPayload(
  payload: Record<string, unknown>,
): GCodeApiRetryStatus | null | undefined {
  const type = stringValue(payload.type);
  if (type === "model_retry_scheduled") {
    return normalizeGCodeApiRetryStatus(payload);
  }
  if (type === "model_request_started") {
    const streamRecoveryRetry = gcodeApiRetryFromStreamRecoveryPayload(payload.streamRecovery);
...
export function isGCodeModelRetryRecoveryProgressPayload(
  payload: Record<string, unknown>,
): boolean {
...
export function gcodeApiRetryFromStreamRecoveryPayload(
  value: unknown,
): GCodeApiRetryStatus | undefined {
  const record = asRecord(value);
