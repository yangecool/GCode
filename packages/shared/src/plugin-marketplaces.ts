...
export const GCODE_OFFICIAL_PLUGIN_MARKETPLACE_ID = "zcode-plugins-official";
...
export const DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS: ReadonlySet<string> = new Set([
  ...
  "gcode-guide@zcode-plugins-official",
...
export const DEFAULT_PLUGIN_MARKETPLACES: DefaultPluginMarketplace[] = [
  {
    // GCode 官方唯一市场：本地 seed 分片与 CDN 分片在 Agent storage 内合并。
    // CDN manifest 的 name 必须与该 canonical id 一致。
    id: GCODE_OFFICIAL_PLUGIN_MARKETPLACE_ID,
    source: "https://cdn-zcode.z.ai/gcode/official-plugin/marketplace.json",
    name: GCODE_OFFICIAL_PLUGIN_MARKETPLACE_ID,
    description: "Official GCode plugins marketplace: built-in and community plugins for GCode.",
    pluginCount: 0,
  },
];

// 商店「公开」分段只有一个 GCode 官方市场 id，内置与 CDN 不再拆分身份。
export const PUBLIC_STORE_MARKETPLACE_IDS = [GCODE_OFFICIAL_PLUGIN_MARKETPLACE_ID] as const;
