...
import { normalizeGCodeEndpointOrigin } from "./gcodeEndpoint.js";
import {
...
const gcodeInteractionBehaviorSchema = z.enum(["queue", "guide"]);
...
const gcodeEndpointOriginSchema = z.preprocess((value) => {
  ...
  try {
    return normalizeGCodeEndpointOrigin(trimmed);
  } catch {
    return undefined;
  }
}, z.string().optional());

function sanitizeGCodeEndpointOrigin(value: unknown): unknown {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return value;
  }
  const raw = value as Record<string, unknown>;
  if (!("gcodeEndpointOrigin" in raw)) {
    return value;
  }
  const parsed = gcodeEndpointOriginSchema.safeParse(raw.gcodeEndpointOrigin);
  if (parsed.success && typeof parsed.data === "string") {
    return { ...raw, gcodeEndpointOrigin: parsed.data };
  }
  const { gcodeEndpointOrigin: _gcodeEndpointOrigin, ...next } = raw;
...
const appSettingsObjectSchema = z.object({
  ...
  gcodeInteractionBehavior: gcodeInteractionBehaviorSchema.default("queue"),
  ...
  gcodeEndpointOrigin: gcodeEndpointOriginSchema.optional(),
});
        ...
        migrateMessageStreamShowReasoningDefault(
          migrateCloseToTrayOnWindowsDefault(
            migrateLegacyLocalePreference(
              sanitizeGCodeEndpointOrigin(migrateLegacyWorkspaceSession(value)),
            ),
          ),
        ),
      ),
...
export const appSettingsPatchSchema = z.object({
  ...
  gcodeInteractionBehavior: gcodeInteractionBehaviorSchema.optional(),
