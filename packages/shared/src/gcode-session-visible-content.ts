...
// 消费者：gcodeTaskServiceAdapter/gcodeSessionProjection/gcodeTaskIndexSyncer、
// CLI bootstrap session-mapper。textFromGCodeMessageParts 已迁
// gcode-protocol-legacy-types.ts（幸存面）。本文件与旧投影栈同生命周期。
import type { GCodeSessionGoal } from "./gcode-protocol/index.js";
...
export function isGCodeGoalContinuationReminderText(text: string): boolean {
...
export function isGCodeGoalContinuationReminderMessage(message: GCodeMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (message.info.source === "goal-continuation" ||
      String(message.info.metadata?.["source"] ?? "") === "goal-continuation" ||
      isGCodeGoalContinuationReminderText(textFromGCodeMessageParts(message.parts)))
  );
}

export function isGCodeGoalStateReminderText(text: string): boolean {
  const normalized = text.trimStart();
  return normalized.startsWith("<system-reminder>") && normalized.includes(GOAL_STATE_TEXT_MARKER);
}

export function isGCodeGoalModelOnlyReminderMessage(message: GCodeMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (isGCodeGoalContinuationReminderMessage(message) ||
      isGCodeGoalStateReminderText(textFromGCodeMessageParts(message.parts)))
  );
}

export function isGCodeModelOnlySyntheticUserMessage(message: GCodeMessageWithParts): boolean {
  if (
    message.info.role === "user" &&
...
export function isGCodeCompactSummaryMessage(message: GCodeMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
...
export function getGCodeUserVisibleMessages(
  messages: readonly GCodeMessageWithParts[],
  _options: { target?: GCodeSessionGoal | null } = {},
): GCodeMessageWithParts[] {
  const visibleMessages: GCodeMessageWithParts[] = [];
  for (const message of messages) {
    if (isGCodeModelOnlySyntheticUserMessage(message) || isGCodeCompactSummaryMessage(message)) {
...
export function getGCodeGoalIterationByAssistantMessageId(
  messages: readonly GCodeMessageWithParts[],
  options: { maxGoalIteration?: number; target?: GCodeSessionGoal | null } = {},
): Map<string, number> {
  ...
  const sortedMessages = [...messages].sort(compareGCodeMessagesByCreatedTime);
  ...
  for (const message of sortedMessages) {
    if (message.info.role === "user") {
      if (isGCodeGoalContinuationReminderMessage(message)) {
...
export interface GCodeGoalIterationCountTimelineItem {
  goalIteration?: number;
  status: "started" | "completed" | "failed_closed" | "cancelled";
  verification?: { passed?: boolean | null } | null;
}

export function getGCodeGoalActiveIterationCount(input: {
  targetStatus?: string | null;
  timeline?: readonly GCodeGoalIterationCountTimelineItem[] | null;
}): number {
...
function compareGCodeMessagesByCreatedTime(
  left: GCodeMessageWithParts,
  right: GCodeMessageWithParts,
) {
...
function isVisibleRealGoalUserMessage(message: GCodeMessageWithParts, target: GCodeSessionGoal) {
  if (
    message.info.role !== "user" ||
...
export function resolveGCodeVisibleSessionTitle(input: {
  title?: string;
  messages: readonly GCodeMessageWithParts[];
  target?: GCodeSessionGoal | null;
  fallback?: string;
}): string {
  const normalizedTitle = input.title?.trim() ?? "";
  if (
    normalizedTitle &&
    !isGCodeGoalContinuationReminderText(normalizedTitle) &&
    !isGCodeGoalStateReminderText(normalizedTitle)
  ) {
  ...
  const firstUser = input.messages.find(
    (message) =>
      message.info.role === "user" &&
      getConversationMessageProjectionPolicy(message) === "realUserInput" &&
      !isGCodeGoalModelOnlyReminderMessage(message),
  );
  const firstUserText = textFromGCodeMessageParts(firstUser?.parts ?? []).trim();
  if (firstUserText) {
