export type GCodeBackgroundTaskControlStatus =
  | "pending"
  | "running"
  | "completed"
  | "failed"
...
export interface GCodeBackgroundTaskControlItem {
  jobId: string;
  toolCallId?: string;
  command: string;
  taskKind: "agent" | "bash";
  cancellable?: boolean;
  title?: string;
  status: GCodeBackgroundTaskControlStatus;
  startedAt?: number;
  elapsedMs?: number;
  pid?: number;
  stdoutTail?: string;
...
const ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES = new Set<GCodeBackgroundTaskControlStatus>([
  "running",
]);
...
type BackgroundTaskControlKind = GCodeBackgroundTaskControlItem["taskKind"];
 ...
 */
export function resolveGCodeBackgroundTaskControlKind(
  value: unknown,
): BackgroundTaskControlKind | undefined {
...
function normalizeStatus(value: string | undefined): GCodeBackgroundTaskControlStatus {
...
function readJobId(record: Record<string, unknown>, command: string): string {
  const explicitId = readStringField(record, [
    // GCode Protocol 后台任务取消入口按 taskId 查找 runtime task 记录；
...
export function parseGCodeBackgroundTaskControlItems(
  value: unknown,
): GCodeBackgroundTaskControlItem[] {
  if (!Array.isArray(value)) {
    return [];
  }
  const jobsById = new Map<string, GCodeBackgroundTaskControlItem>();
  for (const item of value) {
    if (!isPlainRecord(item)) {
      continue;
    }
    const taskKind = resolveGCodeBackgroundTaskControlKind(item);
    if (!taskKind) {
      continue;
    }
    ...
    const job: GCodeBackgroundTaskControlItem = {
      jobId,
...
export function isActiveGCodeBackgroundTaskControlItem(
  job: GCodeBackgroundTaskControlItem,
): boolean {
  return ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES.has(job.status);
}

export function getGCodeBackgroundTaskControlItemElapsedMs(
  job: GCodeBackgroundTaskControlItem,
  now = Date.now(),
): number {
...
export function collectVisibleGCodeBackgroundTaskControlItems(
  jobs: readonly GCodeBackgroundTaskControlItem[],
  now = Date.now(),
  thresholdMs = 30_000,
): Array<GCodeBackgroundTaskControlItem & { elapsedMs: number }> {
  return jobs
    .filter(isActiveGCodeBackgroundTaskControlItem)
    .map((job) => ({
      ...job,
      elapsedMs: getGCodeBackgroundTaskControlItemElapsedMs(job, now),
    }))
