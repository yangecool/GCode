...
import { markGcodeJwtInvalidRestart } from "@/root/gcodeJwtInvalidRestartMarker.js";
...
export function useRootOAuthEffects({
  ...
  useEffect(() => {
    ...
    const disposable = services.broadcastService.onMessage((message) => {
      if (message.channel !== GCODE_JWT_INVALID_BROADCAST_CHANNEL || disposed) {
        return;
      }
      void (async () => {
        ...
        markGcodeJwtInvalidRestart();
        if (typeof window !== "undefined" && !("gcode" in window)) {
