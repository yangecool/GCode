...
import { GCODE_AGENT_PROVIDER, resolveExecutionState } from "@gcode/shared";
...
import type { IModelSelectionService } from "@gcode/services";
...
import { submissionModeSchema } from "@gcode/shared/gcode-protocol-v4";
...
function shouldHydrateWorkspaceCatalog(params: {
  ...
  slashCommands: readonly GCodeSlashCommand[];
}): boolean {
...
export function useDraftConfigControl(params: {
  ...
  const gcodeSessionService = useGCodeSessionService(workspacePath, null, workspaceIdentity);
  ...
  useEffect(() => {
    ...
    const flight = prepareWorkspaceWithGCodeSessionService({
      workspacePath,
      workspaceIdentity,
      ...
      .then((prepareResult) => {
        ...
        const latest = useGCodeSessionStore.getState();
