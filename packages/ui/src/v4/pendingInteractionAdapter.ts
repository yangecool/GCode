...
/** v4 permission payload → 旧 PermissionDialog 可消费的 GCodePermissionRequest。 */
export function pendingPermissionToLegacyRequest(
  ...
  const advertisedOptions = payload.fullAccessOption
    ...
    : payload.options;
  const options: GCodePermissionOption[] = advertisedOptions.map((option) => ({
