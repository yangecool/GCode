...
type AgentIntl = ReturnType<typeof useGCodeIntl>["intl"];
...
export function getAgentKindLabel(
  ...
  const rawName =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "gcode", "agentType"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "gcode", "agent_type"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "gcode", "subagent_type"]);
...
export function getAgentColor(toolCall: AgentToolCall) {
  ...
  const inputRecord = isPlainRecord(toolCall.input) ? toolCall.input : null;
  const rawColor =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "gcode", "color"]) ??
...
export function readBackgroundAgentInfo(toolCall: AgentToolCall) {
  ...
  const gcode = meta && isPlainRecord(meta.gcode) ? meta.gcode : null;
  const gcodeBackgroundAgent =
    gcode && isPlainRecord(gcode.backgroundAgent) ? gcode.backgroundAgent : null;
  const taskNotification =
    gcode && isPlainRecord(gcode.taskNotification) ? gcode.taskNotification : null;
  const backgroundAgent = gcodeBackgroundAgent;
...
export function getAgentActivityContent(toolCall: AgentToolCall) {
  const taskNotificationResult =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "gcode", "taskNotification", "result"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "gcode", "taskNotification", "summary"]);
