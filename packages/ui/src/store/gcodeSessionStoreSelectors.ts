/**
 * GCode Session Store 选择器与内部辅助函数
 *
 * 从 gcodeSessionStore.ts 拆分出来，包含 workspace 状态读取/更新辅助函数，
 * 以及所有按 task 粒度的只读访问器和独立选择器。
 */
import type { GCodeTaskRuntimeStatus, GCodeTaskMeta } from "@gcode/shared";
...
import {
  ...
  type GCodeSessionStoreState,
  type WorkspaceInitState,
  type TaskRuntimeState,
  type WorkspaceGCodeUIState,
} from "./gcodeSessionStoreTypes.js";
...
function collectIdentityTaskIds(
  baseState: WorkspaceGCodeUIState,
  workspaceIdentity: string,
): Set<string> {
...
function createIdentityWorkspaceStateSeed(
  baseState: WorkspaceGCodeUIState | undefined,
  workspaceIdentity?: string,
...
export function updateWorkspaceState(
  state: GCodeSessionStoreState,
  workspacePath: string,
  updater: (current: WorkspaceGCodeUIState) => WorkspaceGCodeUIState,
  workspaceIdentity?: string,
): Pick<GCodeSessionStoreState, "workspaces"> {
  const workspaceKey = resolveWorkspaceStateKey(workspacePath, workspaceIdentity);
  const current =
  ...
  if (nextWorkspaceState === current) {
    // 单 GCode Agent 迁移后旧 provider 选择都会归一为 glm，很多调用实际不会改变状态。
...
export function getTaskRuntimeState(
  workspaceState: WorkspaceGCodeUIState,
  taskId: string,
): TaskRuntimeState {
...
export function getTaskUiState(workspaceState: WorkspaceGCodeUIState, taskId: string) {
...
export function getTaskMeta(
  workspaceState:
    | WorkspaceGCodeUIState
    | Partial<Pick<WorkspaceGCodeUIState, "optimisticTaskListByTaskId" | "taskListCache">>,
  taskId: string,
): GCodeTaskMeta | null {
...
export function getVisibleTaskMetas(
  ...
  const taskById = new Map<string, GCodeTaskMeta>();
...
export function getTaskUnreadIndicator(
  workspaceState:
    ...
    | Partial<
      ...
      >,
  taskId: string,
  fallbackTask?: Pick<GCodeTaskMeta, "unreadAt">,
): boolean {
...
export function selectWorkspaceGCodeState(
  state: GCodeSessionStoreState,
  workspacePath: string,
  workspaceIdentity?: string,
) {
...
export function getWorkspaceInitState(
  workspaceState: Pick<WorkspaceGCodeUIState, "workspaceInit">,
): WorkspaceInitState {
