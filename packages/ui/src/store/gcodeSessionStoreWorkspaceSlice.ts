...
} from "@/store/gcodeSessionStoreSelectors.js";
...
function normalizeThoughtLevelConfigOption(option: GCodeConfigOption): GCodeConfigOption {
  if (
    option.type !== "select" ||
...
function normalizeConfigOptions(options: GCodeConfigOption[]): GCodeConfigOption[] {
  return options.map(normalizeThoughtLevelConfigOption);
}

function cloneConfigOptions(options: readonly GCodeConfigOption[]): GCodeConfigOption[] {
...
function isModeConfigOption(option: GCodeConfigOption): boolean {
  return option.category === "mode" && option.type === "select";
}

function createFallbackModeOption(params: {
  currentModeId: string;
  options?: NonNullable<GCodeConfigOption["options"]>;
}): GCodeConfigOption {
...
function replaceModeConfigOption(
  currentOptions: GCodeConfigOption[] | null,
  nextModeOption: GCodeConfigOption | null,
...
function updateActiveTaskConfigOptions(
  current: WorkspaceGCodeUIState,
  updater: (options: GCodeConfigOption[] | null) => GCodeConfigOption[] | null,
): Pick<WorkspaceGCodeUIState, "taskConfigOptionsByTaskId"> | null {
...
function resolveActiveTaskConfigOptionsOnSwitch(
  current: WorkspaceGCodeUIState,
  taskId: string,
): { configOptions: GCodeConfigOption[]; status: ConfigOptionsStatus } | null {
  ...
  return {
    ...
    promoteGroupedDraftTask: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const optimisticTask: GCodeTaskMeta = {
              taskId,
              traceId: `session-${taskId}` as GCodeTaskMeta["traceId"],
              title: "",
              workspacePath,
    ...
    startDraft: (
      ...
      const normalizedProvider = provider
        ? normalizeAgentProviderToGCodeAgent(provider)
        : undefined;
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const nextSelectedProvider = normalizeAgentProviderToGCodeAgent(
    ...
    bindRuntimeProvider: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => ({
            ...current,
            selectedProvider: normalizeAgentProviderToGCodeAgent(provider),
          }),
          workspaceIdentity,
        ),
      ),
