...
import { convertToGCodeAgentMcpServer } from "@gcode/shared";
...
import {
  importLegacyCommonServersToGCodeAgent,
  migrateStoredCommonMcpToGCodeAgent,
...
interface McpStoreState {
  ...
  addGCodeAgentMcpServer: (name: string, config: McpServerConfig, projectPath?: string) => void;
  updateGCodeAgentMcpServer: (name: string, config: McpServerConfig, projectPath?: string) => void;
  deleteGCodeAgentMcpServer: (name: string, projectPath?: string) => void;
  ...
  getServer: (id: string) => GCodeMcpServer | undefined;
  setCurrentProjectPath: (
    path: string,
    workspaceIdentity?: string,
    directoryService?: McpDirectoryService | null,
  ) => void;
  getEnabledMcpServersForGCode: (provider: string) => GCodeAgentMcpServer[];
...
export const useMcpStore = create<McpStoreState>((set, get) => {
  ...
  const statusListEpochs: Record<GCodeMcpListMode, number> = {
    connect: 0,
    status: 0,
  };
  ...
  function resolveMcpDirectoryService(
    ...
    if (!effectiveWorkspaceIdentity?.trim()) {
      ...
      // 到用户级 GCode Agent MCP 的迁移；目录服务只用于远端 workspace 覆盖路由。
      return null;
    }
  ...
  return {
    ...
    loadConfig: () => {
      ...
      // MCP 启停状态已经迁移到 ~/.gcode/cli/config.json，不能再读取旧 localStorage，
      // 否则旧的本地开关会覆盖新的 GCode Agent 配置来源。
      ...
      loadMcpPromise = (async () => {
        try {
          ...
          if (!activeDirectoryService) {
            servers = await migrateStoredCommonMcpToGCodeAgent(
              mcpPlatformService,
              servers,
              latestWorkspacePath,
            );
    ...
    addMcpServer: (name, config) => get().addGCodeAgentMcpServer(name, config),
    updateMcpServer: (name, config) => get().updateGCodeAgentMcpServer(name, config),
    deleteMcpServer: (name) => get().deleteGCodeAgentMcpServer(name),
    addScopedMcpServer: async (source, name, config, projectPath) => {
      invalidateStatusListRequests();
      const targetSource: CliMcpSource = source === "mcp" ? "gcodeagentmcp" : source;
    ...
    addGCodeAgentMcpServer: (name, config, projectPath) =>
      get().addScopedMcpServer("gcodeagentmcp", name, config, projectPath),
    updateGCodeAgentMcpServer: (name, config, projectPath) =>
      get().updateScopedMcpServer("gcodeagentmcp", name, config, projectPath),
    deleteGCodeAgentMcpServer: (name, projectPath) =>
      get().deleteScopedMcpServer("gcodeagentmcp", name, projectPath),
    ...
    markServerStatusListRefreshFailed: (error, requestEpoch, mode = "connect") => {
      ...
      set((state) => ({
        ...
        servers: state.servers.map((server) => {
          const enabled = state.enabledStates[server.id] ?? server.enabled;
          if (server.source !== "gcodeagentmcp" || !enabled || server.status !== "connecting") {
            return server;
          }
          return {
            ...server,
    ...
    getEnabledMcpServersForGCode: (_provider) => {
      const { servers, enabledStates, currentProjectPath } = get();
      const candidates = new Map<string, GCodeMcpServer>();

      for (const server of servers) {
        const isEnabled = enabledStates[server.id] ?? server.enabled;
        if (!isEnabled) continue;
        if (server.source !== "gcodeagentmcp") continue;
      ...
      for (const server of candidates.values()) {
        ...
        if (gcodeAgentServer) result.push(gcodeAgentServer);
      }
      return result;
    },
    ...
    mergePreloadedMcpServers: (preloaded) => {
      ...
      const targetSource: CliMcpSource = "gcodeagentmcp";
