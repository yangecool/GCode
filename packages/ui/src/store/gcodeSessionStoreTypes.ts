/* oxlint-disable eslint(max-lines) -- GCode Agent store 类型和默认状态集中导出，避免切片间重复定义共享结构。 */
/**
 * GCode Session Store 类型定义、接口、常量与默认值工厂
 *
 * 从 gcodeSessionStore.ts 拆分出来，供 store 本体和 selectors / navigation 等子模块共享。
 */
import {
  ...
  type GCodeWorkspaceInitStatus,
  type GCodeTaskRuntimeStatus,
  type GCodeProvider,
  type GCodeTaskMeta,
  type GCodeContextCacheUsage,
  type GCodeConfigOption,
  type GCodeSlashCommand,
  type GCodePermissionRequest,
  type GCodeElicitationRequest,
  type GCodeBackgroundTaskControlItem,
  type GCodeSessionActiveTurnKind,
...
export interface WorkspaceInitState {
  status: GCodeWorkspaceInitStatus;
  error: string | null;
  attempts: number;
}
...
export interface TaskRuntimeState {
  status: GCodeTaskRuntimeStatus;
  error: string | null;
  /** 该 task 当前运行态绑定的 GCode Agent 进程 provider，用于 workspace 级进程重建 busy lock。 */
  ...
  backgroundTaskControls: GCodeBackgroundTaskControlItem[];
...
export interface TaskUiState {
  permissionRequest: GCodePermissionRequest | null;
  pendingPermissionRequests: GCodePermissionRequest[];
  elicitationRequest: GCodeElicitationRequest | null;
  pendingElicitationRequests: GCodeElicitationRequest[];
...
export interface WorkspaceGCodeUIState {
  /** 当前 workspace 激活中的 task */
  activeTaskId: string | null;
  /**
   * GCode Agent 的 workspace 初始化状态。
   *
   * 单 GCode Agent 迁移后继续按 provider 分桶会保留多份已经不会再被真实运行时更新的旧状态，
  ...
  taskConfigOptionsByTaskId: Record<string, GCodeConfigOption[]>;
  ...
  optimisticTaskListByTaskId: Record<string, GCodeTaskMeta>;
  ...
  /** 当前 workspace 选中的 GCode Agent provider */
  ...
  /** GCode Agent configOptions（模型、模式、思考级别等） */
...
export interface GCodeSessionStoreState {
  /** 按 workspace 维护聊天相关 UI 状态 */
  workspaces: Record<string, WorkspaceGCodeUIState>;
  getWorkspaceState: (workspacePath: string, workspaceIdentity?: string) => WorkspaceGCodeUIState;
  ...
  setTaskPermissionRequest: (
    workspacePath: string,
    taskId: string,
    request: GCodePermissionRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  setTaskElicitationRequest: (
    workspacePath: string,
    taskId: string,
    request: GCodeElicitationRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  /** 任务列表变更时调用（创建/删除任务），驱动 gcodeTaskMetaMerge 重新拉取列表 */
...
const FALLBACK_PROVIDER: GCodeProvider = GCODE_AGENT_PROVIDER;
...
export function getDefaultWorkspaceState(): WorkspaceGCodeUIState {
  // 单 GCode Agent 迁移后默认 provider 必须收敛到 glm。
