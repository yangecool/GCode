...
import type { GCodeTaskListSortBy, IGCodeTaskService } from "@gcode/services";
...
interface RemoteTimelineTaskState {
  itemsByWorkspaceKey: Record<string, GCodeTaskMeta[]>;
  ...
  refreshWorkspace: (params: {
    ...
    gcodeTaskService: Pick<IGCodeTaskService, "listTaskList">;
    sortBy: GCodeTaskListSortBy;
    limit?: number;
  }) => Promise<void>;
  upsertTask: (task: GCodeTaskMeta, sortBy?: GCodeTaskListSortBy) => void;
...
function sortTimelineTasks(
  tasks: GCodeTaskMeta[],
  sortBy: GCodeTaskListSortBy = "updated",
): GCodeTaskMeta[] {
...
export const useRemoteTimelineTaskStore = create<RemoteTimelineTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, gcodeTaskService, sortBy, limit }) {
    ...
    try {
      const result = await gcodeTaskService.listTaskList({
        kind: "timeline",
        workspaceScopes: [
          {
            workspacePath,
...
export async function refreshRemoteTimelineTasksForSession({
  ...
  await useRemoteTimelineTaskStore.getState().refreshWorkspace({
    workspacePath,
    workspaceIdentity,
    gcodeTaskService: session.services.gcodeTaskService,
    sortBy,
    limit,
  });
}
