...
import { getDefaultWorkspaceState } from "@/store/gcodeSessionStoreTypes.js";
...
function sortTasksByUpdatedAt(tasks: readonly GCodeTaskMeta[]): GCodeTaskMeta[] {
...
function arePermissionOptionsEqual(
  left: GCodePermissionRequest["options"],
  right: GCodePermissionRequest["options"],
): boolean {
...
function arePermissionRequestsEquivalent(
  left: GCodePermissionRequest,
  right: GCodePermissionRequest,
): boolean {
  if (left === right) {
    return true;
  }
          ...
          (current) => ({
            ...
            taskUiByTaskId: {
              ...
              [taskId]: {
                ...
                // 完整错误对象之前只放在 ChatView/useGCodeChat 的本地 state。
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => {
            ...
            return {
              ...current,
              selectedProvider: normalizeAgentProviderToGCodeAgent(params.provider),
