...
interface RemotePinnedTaskState {
  ...
  refreshWorkspace: (params: {
    ...
    gcodeTaskService: Pick<IGCodeTaskService, "listPinnedTasks">;
  }) => Promise<void>;
  upsertTask: (task: GCodeTaskMeta) => void;
...
function sortPinnedTasks(tasks: GCodeTaskMeta[]): GCodeTaskMeta[] {
...
export const useRemotePinnedTaskStore = create<RemotePinnedTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, gcodeTaskService }) {
    ...
    try {
      const items = await gcodeTaskService.listPinnedTasks({
        workspacePath,
