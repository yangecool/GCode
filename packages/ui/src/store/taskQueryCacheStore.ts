...
import type { GCodeTaskListItem } from "@gcode/services";
...
interface TaskQueryCacheState {
  ...
  upsertTaskMeta: (task: GCodeTaskMeta) => void;
  updateTaskMetaPreservingMembership: (task: GCodeTaskMeta) => void;
  applyTaskMutation: (params: {
    previousTask: GCodeTaskMeta;
...
type CachedTaskListItem = GCodeTaskListItem & { searchSnippets?: string[] };
...
function matchesTaskSearch(descriptor: TaskListCacheDescriptor, task: GCodeTaskMeta): boolean {
...
function sortTaskKeysByDescriptor(params: {
  taskKeys: TaskEntityKey[];
  taskMetaByEntityKey: Record<TaskEntityKey, GCodeTaskListItem>;
  descriptor: TaskListCacheDescriptor;
}): TaskEntityKey[] {
  ...
  return uniqueTaskKeys.sort((leftKey, rightKey) => {
    ...
    return compareGCodeTaskListItems(leftTask, rightTask, params.descriptor.sortBy);
  });
}
