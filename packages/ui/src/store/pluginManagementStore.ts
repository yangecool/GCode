...
export interface PluginDescribeEntry {
  ...
  data?: GCodePluginsDescribeResult;
  error?: string;
}

// 设置页「插件管理」的数据源: 经 IPluginManagementService 薄服务由 gcode-cli 提供 (list + enable/disable)。
// UI 不再直触 IGCodeAgentService，plugins/* 旧协议词的消费收拢到服务实现一处。
...
export interface PluginManagementState {
  ...
  diagnostics: GCodePluginDiagnostic[];
  loading: boolean;
  error: string | null;
  /**
...
function throwForErrorDiagnostic(diagnostics: GCodePluginDiagnostic[]): void {
