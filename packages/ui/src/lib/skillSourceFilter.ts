...
function resolveSkillSourceType(skillPath: string): SkillSourceType {
  ...
  if (normalized.includes("/.gcode/skills/")) {
    return "glm";
  }
  if (normalized.includes("/.gcode/cli/plugins/cache/")) {
    return "glm";
  }
  return "unknown";
}
...
function isGcodeSkill(skill: { id?: string; path: string; scope?: string }): boolean {
  return (
    // plugin skill 的真实路径在 CLI plugin cache 下，不在 `.gcode/skills`。
...
export function filterSkillsForProvider<T extends { path: string; id?: string; scope?: string }>(
  skills: T[],
  _legacyProvider: GCodeProvider,
): T[] {
  return skills.filter(isGcodeSkill);
}
