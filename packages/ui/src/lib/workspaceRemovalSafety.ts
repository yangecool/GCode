import type { FileEntry, GCodeTaskMeta, GCodeTaskRuntimeStatus } from "@gcode/shared";
import type { IFileService } from "@gcode/services";
import type { WorkspaceGCodeUIState } from "@/store/gcodeSessionStoreTypes.js";
...
export function hasRunningWorkspaceChat(params: {
  workspaceState: Pick<WorkspaceGCodeUIState, "draftRuntime" | "taskRuntimeByTaskId">;
  taskItems: Pick<GCodeTaskMeta, "taskId">[];
}): boolean {
...
function isWorkspaceRemovalBlockingRuntimeStatus(status: GCodeTaskRuntimeStatus): boolean {
