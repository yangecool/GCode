import {
  getGCodeToolFamilyForName,
  isTodoPlanToolName,
  isGCodeFileContentWriteToolName,
  normalizeGCodeToolName,
  type GCodeKnownToolName,
  type GCodeToolFamily,
} from "@gcode/shared";
...
export interface ToolCallIdentity {
  toolName: GCodeKnownToolName | string | null;
...
function identityFromKnownToolName(
  ...
  const family = getGCodeToolFamilyForName(toolName);
  if (!family) {
    return null;
  }
  return {
...
function readRawToolNameCandidates(raw: unknown) {
  return {
    ...
    gcode: readNestedString(raw, ["_meta", "gcode", "toolName"]),
...
function isLegacyAgentTool(
  ...
  const rawGCode = normalizeLegacyToken(rawNames.gcode);
...
export function resolveToolCallIdentity(toolCall: ToolIdentityLike): ToolCallIdentity {
  ...
  for (const candidate of [
    ...
    { value: rawNames.gcode, source: "raw-gcode-meta" as const },
  ]) {
  ...
  if (isLegacyAgentTool(toolCall, rawNames)) {
    return identityFromLegacyFamily(
      rawNames.direct ?? rawNames.gcode ?? "Agent",
      "agent",
      "legacy-payload",
    );
  }

  // `Task` 现在是现役 Claude 兼容工具名，但历史 GCode Agent 投影会用
  ...
  if (
    // 当前 GCode Agent 传给 app 的 plan mode 退出工具是 ExitPlanMode，
...
export function isFileContentWriteToolCall(
  ...
  if (isGCodeFileContentWriteToolName(identity.toolName)) {
    return true;
  }
