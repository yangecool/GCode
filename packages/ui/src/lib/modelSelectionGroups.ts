...
function supportsRegistryApiFormat(
  ...
  // 仅剩 glm（GCode Agent）provider；三方 CLI 的 api format 差异已随 provider 下线。
  return isGCodeAgentProvider(selectedProvider);
}
...
export function buildRegistryModelSelectGroups(
  ...
  return view.providers.flatMap((provider) => {
    ...
    const accountAccess = gcodeProviderAccountAccessSchema.safeParse(provider.config.access);
