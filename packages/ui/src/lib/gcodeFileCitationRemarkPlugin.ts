...
import { extractGCodeFileCitations } from "@/lib/gcodeFileCitation.js";
...
function projectCitationTextNode(
  ...
  const citations = extractGCodeFileCitations(value);
...
export function createGCodeFileCitationRemarkPlugin(
  workspacePath: string,
  homePath?: string,
): Plugin {
  return function gcodeFileCitationRemarkPlugin() {
    return (tree: unknown) => {
