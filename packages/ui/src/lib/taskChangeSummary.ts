...
export function getTaskChangeSummary(
  task: Pick<GCodeTaskMeta, "changeSummary"> | null | undefined,
): GCodeTaskChangeSummary | null {
...
function formatTaskChangeStats(
  summary: GCodeTaskChangeSummary,
...
export function buildTaskChangeSummary(
  fileChanges: readonly GCodePersistedFileChange[] | undefined,
): GCodeTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: GCodeTaskChangedFileSummary[] = Array.from(changedFileMap.values())
    .map((file) => {
...
export function buildTurnChangeSummary(
  turn: GCodePersistedFileChange | null | undefined,
): GCodeTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: GCodeTaskChangedFileSummary[] = Array.from(filesByPath.entries())
    .map(([path, snapshot]) => {
...
export function buildPerTurnChangeSummaries(
  fileChanges: readonly GCodePersistedFileChange[] | undefined,
): Map<number, GCodeTaskChangeSummary> {
  const result = new Map<number, GCodeTaskChangeSummary>();
  if (!fileChanges || fileChanges.length === 0) {
    return result;
  }
