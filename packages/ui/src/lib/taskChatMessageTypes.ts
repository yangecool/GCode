// re-home 产物（为删除 gcodeChatMessages 铺路，模式同 gcode-task-types-core）。
// 本文件承载旧消息转换层（gcodeChatMessages/gcodeChatMessageHelpers）中仍被存活栈
...
// 旧消息拼装运行时（gcodeChatMessages.ts）删除后，这里是该类型面的唯一事实源。
import type {
  GCodeAssistantMessagePart,
  GCodeAssistantMessageFeedback,
  GCodePromptAttachment,
  GCodeTimelineMeta,
  GCodeTaskSnapshotBodyRef,
  GCodeTaskSnapshotToolFieldRef,
  GCodeTaskSnapshotToolSlice,
} from "@gcode/shared";
...
export interface TaskChatToolCall {
  ...
  /** GCode 固定工具名；kind 仍保留给旧 GCode Agent 快照和 UI 聚合分类兼容。 */
  toolName?: string;
  kind: string;
  title?: string;
  input: unknown;
...
export type TaskChatMessagePart = GCodeAssistantMessagePart;
