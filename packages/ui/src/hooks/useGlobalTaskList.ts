...
function buildWorkspaceScopes(workspaceTabs: WorkspaceTabState[]): GCodeTaskListWorkspaceScope[] {
  const scopes = new Map<string, GCodeTaskListWorkspaceScope>();
...
export function useGlobalTaskList(params: {
  ...
  const taskListVersionSignature = useGCodeSessionStore((state) =>
    JSON.stringify(
      params.workspaceTabs
        .map((tab) => {
          const workspace = selectWorkspaceGCodeState(
            state,
            tab.workspacePath,
