...
import { APP_RUNTIME_PREFERENCES_CHANGED_BROADCAST_CHANNEL, type AppSettings } from "@gcode/shared";
...
export function useSettings() {
  const { broadcastService, settingService, gcodeAgentService } = useServices();
  ...
  const update = useCallback(
    async (patch: Partial<AppSettings>) => {
      ...
      if (
        ...
        const syncResults = await Promise.allSettled([
          gcodeAgentService.syncAppRuntimePreferences(preferences),
        ]);
    ...
    [broadcastService, settingService, settingsStore, gcodeAgentService, platform, refresh],
  );
