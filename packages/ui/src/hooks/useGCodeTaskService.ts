...
import type { GCodeTaskSnapshot } from "@gcode/shared";
import { uiMemoryDiagnosticsRegistry } from "@/lib/memoryDiagnostics.js";

type GetTaskSnapshotParams = Parameters<IGCodeTaskService["getTaskSnapshot"]>[0];
type GetTaskSnapshotResult = Promise<GCodeTaskSnapshot | null>;
type GetTaskSnapshotWithEtagParams = Parameters<IGCodeTaskService["getTaskSnapshotWithEtag"]>[0];

const gcodeTaskServiceProxyCache = new WeakMap<IGCodeTaskService, IGCodeTaskService>();
...
const snapshotCacheByService = new WeakMap<
  IGCodeTaskService,
  Map<string, { etag: string; snapshot: GCodeTaskSnapshot }>
>();
const SNAPSHOT_CACHE_STORAGE_KEY = "gcode-task-snapshot-cache:v1";
...
let latestSnapshotCache: Map<string, { etag: string; snapshot: GCodeTaskSnapshot }> | undefined;
...
function getOrCreateSnapshotCacheMap(
  ...
  const created = new Map<string, { etag: string; snapshot: GCodeTaskSnapshot }>();
...
function writePersistedSnapshotEntry(key: string, etag: string, snapshot: GCodeTaskSnapshot): void {
  ensurePersistedSnapshotCacheLoaded();
...
function createGCodeTaskServiceProxy(service: IGCodeTaskService): IGCodeTaskService {
...
/** 获取 GCode task wrapper 服务实例 */
export function useGCodeTaskService(
  workspacePath?: string,
  preferredRemoteSessionId?: string | null,
  workspaceIdentity?: string | null,
): IGCodeTaskService {
  // GCode task 服务按 workspace 身份解析，保证所有 task RPC 都落到对应的 host。
  const services = workspacePath
    ? useWorkspaceServices(workspacePath, preferredRemoteSessionId, workspaceIdentity)
    : useServices();
  const rawService = services.gcodeTaskService;
  if (!rawService || typeof rawService !== "object") {
    return rawService;
  }
  const cachedProxy = gcodeTaskServiceProxyCache.get(rawService);
  if (cachedProxy) {
    return cachedProxy;
  }
  const nextProxy = createGCodeTaskServiceProxy(rawService);
  gcodeTaskServiceProxyCache.set(rawService, nextProxy);
  return nextProxy;
}
