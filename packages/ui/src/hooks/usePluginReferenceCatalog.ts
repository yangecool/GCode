...
let sessionCatalogRequests = new WeakMap<
  object,
  Map<string, Promise<GCodePluginsReferenceCatalogResult>>
>();

function releaseSessionCatalogRequest(
  services: object,
  serviceCache: Map<string, Promise<GCodePluginsReferenceCatalogResult>>,
  requestKey: string,
  request: Promise<GCodePluginsReferenceCatalogResult>,
): void {
...
export function usePluginReferenceCatalog(
  ...
  useEffect(() => {
    ...
    let request: Promise<GCodePluginsReferenceCatalogResult>;
    let requestServiceCache: Map<string, Promise<GCodePluginsReferenceCatalogResult>> | undefined;
