...
import type {
  ...
  RemoteTarget,
  GCodePluginOptionValue,
  GCodePluginInfo,
  GCodePluginMarketplaceSummary,
  GCodePluginUserConfigOption,
  GCodePluginsOverviewResult,
} from "@gcode/shared";
import type { IPluginSyncService, IGCodeAgentService } from "@gcode/services";
...
interface RemotePluginSyncPluginOptions {
  configuredOptions: Record<string, GCodePluginOptionValue>;
  userConfig: Record<string, GCodePluginUserConfigOption>;
}
...
interface RemotePluginSyncDialogProps {
  ...
  localGCodeAgentService?: RemotePluginSyncAgentService | null;
  remoteGCodeAgentService?: RemotePluginSyncAgentService | null;
...
function buildMarketplaceRemotePluginSyncCandidates(
  overview: GCodePluginsOverviewResult,
  plugins: readonly GCodePluginInfo[],
): RemotePluginSyncCandidate[] {
...
function buildMarketplaceRemotePluginStatuses(
  candidates: readonly RemotePluginSyncCandidate[],
  remoteOverview: GCodePluginsOverviewResult,
...
function componentTypesFromPluginInfo(plugin: GCodePluginInfo | undefined): string[] {
  return plugin?.components?.map((component) => component.kind) ?? [];
}

function buildRemotePluginSyncPluginOptions(
  plugin: GCodePluginInfo | undefined,
...
function serializeMarketplaceSourceInput(
  marketplace: GCodePluginMarketplaceSummary,
): string | undefined {
...
async function loadRemotePluginSyncCandidates(params: {
  ...
  if (!params.localGCodeAgentService || !params.remoteGCodeAgentService) {
  ...
  const [localOverview, localPluginsResult, remoteOverview] = await Promise.all([
    params.localGCodeAgentService.getPluginsOverview({
      workspacePath: params.localWorkspacePath,
    }),
    params.localGCodeAgentService.listPlugins({
      workspacePath: params.localWorkspacePath,
    }),
    params.remoteGCodeAgentService.getPluginsOverview({
...
function isPathPluginOption(option: GCodePluginUserConfigOption | undefined): boolean {
  return option?.type === "file" || option?.type === "directory";
}

function isPortablePluginOptionValue(
  value: GCodePluginOptionValue,
  localOption: GCodePluginUserConfigOption | undefined,
  remoteOption: GCodePluginUserConfigOption | undefined,
): boolean {
...
function buildPortablePluginOptionsSyncPlan(
  ...
  remotePlugin: GCodePluginInfo,
): { options: Record<string, GCodePluginOptionValue>; skippedCount: number } {
  const options: Record<string, GCodePluginOptionValue> = {};
  let skippedCount = 0;
...
function arePluginOptionsEqual(
  left: Record<string, GCodePluginOptionValue>,
  right: Record<string, GCodePluginOptionValue>,
): boolean {
...
async function syncPortablePluginOptions(
  ...
  const remoteGCodeAgentService = params.remoteGCodeAgentService;
  if (!remoteGCodeAgentService) {
    emitRemotePluginSyncProgress(
      params,
      row,
      "syncing",
  ...
  const remotePlugins = await awaitRemotePluginSyncStep(
    ...
    () => remoteGCodeAgentService.listPlugins(workspace),
  );
  ...
  await awaitRemotePluginSyncStep(
    ...
    () =>
      remoteGCodeAgentService.configurePlugin({
        ...workspace,
...
async function syncMarketplaceRemotePlugin(
  ...
  try {
    throwIfRemotePluginSyncStopped(params, row);
    if (!params.remoteGCodeAgentService || !marketplacePlugin) {
    ...
    const installResult = await awaitRemotePluginSyncStep(
      ...
      (operationId) =>
        remoteGCodeAgentService.installPlugin({
          ...workspace,
    ...
    await awaitRemotePluginSyncStep(
      ...
      () =>
        remoteGCodeAgentService.setPluginEnabled({
          ...workspace,
          enabled: candidate.enabled,
          pluginId: candidate.pluginId,
          scope: REMOTE_PLUGIN_SYNC_CONFIG_SCOPE,
...
async function prepareRemoteMarketplaceSource(
  params: {
    ...
    onItemProgress?: (event: RemotePluginSyncProgressEvent) => void;
    remotePluginSyncService: IPluginSyncService;
    remoteGCodeAgentService: RemotePluginSyncAgentService;
    selectedMarketplacePluginNames: ReadonlyMap<string, string[]>;
    signal?: AbortSignal;
    stopControl?: RemotePluginSyncStopControl;
  },
  ...
  if (marketplaceSourceInput) {
    await awaitRemotePluginSyncStep(
      ...
      (operationId) =>
        params.remoteGCodeAgentService.addPluginMarketplace({
          ...workspace,
          operationId,
      ...
      const result = await syncSelectedRemotePlugins({
        ...
        stopControl: {
          cancelOperation: async (operationId) => {
            if (!remoteGCodeAgentService?.cancelPluginOperation) {
              return;
            }
            await remoteGCodeAgentService.cancelPluginOperation({ operationId });
          },
