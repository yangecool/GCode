import type {
  ...
  GCodePluginInfo,
  GCodePluginComponentKind,
  GCodePluginsDescribeResult,
} from "@gcode/shared";
import { isPluginCommand, isUserCommand, GCODE_COMMAND_AGENT_SOURCE } from "@gcode/shared";
...
const COMPONENT_KIND_ORDER: GCodePluginComponentKind[] = [
  "agent",
  "command",
  "skill",
  "hook",
...
export function describeResultToDisplayGroups(
  result: GCodePluginsDescribeResult,
): PluginComponentDisplayGroup[] {
  const byKind = new Map<GCodePluginComponentKind, PluginComponentDisplayGroup>();
...
export interface PluginMcpServerItem {
  active: boolean;
  authorization?: GCodeMcpServerStatusSnapshot["authorization"];
  error?: string;
  failureKind?: GCodeMcpServerStatusSnapshot["failureKind"];
  id: string;
...
export function groupCommandsByPlugin(
  ...
  for (const command of commands) {
    if (
      ...
      command.agentSource === GCODE_COMMAND_AGENT_SOURCE &&
...
export function filterLocalMcpServers(servers: GCodeMcpServer[], query: string): GCodeMcpServer[] {
  return servers.filter((server) => {
    if (server.source !== "gcodeagentmcp") {
      return false;
    }
...
function toPluginMcpServerDisplayName(plugin: GCodePluginInfo, serverName: string): string {
...
function resolvePluginMcpRuntimeServerName(plugin: GCodePluginInfo, displayName: string): string {
...
function mapPluginRuntimeStatus(
  snapshot: GCodeMcpServerStatusSnapshot | undefined,
): Pick<
  PluginMcpServerItem,
