...
type DetailLoad =
  | { status: "loading" }
  | { status: "ready"; detail: Extract<GCodeWorkflowsGetResult, { ok: true }> }
  | { status: "failed"; reason: string };

type MetaDraft = SavedWorkflowMetaDraft;

function draftFromMeta(meta: GCodeSavedWorkflowMeta): MetaDraft {
  return {
...
export function SavedWorkflowDetailView({
  ...
  const save = useCallback(async () => {
    ...
    const meta: GCodeSavedWorkflowMeta = {
      description,
