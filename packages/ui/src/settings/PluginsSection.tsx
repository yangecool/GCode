...
import { TID_PLUGIN_STORE_BROWSE } from "@gcode/shared";
import type { GCodePluginInfo, GCodePluginScope, GCodePluginUserConfigOption } from "@gcode/shared";
...
function PluginList({
  ...
  const getPluginOptionValue = (
    ...
    key: string,
    option: GCodePluginUserConfigOption,
  ...
  const savePluginOptions = async (plugin: GCodePluginInfo) => {
  ...
  const renderPluginRows = (items: GCodePluginInfo[]) => (
  ...
  return (
    <section className="space-y-6" aria-labelledby="plugin-installed-title">
      ...
      <RemoteSyncDialogs
        ...
        remoteGCodeAgentService={targetServiceResolution.services.gcodeAgentService}
