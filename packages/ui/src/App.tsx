...
import type { GitChangeSourceId, WorkspacePurpose } from "@gcode/shared";
...
import { GCODE_PRODUCT_DOCS_URL } from "@/lib/productDocs.js";
...
import { useWorkspaceShellGCodeState } from "@/app-shell/useWorkspaceShellGCodeState.js";
...
export function App({
  ...
  const workspaceReadOnlyReason = workspaceReadOnly
    ...
    : undefined;
  const { workspaceShellGCodeState, reloadSessionDisabled } = useWorkspaceShellGCodeState(
    workspaceAbsPath,
    workspaceIdentity,
  );
  const activeTaskId = workspaceShellGCodeState.activeTaskId;
  ...
  const draftSessionId = useGCodeSessionStore(
  ...
  const draftFocusVersion = workspaceShellGCodeState.draftFocusVersion;
  const {
    isTerminalOpen,
    setIsTerminalOpen,
    sidePaneState,
  ...
  const notificationEnabled = useGCodeStore((s) => s.notificationEnabled);
  ...
  const {
    ...
    workspaceRemoteSessionId,
    workspaceIdentity,
    selectedProvider: workspaceShellGCodeState.selectedProvider,
    intl,
  });
  ...
  const handleOpenProductDocs = useCallback(() => {
    platform.openExternal(GCODE_PRODUCT_DOCS_URL);
  }, [platform]);
  ...
  const handleStartDraftInWorkspace = useCallback(
    (
      ...
      createSource?: import("@gcode/shared").SessionCreateSource,
    ) => {
      ...
      const targetSelectedProvider = resolveWorkspaceSwitchDraftProvider({
        currentSelectedProvider: workspaceShellGCodeState.selectedProvider,
        targetWorkspacePath,
    ...
    [
      ...
      workspaceAbsPath,
      workspaceIdentity,
      workspaceShellGCodeState.selectedProvider,
    ],
  );
  ...
  const testActions = useMemo<TestActions>(
    () => ({
      ...
      getPluginsOverview: (params) => services.gcodeAgentService.getPluginsOverview(params),
      addPluginMarketplace: (params) => services.gcodeAgentService.addPluginMarketplace(params),
      updatePluginMarketplace: (params) =>
        services.gcodeAgentService.updatePluginMarketplace(params),
      installPlugin: (params) => services.gcodeAgentService.installPlugin(params),
      listPlugins: (params) => services.gcodeAgentService.listPlugins(params),
      getPluginReferenceCatalog: (params) =>
        services.gcodeAgentService.getPluginReferenceCatalog(params),
    }),
    [locale, services.gcodeAgentService, setLocale, theme, setTheme, testMessages],
  );
  ...
  const handleSelectAdjacentConversation = useCallback(
    (direction: "previous" | "next") => {
      runVisibleWorkspaceCommand(() => {
        const sessionState = useGCodeSessionStore.getState();
