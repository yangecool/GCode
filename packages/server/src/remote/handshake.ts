...
import { GCODE_VERSION, formatZodError, helloMessageSchema } from "@gcode/shared";
 ...
 * 1. Read lines from stdout until we find a gcode-hello JSON
 *    (skip SSH banner/motd lines)
 * 2. Send a gcode-hello-ack to stdin
...
export function performHandshake(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      rejectWithDiagnostics("Handshake timeout: no gcode-hello received within timeout");
    }, timeoutMs);
        ...
        if (line.startsWith("{")) {
          try {
            ...
            if (
              ...
              "type" in rawValue &&
              (rawValue as { type?: unknown }).type === "gcode-hello"
            ) {
              rejectWithDiagnostics(`Invalid gcode-hello: ${formatZodError(result.error)}`);
              return;
            }
          } catch {
