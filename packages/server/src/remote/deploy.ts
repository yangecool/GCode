...
import { deployGCodeAgentRuntime } from "./gcodeAgentDeploy.js";
import {
  deployNodePtyPrebuilds,
  deployNodeRuntime,
  createRemoteComponentVersionResolver,
  logDeployRequired,
} from "@gcode/server/remote/remoteAssetDeployDecision.js";
import {
  REMOTE_BASE,
  fileExists,
  formatOptionalValue,
...
import { checkServerBundleRequiredMarkers } from "@gcode/server/remote/serverBundleDeployCheck.js";
import { deployRuntimeTools } from "@gcode/server/remote/runtimeToolDeploy.js";
import { REMOTE_AGENT_OFFICIAL_PLUGIN_REQUIRED_RELATIVE_PATHS } from "@gcode/server/remote/gcodeAgentOfficialPluginAssets.js";
import {
...
} from "@gcode/server/remote/remoteAssetInstaller.js";
import {
...
import { detectRemoteAssetTools } from "@gcode/server/remote/remoteAssetPreflight.js";
import { assertSupportedRemoteEnvironment } from "@gcode/server/remote/remotePlatformSupport.js";
import { acquireRemoteDeployLock } from "@gcode/server/remote/remoteDeployLock.js";
import type { RemoteAssetNetworkPort } from "@gcode/server/remote/remoteAssetNetwork.js";
...
/**
 * Deploy the gcode server to the remote machine.
...
export async function deployServer(
  ...
  const deployWithDecision = async (
    ...
    if (!serverDeployDecision.shouldDeploy) {
      ...
      // 主 server 版本相同只证明 node/gcode-server.cjs 可启动，不代表随包工具仍存在。
    ...
    await installer.installFile({
      componentId: SERVER_BUNDLE_COMPONENT_ID,
      sourceRelativePath: "server/gcode-server.cjs",
      remotePath: `${REMOTE_BASE}/gcode-server.cjs`,
    ...
    // 部署 GCode Agent runtime 到远程，历史资源包选择已在入口统一忽略。
    await deployGCodeAgentRuntime(
      backend,
      env,
      {
...
async function checkServerDeployDecision(
  ...
  try {
    ...
    const serverPath = `${REMOTE_BASE}/gcode-server.cjs`;
    ...
    log("remote version:", JSON.stringify(version), "local:", GCODE_VERSION);
    if (version !== GCODE_VERSION) {
      return {
        shouldDeploy: true,
        reason: `remote server version mismatch remote=${version} expected=${GCODE_VERSION}`,
...
function resolveMockCdnReleaseDir(mockCdnDir?: string): string | null {
  ...
  return join(mockCdnDir, "releases", GCODE_VERSION);
}
  ...
  for (const componentId of ids) {
    switch (componentId) {
      case SERVER_BUNDLE_COMPONENT_ID:
        requiredPaths.add("server/gcode-server.cjs");
        break;
      case "node-runtime":
      ...
      case "node-pty":
        ...
        break;
      case "glm":
        requiredPaths.add(`glm/${platformArch}/gcode.cjs`);
