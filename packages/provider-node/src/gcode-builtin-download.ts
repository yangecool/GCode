import { z } from "zod";
import { decodeGCodeBuiltinRelease, type GCodeBuiltinRelease } from "./gcode-builtin-release.js";
...
export interface GCodeBuiltinDownloadOptions {
...
export async function downloadGCodeBuiltinRelease(
  options: GCodeBuiltinDownloadOptions,
  ...
  try {
    ...
    stage = "cdn";
    return decodeGCodeBuiltinRelease(await readJson(new URL(downloadUrl)));
  } catch (error) {
    ...
    const reason = signal.aborted
      ...
      : error instanceof DownloadBoundaryError
        ...
        : error instanceof z.ZodError
          ...
          : "invalid response";
    throw new Error(`GCode Built-in ${stage}: ${reason}`);
  } finally {
    clearTimeout(timer);
  }
