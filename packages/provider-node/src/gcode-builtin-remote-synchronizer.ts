...
import type { NodeGCodeBuiltinProviderConfigSource } from "./gcode-builtin-provider-config-source.js";
import type { GCodeBuiltinRelease } from "./gcode-builtin-release.js";

const HOUR_MS = 60 * 60 * 1_000;

export type GCodeBuiltinRefreshResult =
  | "updated"
  | "unchanged"
  | "stale"
  | "missing"
...
export interface GCodeBuiltinRemoteSynchronizerOptions {
  ...
  readonly onRefreshResult?: (event: GCodeBuiltinRefreshEvent) => void;
  readonly now?: () => number;
...
export interface GCodeBuiltinRefreshEvent {
  readonly result: GCodeBuiltinRefreshResult;
...
export class GCodeBuiltinRemoteSynchronizer {
  readonly #options: GCodeBuiltinRemoteSynchronizerOptions;
  readonly #now: () => number;
  #inFlight: Promise<GCodeBuiltinRefreshResult> | null = null;
  #disposed = false;
  #abortController: AbortController | null = null;

  constructor(options: GCodeBuiltinRemoteSynchronizerOptions) {
    this.#options = options;
    this.#now = options.now ?? Date.now;
  }

  refresh(options: { readonly force?: boolean } = {}): Promise<GCodeBuiltinRefreshResult> {
  ...
  async #refresh(force: boolean, signal: AbortSignal): Promise<GCodeBuiltinRefreshResult> {
    const endpointKey = (await this.#options.resolveEndpointKey()).trim();
    if (this.#disposed) return "disposed";
    if (!endpointKey) throw new Error("GCode Built-in 远端 Endpoint 不能为空");
  ...
  #report(event: GCodeBuiltinRefreshEvent): void {
