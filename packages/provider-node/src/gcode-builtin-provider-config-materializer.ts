...
export interface MaterializeGCodeBuiltinProviderConfigOptions {
 ...
 */
export async function materializeGCodeBuiltinProviderConfig(
  options: MaterializeGCodeBuiltinProviderConfigOptions,
): Promise<string> {
  const content = `${serializeGCodeBuiltinRelease(
    decodeGCodeBuiltinRelease(JSON.parse(options.content)),
  )}\n`;
  const filePath = join(
