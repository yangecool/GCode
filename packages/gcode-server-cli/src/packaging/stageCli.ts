...
async function ensureNodeBinary(repoRoot: string, target: ServerTarget): Promise<string> {
  ...
  const cacheDir = join(
    repoRoot,
    "node_modules/.cache/gcode-server-cli",
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "gcode-server-node-"));
  try {
...
async function ensureAgentBundle(repoRoot: string, skipBuild: boolean): Promise<string> {
  const bundlePath = join(repoRoot, "apps/gcode-cli/packages/cli/dist/gcode.cjs");
...
export async function resolveNativeToolsDir(
  ...
  const cacheDir = join(repoRoot, "node_modules/.cache/gcode-server-cli", "tools", target);
...
async function resolveWorkspacePackageDirs(repoRoot: string): Promise<Map<string, string>> {
  ...
  for (const root of [join(repoRoot, "apps/gcode-cli/packages"), join(repoRoot, "packages")]) {
...
async function main(): Promise<void> {
  ...
  if (!(await pathExists(join(distDir, "server-cli.js")))) {
    throw new Error(
      `Missing tsup output in ${distDir}; run pnpm --filter @gcode/server-cli build first`,
    );
  }
  ...
  const [nativeToolsDir, officialPluginsDir, workspacePackageDirs] = await Promise.all([
    ...
    Promise.resolve(join(repoRoot, "apps/gcode-cli/packages")),
