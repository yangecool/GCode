...
import { IGCodeAgentService } from "@gcode/services";
...
export async function runServerCore(generation: number): Promise<void> {
  ...
  const explicitGCodeBuiltinProviderConfigFilePath =
    process.env[GCODE_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  const gcodeBuiltinProviderConfigFilePath = explicitGCodeBuiltinProviderConfigFilePath
    ? explicitGCodeBuiltinProviderConfigFilePath
    : typeof __GCODE_BUILTIN_PROVIDER_CONFIG_JSON__ === "string"
      ? await materializeGCodeBuiltinProviderConfig({
          environmentConfigRoot: getAppConfigDir(),
          content: __GCODE_BUILTIN_PROVIDER_CONFIG_JSON__,
        })
      : undefined;
  if (!gcodeBuiltinProviderConfigFilePath) {
    throw new Error(
      `当前构建未嵌入 GCode Built-in Provider Config，且未设置 ${GCODE_BUILTIN_PROVIDER_CONFIG_FILE_ENV}`,
    );
  }
  ...
  const taskActivityTracker = createTaskActivityTracker(services.getOptional(IGCodeAgentService));
