...
const SERVER_INSTALL_OWNERSHIP_PRODUCT = "gcode-server";
...
async function readOwnership(layout: ServerLayout): Promise<ServerInstallOwnership> {
  ...
  if (!markerStat?.isFile()) {
    throw new Error(`GCode Server ownership marker is missing or invalid: ${layout.installFile}`);
  }
  try {
    ...
    throw new Error(`GCode Server ownership marker is invalid: ${layout.installFile}`, {
      cause: error,
    });
  }
}
...
export async function validateServerInstallOwnership(
  ...
  const [ownership, canonicalServerRoot] = await Promise.all([
    ...
    realpath(layout.serverRoot).catch((error: unknown) => {
      throw new Error(`GCode Server ownership root cannot be resolved: ${layout.serverRoot}`, {
        cause: error,
      });
    }),
  ]);
  if (ownership.canonicalServerRoot !== canonicalServerRoot) {
    throw new Error(
      `GCode Server ownership root mismatch: expected ${ownership.canonicalServerRoot}, received ${canonicalServerRoot}`,
    );
  }
  return ownership;
}
