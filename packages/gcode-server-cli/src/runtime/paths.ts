...
function getDefaultServerDataRoot(): string {
  const configured = process.env.GCODE_DATA_BASE_DIR?.trim();
  return join(configured || homedir(), ".gcode", "server");
}
  ...
  return {
    ...
    controlEndpoint:
      process.platform === "win32"
        ? `\\\\.\\pipe\\gcode-server-${stablePathId(root)}`
...
function inferDataBaseDir(serverRoot: string): string {
  ...
  if (basename(serverRoot) === "server" && basename(parent) === ".gcode") {
