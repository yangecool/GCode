  ...
  async #runRefreshLoop(): Promise<AccountProviderConfigSnapshot> {
    ...
    while (this.#pendingReasons.size > 0) {
      ...
      try {
        ...
        const configuredProviders = config.gcodeBuiltinProviders;
        const { providers, states } = await this.#resolve({
          configRevision: config.gcodeBuiltinRevision,
          configuredProviders,
        ...
        if (
          ...
          currentConfig.gcodeBuiltinRevision !== config.gcodeBuiltinRevision
        ) {
          this.#pendingReasons.add("superseded-resolution");
          continue;
        }
        const basedOnGCodeBuiltinRevision = config.gcodeBuiltinRevision;
