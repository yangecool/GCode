...
import type { ModelSelection } from "@gcode/shared/model-selection";
...
export interface ProviderConfigServiceDependencies {
  readonly gcodeBuiltinSource: ProviderSource<ProviderConfigLayerSnapshot>;
...
export class ProviderConfigService implements ProviderSource<ProviderConfigSnapshot> {
  ...
  constructor(dependencies: ProviderConfigServiceDependencies) {
    this.#gcodeBuiltinSource = dependencies.gcodeBuiltinSource;
    this.#personalRepository = dependencies.personalRepository;
    this.#sourceDisposers = [
      this.#gcodeBuiltinSource.onDidChange((reason) => this.#emit(`gcodeBuiltin:${reason}`)),
  ...
  async read(): Promise<ProviderConfigSnapshot> {
    ...
    const [gcodeBuiltin, personal] = await Promise.all([
      this.#gcodeBuiltinSource.read(),
      this.#personalRepository.read(),
    ]);
    return Object.freeze({
      revision: JSON.stringify([gcodeBuiltin.revision, personal.revision]),
      gcodeBuiltinRevision: gcodeBuiltin.revision,
      personalRevision: personal.revision,
      gcodeBuiltinProviders: gcodeBuiltin.providers,
      gcodeBuiltinProviderTemplates: gcodeBuiltin.providerTemplates ?? ProviderTemplateMap.empty(),
      personalProviders: personal.providers,
      gcodeBuiltinModelRules: gcodeBuiltin.models,
  ...
  async savePersonalProviderOverlay(
    ...
    const gcodeBuiltin = await this.#gcodeBuiltinSource.read();
    return this.#updatePersonal((current) => {
      assertMembershipCurrent(membership, providerId, current);
      const builtin = gcodeBuiltin.providers.get(providerId);
      const currentPersonal = current.providers.get(providerId);
      const currentEffectiveProviders = gcodeBuiltin.providers.overlay(current.providers);
      ...
      if (builtin?.access?.type === "zhipu-account" && config.access !== undefined) {
        ...
        throw new Error(
          `固定 Account Provider 的 Access 只能由 GCode Built-in Config 声明: ${providerId}`,
        );
      }
      ...
      const membershipBaseline =
        builtin ?? resolveTemplateBaseline(gcodeBuiltin, current.providers, providerId);
      ...
      const nextEffectiveProviders = gcodeBuiltin.providers.overlay(providers);
  ...
  async createPersonalProvider(
    ...
    const template = templateId ? gcodeBuiltin.providerTemplates?.get(templateId) : undefined;
    ...
    await this.#updatePersonal((current) => {
      const occupied = new Set([...gcodeBuiltin.providers.keys(), ...current.providers.keys()]);
      const providerId = nextPersonalProviderId(occupied, templateId);
      createdProviderId = providerId;
      const effectiveProviders = resolvePersonalProviderBaselines(gcodeBuiltin, current.providers);
  ...
  async reorderPersonalProviders(
    ...
    return this.#updatePersonal((current) => ({
      ...
      models: current.models,
      providerOrder: normalizeProviderOrder(gcodeBuiltin.providers, current.providers, providerIds),
    }));
  }
  ...
  async reorderPersonalModels(
    ...
    return this.#updatePersonal((current) => {
      ...
      const builtinProvider =
        gcodeBuiltin.providers.get(providerId) ??
        resolveTemplateBaseline(gcodeBuiltin, current.providers, providerId);
      const provider = writableProviderOverlay(gcodeBuiltin, current, providerId);
  ...
  async addPersonalModel(
    ...
    return this.#updatePersonal((current) => {
      ...
      const provider = writableProviderOverlay(gcodeBuiltin, current, normalizedProviderId);
      const builtinModelIds =
        membership?.inheritedModelIds ??
        resolveProviderBuiltinModelIds(gcodeBuiltin, current.providers, normalizedProviderId);
  ...
  async setPersonalModelEnabled(
    ...
    const builtin = await this.#gcodeBuiltinSource.read();
    return this.#updatePersonal((current) => {
...
function normalizeProviderOrder(
  _gcodeBuiltinProviders: ProviderConfigMap,
...
function appendCurrentProviderOrder(
  gcodeBuiltinProviders: ProviderConfigMap,
  personalProviders: ProviderConfigMap,
  currentOrder: readonly ProviderId[] | undefined,
  addedProviderId: ProviderId,
): ProviderId[] {
  const current = normalizeProviderOrder(
    gcodeBuiltinProviders,
    personalProviders,
    currentOrder ?? [],
  );
  return normalizeProviderOrder(gcodeBuiltinProviders, personalProviders, [
...
function resolvePersonalProviderBaselines(
  ...
  const personal = personalProviders.mapConfigs((config, _id, rule) => {
    const template = rule.templateId
      ? gcodeBuiltin.providerTemplates?.get(rule.templateId)
      : undefined;
    return template ? template.config.overlay(config) : config;
  });
  return gcodeBuiltin.providers.overlay(personal);
}
