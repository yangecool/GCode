...
import {
  ...
  updateGCodeStdioTapDevMenuState,
...
import { resolveGCodeBuiltinProviderConfigFilePath } from "./desktopProviderConfig.js";
import {
  getCredentialsDir,
  ...
  resolveGCodeEndpointEnvBaseOrigin,
  desktopRuntimeEnv,
...
import {
  registerDesktopGCodeDataSizeTelemetry,
  stopDesktopGCodeDataSizeTelemetry,
} from "./desktopGCodeDataSizeTelemetry.js";
...
import { mapGCodeEnvToArmsRumEnv } from "@gcode/shared";
import {
...
// 会和开发态已打开的 GCode Dev 抢端口，导致 WebDriver session 创建前白屏超时。
// 仅本地开发运行默认开启远程调试端口，并允许 e2e 通过环境变量交给 Chromedriver 接管。
if (!app.isPackaged && process.env.GCODE_DISABLE_FIXED_REMOTE_DEBUGGING_PORT !== "1") {
...
const settingsFile = join(homedir(), ".gcode", "v2", "setting.json");
...
async function resolveCurrentGCodeEndpointOrigin() {
  return resolveGCodeEndpointOrigin({
    env: GCODE_ENV,
    envBaseOrigin: resolveGCodeEndpointEnvBaseOrigin(hostProcessLocalEnv),
    overrideOrigin: (await mainSettingService.get()).gcodeEndpointOrigin,
  });
}
...
app.on("browser-window-focus", (_event, win) => {
  ...
  rebuildMenu();
  // 设置/更新等无 Host 的 GCode 窗口也算前台：router 会先把旧 workspace Host 清成 null，
...
const readHelpConfig = createDesktopHelpConfigReader({
  appVersion: GCODE_VERSION || app.getVersion(),
  deviceMid,
...
const localTtftExporter = createLocalTtftExporter({
  ...
  version: GCODE_VERSION || app.getVersion(),
  logger,
});
...
const armsUserIdentitySync = createArmsUserIdentitySync({
  ...
  setUser:
    GCODE_TELEMETRY_ENABLED && GCODE_ARMS_RUM_ENDPOINT
...
async function prepareAppQuit(reason: string, kind: AppShutdownKind = "normal"): Promise<void> {
  ...
  appQuitPreparationInFlight = Promise.all([
    ...
    .finally(() => {
      ...
      // host 还没等到 agent 进程树的 SIGTERM/SIGKILL 兜底完成就被带走，gcode-cli 会被 init 接管成残留进程。
...
function exitPreparedApp(reason: string): never | void {
  ...
  if (process.env.GCODE_E2E_RUN_ID?.trim()) {
...
function shouldConfirmAppQuit() {
  ...
  return GCODE_ENV === "production" && getRunningAgentSessionCount() > 0;
}

function confirmAppQuit(originWindow?: BrowserWindow | null) {
  if (!shouldConfirmAppQuit()) {
    logger.info(`[app-quit] quit confirmation skipped in ${GCODE_ENV}`);
    return true;
  }
...
async function executeDesktopCommandForApp(
  ...
  return executeDesktopCommand({
    ...
    onGCodeEndpointChanged: handleGCodeEndpointChanged,
    gcodeEndpointEnvBaseOrigin: resolveGCodeEndpointEnvBaseOrigin(hostProcessLocalEnv),
    onRelaunchApp: async () => {
...
async function resolveGCodeEndpointSelection(): Promise<"production" | "test" | "custom"> {
  if (GCODE_ENV === "production") {
    return "production";
  }
  const origin = await resolveCurrentGCodeEndpointOrigin();
  if (origin === DEFAULT_GCODE_ENDPOINT_ORIGIN) {
    return "production";
  }
  return "custom";
}

async function handleGCodeEndpointChanged() {
  rebuildMenu();
}
...
function rebuildMenu() {
  void Promise.all([resolveGCodeEndpointSelection(), mainSettingService.get()]).then(
    ([gcodeEndpointSelection, settings]) => {
      rebuildApplicationMenu({
        currentApplicationLocale,
        gcodeEndpointSelection,
  ...
  const win = createWindow({
    ...
    spawnHostProcess: (win, label, initMessage) =>
      spawnHostProcess(
        ...
        {
          ...initMessage,
          gcodeBuiltinProviderConfigFilePath: resolveGCodeBuiltinProviderConfigFilePath({
            env: { ...hostProcessLocalEnv, ...process.env },
          }),
        },
        {
...
app.whenReady().then(async () => {
  ...
  if (process.platform === "win32") {
    ...
    app.setAppUserModelId(
      resolveWindowsAppUserModelIdForFlavor(GCODE_PRODUCT_FLAVOR, { isPackaged: app.isPackaged }),
    );
  }
  ...
  // Preview 身份无论连接哪个后端都不自动更新：stable feed 上只分发正式 GCode 安装包，
  // 不向 Preview 渠道提供更新。
  void initAutoUpdater({
    enabled: GCODE_PRODUCT_FLAVOR === "production",
  ...
  registerRemoteIpcHandlers({
    ...
    armsCustomContext: {
      ...
      armsEnv: mapGCodeEnvToArmsRumEnv(desktopRuntimeEnv),
    },
  ...
  if (GCODE_TELEMETRY_ENABLED && GCODE_ARMS_RUM_ENDPOINT) {
    configureDesktopStabilityTelemetry({
      deviceMid,
      platform: process.platform,
      appVersion: GCODE_VERSION,
  ...
  registerDesktopGCodeDataSizeTelemetry({
    ...
    isGCodeBusy: () => getRunningAgentSessionCount() > 0,
    logger,
    rootPath: getGCodeDataRootDir(),
    stateFile: join(app.getPath("userData"), "gcode-data-size-telemetry.json"),
  });
  registerDesktopNetworkTelemetry(logger);

  // 本地未打包 dev 构建（app.isPackaged === false）必须跳过远端强制升级 gate。
  // 原因：force-update gate 只看 GCODE_ENV === "production"，但 dev 构建（如 dev:desktop:cua
  ...
  const forceUpdateGuardResult =
    GCODE_PRODUCT_FLAVOR === "production" && !skipForceUpdateForLocalDevRuntime
