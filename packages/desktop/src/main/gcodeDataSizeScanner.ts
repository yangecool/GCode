...
export type GCodeDataSizePartialReason = "file_limit" | "io_error" | "time_limit";

export type GCodeDataSizeScanResult = {
  bytes: number;
  ...
  | { status: "partial"; partialReason: GCodeDataSizePartialReason }
);

export interface GCodeDataSizeScanRequest {
  rootPath: string;
  maxDurationMs: number;
  maxFiles: number;
}

interface GCodeDataSizeScanOptions extends GCodeDataSizeScanRequest {
  signal?: AbortSignal;
...
function createAbortError(): DOMException {
  return new DOMException("GCode data size scan aborted", "AbortError");
}
 ...
 */
export async function scanGCodeDataDirectory(
  options: GCodeDataSizeScanOptions,
  ...
  let terminalPartialReason: Exclude<GCodeDataSizePartialReason, "io_error"> | null = null;
...
export function isGCodeDataSizeScanResult(value: unknown): value is GCodeDataSizeScanResult {
  if (typeof value !== "object" || value === null) {
    return false;
  }
  const candidate = value as Partial<GCodeDataSizeScanResult>;
  const finiteNumbers = [
    candidate.bytes,
