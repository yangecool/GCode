...
import {
  HELP_TOGGLE_GCODE_STDIO_TAP_MENU_ID,
  HELP_TOGGLE_DEV_TOOLS_MENU_ID,
} from "./desktopCommandHandlers.js";

const HELP_GCODE_ENDPOINT_PRODUCTION_MENU_ID = "help.gcode-endpoint.production";
...
export function updateGCodeStdioTapDevMenuState() {
  const menu = Menu.getApplicationMenu();
  const item = menu?.getMenuItemById(HELP_TOGGLE_GCODE_STDIO_TAP_MENU_ID);
  if (!item) {
    return;
  }
  const state = readGCodeStdioTapDevState();
...
function buildApplicationMenuTemplate(options: {
  ...
  gcodeEndpointSelection?: "production" | "test" | "custom";
  ...
  const stdioTapState = readGCodeStdioTapDevState();
    ...
    },
    {
      ...
      submenu: [
        ...
        ...(isLocalDevelopmentRuntime && stdioTapState.visible
          ? [
              {
                id: HELP_TOGGLE_GCODE_STDIO_TAP_MENU_ID,
                label: getLabel(desktopMenuMessageIds.helpToggleGCodeStdioTap),
                type: "checkbox" as const,
                checked: stdioTapState.enabled,
                click: () =>
                  void options.executeDesktopCommand(DesktopCommandIds.ToggleGCodeStdioTapDevProxy),
              },
              { type: "separator" as const },
            ]
          : []),
        ...(GCODE_ENV === "test"
          ? [
              {
                label: getLabel(desktopMenuMessageIds.helpGCodeEndpoint),
                submenu: [
                  {
                    id: HELP_GCODE_ENDPOINT_PRODUCTION_MENU_ID,
                    label: getLabel(desktopMenuMessageIds.helpGCodeEndpointProduction),
                    type: "radio" as const,
                    checked: (options.gcodeEndpointSelection ?? "production") === "production",
                    click: () =>
                      void options.executeDesktopCommand(
                        DesktopCommandIds.SetGCodeEndpointProduction,
                      ),
                  },
                  { type: "separator" as const },
                  {
                    label: getLabel(desktopMenuMessageIds.helpGCodeEndpointCustom),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.SetGCodeEndpointCustom),
                  },
                  {
                    label: getLabel(desktopMenuMessageIds.helpGCodeEndpointReset),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.ResetGCodeEndpoint),
                  },
                ],
              },
              { type: "separator" as const },
  ...
  Menu.setApplicationMenu(
    Menu.buildFromTemplate(
      buildApplicationMenuTemplate({
        ...
        gcodeEndpointSelection: options.gcodeEndpointSelection,
