...
export interface GCodeDataSizeTelemetryState {
  lastReportedAt?: number;
  reportReservedAt?: number;
}

export async function readGCodeDataSizeTelemetryState(
  stateFile: string,
): Promise<GCodeDataSizeTelemetryState | null> {
  try {
    const parsed: unknown = JSON.parse(await readFile(stateFile, "utf8"));
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
      throw new TypeError("Invalid GCode data size telemetry state");
    }
    const record = parsed as Record<string, unknown>;
    const state: GCodeDataSizeTelemetryState = {};
    if (record.lastReportedAt !== undefined) {
      if (typeof record.lastReportedAt !== "number" || !Number.isFinite(record.lastReportedAt)) {
        throw new TypeError("Invalid lastReportedAt in GCode data size telemetry state");
      }
    ...
    if (record.reportReservedAt !== undefined) {
      if (
        ...
        throw new TypeError("Invalid reportReservedAt in GCode data size telemetry state");
      }
...
export async function writeGCodeDataSizeTelemetryState(
  stateFile: string,
  state: GCodeDataSizeTelemetryState,
): Promise<void> {
