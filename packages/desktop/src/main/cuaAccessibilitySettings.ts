...
import type { CuaHelperInstallerOptions } from "@gcode/services/node";
import {
  resolveHelperPermissionSubjectIdentity,
  type HelperPermissionSubjectIdentity,
} from "@gcode/services/cua-permission-broker";
import type {
...
async function verifyHelperPermissionIdentityUnchanged(
  ...
  if (!cuaHelperBundleFingerprintUnchanged(identity.appPath, fingerprint)) {
    throw new Error(`GCode Computer Use changed while its ${phase} signature was being verified`);
  }
  ...
  if (!sameHelperPermissionIdentity(identity, currentIdentity)) {
    throw new Error(`GCode Computer Use permission identity changed during ${phase} verification`);
  }
  if (!cuaHelperBundleFingerprintUnchanged(identity.appPath, fingerprint)) {
    throw new Error(
      `GCode Computer Use changed while its ${phase} permission identity was being resolved`,
    );
  }
  return fingerprint;
}
...
export async function openCuaPermissionOnboarding(
  ...
  if (platform !== "darwin") {
    return {
      success: false,
      error: "GCode Computer Use permissions are only available on macOS.",
    };
  }
  ...
  try {
    ...
    // GCODE_CUA_HELPER_ALLOW_UNSIGNED_LOCAL 承接：通过 dev 校验时 ensureInstalled 会正常返回本地 app，
    // 根本不会进到这个 catch；只有真正校验失败才会到这里。
    return {
      success: false,
      error: `GCode Computer Use is unavailable (install/verification failed): ${messageOf(error)}`,
    };
  }
  ...
  try {
    ...
    return {
      success: false,
      returnedFromSettings: false,
      error: `GCode Computer Use permission identity verification failed: ${messageOf(error)}`,
    };
  }
...
export async function prepareCuaHelperPermissionDrag(
  ...
  try {
    ...
    if (!cuaHelperBundleFingerprintUnchanged(helperAppPath, verifiedFingerprint)) {
      throw new Error("GCode Computer Use changed while its drag signature was being verified");
    }
    const identity = await (
      options.resolveHelperIdentity ?? resolveHelperPermissionSubjectIdentity
    )(helperAppPath);
    if (!cuaHelperBundleFingerprintUnchanged(helperAppPath, verifiedFingerprint)) {
      throw new Error("GCode Computer Use changed while its drag identity was being resolved");
    }
    return {
      success: true,
      helperAppPath,
