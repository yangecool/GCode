import type { GCodeAutomationRunOutcome, GCodeAutomationTrigger } from "@gcode/shared";
...
interface CronRunLifecycleRepo {
  ...
  markRunOutcome(runId: string, outcome: GCodeAutomationRunOutcome, error?: string): Promise<void>;
...
export async function settleCronRunTerminalOutcome(
  params: CronRunLifecycleIdentity & {
    repo: CronRunLifecycleRepo;
    outcome: Exclude<GCodeAutomationRunOutcome, "running">;
    error?: string;
    logWarn: LogWarn;
  },
): Promise<void> {
