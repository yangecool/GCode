...
import { matchesTaskListMembershipKind } from "@gcode/shared/gcode-protocol-v4";
import type {
...
function taskKey(task: Pick<GCodeTaskMeta, "taskId" | "workspacePath" | "workspaceIdentity">) {
...
function compareItems(
  ...
  sortBy: GCodeTaskListQuery["sortBy"],
): number {
...
function sessionOverlay(
  summary: import("@gcode/shared/gcode-protocol-v4").SessionSummary,
...
function liveStatusFromMeta(meta: GCodeTaskMeta): WindowHostControllerTaskRow["liveStatus"] {
...
export function createWindowHostControllerRuntime(options: {
  createId: () => string;
  resolveSource: (scope: GCodeTaskListWorkspaceScope) => ResolvedWindowHostControllerSource | null;
  onSourceError?: (
    scope: WindowHostControllerSourceScope,
  ...
  const sourceTaskServices = new Map<string, IGCodeTaskService>();
  const sourceSnapshotTaskServices = new Map<string, IGCodeTaskService>();
  const sourceEventSubscriptions = new Map<string, { dispose(): void }>();
  const sourceRefreshFlights = new Map<
    string,
    { taskService: IGCodeTaskService; promise: Promise<void> }
  >();
  ...
  const sourceSessionObservers = new Map<
    string,
    { agentService: IGCodeAgentService; observer: WindowHostSessionsIndexObserver }
  >();
  ...
  return {
    ...
    async resolveTaskAddress(params: {
      ...
      attachmentScope?: import("@gcode/shared").WindowHostAttachmentScope;
