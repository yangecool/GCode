...
import { resolveGCodeEndpointOrigin, pickProductEndpointEnv } from "@gcode/shared/gcodeEndpoint";
...
function resolveGCodeEnv(value: string | undefined): "test" | "production" {
...
function createE2EUIRendererCoveragePlugin(repoRoot: string): Plugin {
  ...
  return {
    name: "gcode:e2e-ui-source-coverage",
    enforce: "pre",
...
export default defineConfig(({ mode }) => {
  // `.env*` 只提供链接常量；当前产品环境由启动脚本或 CI 注入 GCODE_ENV。
  const env = { ...loadEnv(mode, "../..", ""), ...process.env };
  const repoRoot = resolve(__dirname, "../..");
  const gcodeEnv = resolveGCodeEnv(env.GCODE_ENV);
  // 安装包身份与后端环境分轴；renderer 用它决定是否展示更新入口。
  const gcodeProductFlavor = resolveDesktopProductFlavor({
    ...process.env,
    ...env,
    GCODE_ENV: gcodeEnv,
  });
  const e2eCoverageEnabled =
    env.GCODE_E2E_COVERAGE === "1" || process.env.GCODE_E2E_COVERAGE === "1";
  const e2eStoreBridgeEnabled =
    env.VITE_GCODE_E2E_STORE_BRIDGE === "1" || process.env.VITE_GCODE_E2E_STORE_BRIDGE === "1";
  const gcodeEndpointOrigin = resolveGCodeEndpointOrigin({
    env: gcodeEnv,
    envBaseOrigin: env.GCODE_BASE_URL ?? env.GCODE_ENDPOINT_ORIGIN,
  });
  ...
  return {
    ...
    define: {
      ...
      __GCODE_LOCAL_DEVELOPMENT_RUNTIME__: JSON.stringify(mode !== "production"),
      "import.meta.env.VITE_GCODE_BASE_URL": JSON.stringify(gcodeEndpointOrigin),
      // 兼容旧 renderer 读取名；新代码统一读 VITE_GCODE_BASE_URL。
      "import.meta.env.VITE_GCODE_ENDPOINT_ORIGIN": JSON.stringify(gcodeEndpointOrigin),
      ...
      // E2E store bridge 只能由 WDIO 专用变量打开，避免把 GCODE_ENV=test 产品环境误当成测试运行态。
      "import.meta.env.VITE_GCODE_E2E_STORE_BRIDGE": JSON.stringify(
