import { ProxyChannel, type IChannelClient } from "@gcode/rpc";
import {
  IFileService,
...
export class RemoteServiceAccess implements IServiceAccessor {
  ...
  readonly gcodeTaskService: IGCodeTaskService;
  readonly windowControllerService: IWindowControllerService;
  readonly gcodeAgentService: IGCodeAgentService;
  readonly gcodeSessionService: IGCodeSessionService;
  ...
  constructor(channelClient: IChannelClient) {
    ...
    this.gcodeTaskService = ProxyChannel.toService<IGCodeTaskService>(
      channelClient.getChannel(IGCodeTaskService.channelName),
    );
    ...
    this.gcodeAgentService = ProxyChannel.toService<IGCodeAgentService>(
      channelClient.getChannel(IGCodeAgentService.channelName),
    );
    this.gcodeSessionService = ProxyChannel.toService<IGCodeSessionService>(
      channelClient.getChannel(IGCodeSessionService.channelName),
    );
